...
try:
    from search import run_search_pipeline as run_nexora_search_pipeline
except Exception:
    run_nexora_search_pipeline = None
...
APP_VERSION = "10.15.0-main-layered-memory-quality"
...
MEMORY_STACK_MODE = "chatgpt_style_layered_memory"
MEMORY_STACK_VERSION = "profile_project_conversation_semantic_procedural_working_retrieval_reasoning_v1"
...
def memory_stack_payload() -> Dict[str, Any]:
    return {
        "mode": MEMORY_STACK_MODE,
        "version": MEMORY_STACK_VERSION,
        "level": CAPABILITY_SCALE_TEXT,
        "layers": [
            {
                "name": "profile_memory",
                "purpose": "Stable user preferences, identity hints, tone, length preference, and corrections.",
            },
            {
                "name": "project_memory",
                "purpose": "Long-running Nexora/project/investor/build context summarized instead of replaying every message.",
            },
            {
                "name": "conversation_memory",
                "purpose": "Recent session focus so follow-ups like 'this' and 'same as before' resolve correctly.",
            },
            {
                "name": "semantic_memory",
                "purpose": "General facts and learned topic context that can guide stable explanations.",
            },
            {
                "name": "procedural_memory",
                "purpose": "How-to skills, workflows, writing formats, code/debug patterns, and reusable methods.",
            },
            {
                "name": "working_memory",
                "purpose": "The current user message, inferred task, constraints, accuracy need, and best answer shape.",
            },
            {
                "name": "retrieval_system",
                "purpose": "Ranked recall by keywords, explicit importance, recency, and repeated use.",
            },
            {
                "name": "reasoning_layer",
                "purpose": "Silent intent parsing, route choice, factuality checks, and final answer shaping.",
            },
            {
                "name": "grammar_style_model",
                "purpose": "Polished grammar, sentence flow, readable structure, and normal text by default.",
            },
            {
                "name": "adaptive_length_controller",
                "purpose": "Answer length chosen from the user request, complexity, topic, and learned preference.",
            },
        ],
        "principles": [
            "Memory is selective: store important summaries, preferences, corrections, projects, and repeated patterns.",
            "Current conversation stays in working memory; long projects are compressed into durable summaries.",
            "Stable facts can be answered from model/context; current or changeable facts require verification.",
            "The final answer should show only the useful response, not backend routes, raw logs, or provider details.",
        ],
    }
...
def routing_matrix_payload() -> Dict[str, Any]:
    return {
        ...
        "intents": [
            ...
            "openai_product_agi_strategy",
            "deepmind_unified_ecosystem_strategy",
            "anthropic_safety_autonomy_strategy",
            "ai_lab_strategy_comparison",
            "workspace_agents_strategy",
            "multimodal_agent_strategy",
            "computer_control_robotics_strategy",
            "mcp_interoperability_strategy",
            "enterprise_agentic_workflow_automation",
            "legacy_system_integration",
            "enterprise_data_privacy_sovereignty",
            "custom_private_model_strategy",
            "human_in_loop_governance",
            "measurable_roi_agentic_ai",
            "agentic_reliability_guardrail",
            "long_term_planning_guardrail",
            "memory_learning_guardrail",
            "real_world_understanding_guardrail",
            "image_generation",
            "code_or_debug",
            "bug_triage",
...
SELF_LEARNING_FILE = DATA_DIR / "self_learning.json"
NEXORA_CORE_FILE = DATA_DIR / "nexora_core.json"
...
GUIDE_INDEX = BASE_DIR.parent / "frontend" / "guide.html"
MCP_INDEX = BASE_DIR.parent / "frontend" / "mcp.html"
...
ENV = os.getenv("NEXORA_ENV", os.getenv("SOFTWARE_ENV", os.getenv("ENV", "development"))).strip().lower() or "development"
is_development = ENV in ["development", "test"]
PRODUCTION_ENVIRONMENTS = {"production", "staging"}
if is_development:
    DATA_DIR.mkdir(parents=True, exist_ok=True)
    UPLOAD_DIR.mkdir(parents=True, exist_ok=True)
AUTH_PROTECTION_UNAVAILABLE_MESSAGE = "Authentication protection temporarily unavailable. Please try again later."
AUTH_RATE_LIMIT_MAX_ATTEMPTS = max(1, int(os.getenv("NEXORA_AUTH_RATE_LIMIT_MAX_ATTEMPTS", "20")))
AUTH_RATE_LIMIT_WINDOW_SECONDS = max(1, int(os.getenv("NEXORA_AUTH_RATE_LIMIT_WINDOW_SECONDS", "300")))
AUTH_RATE_LIMIT_TIMEOUT_SECONDS = max(0.1, float(os.getenv("NEXORA_AUTH_RATE_LIMIT_TIMEOUT_SECONDS", "2")))
UPSTASH_REDIS_REST_URL = os.getenv("UPSTASH_REDIS_REST_URL", "").strip().rstrip("/")
UPSTASH_REDIS_REST_TOKEN = os.getenv("UPSTASH_REDIS_REST_TOKEN", "").strip()

OLLAMA_URL = os.getenv("OLLAMA_URL", "http://127.0.0.1:11434")
FAST_MODEL = os.getenv("NEXORA_FAST_MODEL", "qwen2.5:1.5b")
THINKING_MODEL = os.getenv("NEXORA_THINKING_MODEL", FAST_MODEL)
DEFAULT_MODEL = FAST_MODEL
OLLAMA_MODE = os.getenv("NEXORA_OLLAMA_MODE", "primary").strip().lower()
OLLAMA_KEEP_ALIVE = os.getenv("NEXORA_OLLAMA_KEEP_ALIVE", "4h").strip()
AUTH_SECRET = os.getenv("NEXORA_AUTH_SECRET", "").strip() or hashlib.sha256((APP_NAME + str(BASE_DIR)).encode("utf-8")).hexdigest()
AUTH_TOKEN_TTL_DAYS = int(os.getenv("NEXORA_AUTH_TOKEN_TTL_DAYS", "30"))
...
MAX_MODEL_TOKENS = int(os.getenv("NEXORA_MAX_TOKENS", "12000"))
INSTANT_TIMEOUT = int(os.getenv("NEXORA_INSTANT_TIMEOUT", "65"))
THINKING_TIMEOUT = int(os.getenv("NEXORA_THINKING_TIMEOUT", "220"))
OLLAMA_INSTANT_TOTAL_TIMEOUT = int(os.getenv("NEXORA_OLLAMA_INSTANT_TOTAL_TIMEOUT", "38"))
OLLAMA_THINKING_TOTAL_TIMEOUT = int(os.getenv("NEXORA_OLLAMA_THINKING_TOTAL_TIMEOUT", "200"))
OLLAMA_INSTANT_MAX_TOKENS = int(os.getenv("NEXORA_OLLAMA_INSTANT_MAX_TOKENS", "1600"))
OLLAMA_THINKING_MAX_TOKENS = int(os.getenv("NEXORA_OLLAMA_THINKING_MAX_TOKENS", "7000"))
OLLAMA_PRIMARY_WAIT_SECONDS = int(os.getenv("NEXORA_OLLAMA_PRIMARY_WAIT_SECONDS", "14"))
OLLAMA_NUM_CTX = int(os.getenv("NEXORA_OLLAMA_NUM_CTX", "4096"))
OLLAMA_THINKING_NUM_CTX = int(os.getenv("NEXORA_OLLAMA_THINKING_NUM_CTX", "12288"))
OLLAMA_NUM_THREAD = int(os.getenv("NEXORA_OLLAMA_NUM_THREAD", str(max(1, min((os.cpu_count() or 2), 8)))))
OLLAMA_NUM_BATCH = int(os.getenv("NEXORA_OLLAMA_NUM_BATCH", "128"))
OLLAMA_STREAM = os.getenv("NEXORA_OLLAMA_STREAM", "false").strip().lower() not in {"0", "false", "off", "no"}
OLLAMA_MODELS_CACHE_TTL = float(os.getenv("NEXORA_OLLAMA_MODELS_CACHE_TTL", "20"))
FREE_CLUB_MODE = os.getenv("NEXORA_FREE_CLUB_MODE", "off" if FREE_API_PROVIDER == "ollama" else "auto").strip().lower()
...
POLLINATIONS_REVIEW_OLLAMA = os.getenv("NEXORA_POLLINATIONS_REVIEW_OLLAMA", "false" if FREE_API_PROVIDER == "ollama" else "true").strip().lower() not in {"0", "false", "off", "no"}
FREE_API_HARD_TIMEOUT = int(os.getenv("NEXORA_FREE_API_HARD_TIMEOUT", "16"))
...
MAX_MEMORY_ITEMS = int(os.getenv("NEXORA_MAX_MEMORY_ITEMS", "5000"))
MAX_MEMORY_CONTEXT_ITEMS = int(os.getenv("NEXORA_MAX_MEMORY_CONTEXT_ITEMS", "48"))
MAX_MEMORY_CONTEXT_CHARS = int(os.getenv("NEXORA_MEMORY_CONTEXT_CHARS", "12000"))
MAX_IMAGE_MEMORY_ITEMS = int(os.getenv("NEXORA_MAX_IMAGE_MEMORY_ITEMS", "600"))
MAX_PERSONA_RULES = int(os.getenv("NEXORA_MAX_PERSONA_RULES", "64"))
MAX_BEHAVIOR_EVENTS = int(os.getenv("NEXORA_MAX_BEHAVIOR_EVENTS", "240"))
AUTONOMOUS_SELF_LEARNING = os.getenv("NEXORA_AUTONOMOUS_SELF_LEARNING", "true").strip().lower() not in {"0", "false", "off", "no"}
MAX_SELF_LEARNING_RULES = int(os.getenv("NEXORA_MAX_SELF_LEARNING_RULES", "120"))
MAX_SELF_LEARNING_EVENTS = int(os.getenv("NEXORA_MAX_SELF_LEARNING_EVENTS", "320"))
MAX_SELF_LEARNING_SUMMARIES = int(os.getenv("NEXORA_MAX_SELF_LEARNING_SUMMARIES", "80"))
...
NEXORA_NATIVE_CORE_LOOP = os.getenv("NEXORA_NATIVE_CORE_LOOP", "true").strip().lower() not in {"0", "false", "off", "no"}
NEXORA_NATIVE_CORE_INTERVAL = max(5, int(os.getenv("NEXORA_NATIVE_CORE_INTERVAL", "30")))
...
OLLAMA_EXECUTOR = concurrent.futures.ThreadPoolExecutor(max_workers=1, thread_name_prefix="nexora-ollama")
FREE_API_EXECUTOR = concurrent.futures.ThreadPoolExecutor(max_workers=2, thread_name_prefix="nexora-free-api")
NEXORA_CORE_STOP = threading.Event()
NEXORA_CORE_THREAD: Optional[threading.Thread] = None
LOG_LEVEL = os.getenv("NEXORA_LOG_LEVEL", "INFO").strip().upper() or "INFO"
logging.basicConfig(level=getattr(logging, LOG_LEVEL, logging.INFO))
LOGGER = logging.getLogger("nexora")

AUTH_RATE_LIMIT_MEMORY: Dict[str, Tuple[int, float]] = {}
AUTH_RATE_LIMIT_LOCK = threading.Lock()


def is_production_like_environment() -> bool:
    return ENV in PRODUCTION_ENVIRONMENTS
...
def validate_runtime_environment() -> None:
    """Fail startup before unsafe production fallbacks can serve traffic."""
    if ENV != "production":
        return
    ...
    if not _env_value("DATABASE_URL", "SUPABASE_DB_URL"):
...
def _auth_rate_limit_key(request: Request) -> str:
    ...
    return f"auth_rate:{client_host}:{request.method}:{request.url.path}"
...
def _upstash_command(*args: Any) -> Any:
    ...
    response = HTTP.post(
        ...
        json=list(args),
        timeout=AUTH_RATE_LIMIT_TIMEOUT_SECONDS,
    )
...
def _in_memory_rate_limit(key: str, now: float) -> Dict[str, Any]:
    with AUTH_RATE_LIMIT_LOCK:
        count, expires_at = AUTH_RATE_LIMIT_MEMORY.get(key, (0, 0.0))
        if expires_at <= now:
            count = 0
            expires_at = now + AUTH_RATE_LIMIT_WINDOW_SECONDS
        count += 1
        AUTH_RATE_LIMIT_MEMORY[key] = (count, expires_at)
    blocked = count >= AUTH_RATE_LIMIT_MAX_ATTEMPTS
    return {
        "allowed": not blocked,
        "status_code": 429 if blocked else 200,
        "count": count,
        "storage": "memory",
        "detail": AUTH_PROTECTION_UNAVAILABLE_MESSAGE if blocked else "",
    }


def check_rate_limit(request: Request) -> Dict[str, Any]:
    """Auth rate limit that fails closed outside development/test."""
    key = _auth_rate_limit_key(request)
    if UPSTASH_REDIS_REST_URL and UPSTASH_REDIS_REST_TOKEN:
        try:
            count = int(_upstash_command("INCR", key) or 0)
            if count == 1:
                _upstash_command("EXPIRE", key, AUTH_RATE_LIMIT_WINDOW_SECONDS)
            blocked = count >= AUTH_RATE_LIMIT_MAX_ATTEMPTS
            return {
                "allowed": not blocked,
                "status_code": 429 if blocked else 200,
            ...
            LOGGER.warning("auth_rate_limit_storage_error: %s", error)
            if is_development:
                return _in_memory_rate_limit(key, time.time())
            return {
                "allowed": False,
                "status_code": 503,
                "count": 0,
                "storage": "unavailable",
                "detail": AUTH_PROTECTION_UNAVAILABLE_MESSAGE,
            }
...
async def enforce_auth_rate_limit(request: Request, call_next):
    if request.url.path.startswith("/auth/"):
        verdict = check_rate_limit(request)
        if not verdict.get("allowed"):
            return JSONResponse(
                status_code=int(verdict.get("status_code") or 503),
                content={
                    "ok": False,
                    "detail": verdict.get("detail") or AUTH_PROTECTION_UNAVAILABLE_MESSAGE,
...
class CustomerApiKeyRequest(BaseModel):
    name: str = Field("Default key", max_length=80)


class CustomerProjectRequest(BaseModel):
    name: str = Field(..., min_length=1, max_length=80)
    framework: str = Field("Generic", max_length=80)
    environment: str = Field("Production", max_length=80)
    url: str = Field("", max_length=300)
    description: str = Field("", max_length=800)


class CustomerEventIngestRequest(BaseModel):
    project_id: str = Field("", max_length=120)
    event_type: str = Field("agent_run", max_length=80)
    message: str = Field("", max_length=1000)
    provider: str = Field("generic", max_length=80)
    model: str = Field("unknown", max_length=160)
    latency_ms: int = Field(0, ge=0)
    prompt_tokens: int = Field(0, ge=0)
    completion_tokens: int = Field(0, ge=0)
    success: bool = True
    error: Optional[str] = Field(None, max_length=1000)
    trace_id: str = Field("", max_length=160)
...
class SearchPipelineRequest(BaseModel):
    ...
    max_sources: int = Field(8, ge=5, le=10)
    use_llm: Optional[bool] = True
    model: Optional[str] = Field(None, max_length=160)


class NexoraCoreFeedRequest(BaseModel):
    text: str = Field(..., min_length=1, max_length=20000)
    source: Optional[str] = Field("manual_feed", max_length=80)
    user_id: Optional[str] = None
    tags: Optional[List[str]] = None
    run_growth_cycle: Optional[bool] = True


class NexoraCoreGrowthRequest(BaseModel):
    cycles: int = Field(1, ge=1, le=25)
    source: Optional[str] = Field("manual_growth", max_length=80)
...
def make_user_id_from_email(email: str) -> str:
    return "usr_" + stable_hash(normalize_email(email), 18)
...
def hash_password(password: str, salt: Optional[str] = None) -> Dict[str, str]:
    ...
    return {"salt": salt, "hash": digest, "algorithm": "pbkdf2_sha256"}
...
def verify_password(password: str, password_record: Dict[str, Any]) -> bool:
    try:
        ...
        actual = hash_password(password, salt).get("hash", "")
...
def decode_access_token(token: str) -> Optional[str]:
    try:
        ...
        return user_id if re.fullmatch(r"[a-zA-Z0-9_\-]{4,80}", user_id) else None
    except Exception:
        return None
...
def bearer_token_from_request(request: Request) -> str:
    ...
    return request.headers.get("X-Nexora-Token", "").strip()
...
def public_base_url(request: Request) -> str:
    configured = PUBLIC_APP_URL.strip().rstrip("/")
    if configured:
        return configured
...
def supabase_session_from_payload(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if isinstance(payload.get("data"), dict) and isinstance(payload["data"].get("session"), dict):
...
def get_user_by_token(token: str) -> Optional[Dict[str, Any]]:
    if not token:
        return None
    if supabase_auth_client.configured:
        try:
...
def register_account(name: str, email: str, password: str) -> Dict[str, Any]:
    ...
    if existing_id and isinstance(users.get(existing_id), dict):
        raise HTTPException(status_code=409, detail="An account with this email already exists.")
    user_id = make_user_id_from_email(normalized_email)
    now = now_iso()
    user = {
        "id": user_id,
...
def authenticate_account(email: str, password: str) -> Dict[str, Any]:
    ...
    users = load_users()
    email_index = users.get("__email_index__") if isinstance(users.get("__email_index__"), dict) else {}
    user_id = email_index.get(normalized_email)
    user = users.get(user_id) if user_id else None
    if not isinstance(user, dict) or not verify_password(password, user.get("password", {})):
...
def local_fast_reply(message: str) -> Optional[str]:
    ...
    if not text:
        return None
    if (
        detect_task_intent(message, False) in FRONTIER_ROUTE_NAMES
        or detect_task_intent(message, False) in AI_LAB_STRATEGY_ROUTE_NAMES
        or detect_task_intent(message, False) in ENTERPRISE_AGENTIC_ROUTE_NAMES
    ):
        return None
    math_match = re.fullmatch(
        r"(?:what is|calculate|solve)?\s*(-?\d+(?:\.\d+)?)\s*"
        r"([+\-*/x]|plus|minus|times|multiplied by|divided by|over)\s*"
        r"(-?\d+(?:\.\d+)?)\??",
        text,
    )
    if math_match:
...
def local_ai_memory_architecture_reply(message: str) -> Optional[str]:
    lower = clean_text(message).lower()
    if not re.search(r"\b(nexora|chatgpt|chat gpt|assistant|ai|model)\b", lower):
        return None
    if not re.search(
        r"\b(memory|remember|preference|preferences|answer length|right length|grammar|accurate|accuracy|structured|context|project summary|retrieval|reasoning)\b",
        lower,
    ):
        return None
    return (
        "Nexora should work like a layered assistant, not like a single text generator.\n\n"
        "How it should remember and answer:\n"
        "- Profile memory: keep stable preferences such as tone, answer length, formatting, and corrections.\n"
        "- Project memory: summarize long-running work like Nexora, investors, technical changes, and design goals so it does not restart from zero.\n"
        "- Conversation memory: use the recent chat as working context, especially for follow-ups like this, same as before, or fix that.\n"
        "- Semantic memory: keep useful facts and topic context for stable questions.\n"
        "- Procedural memory: remember reusable skills, such as how to write letters, explain school answers, debug code, or plan workflows.\n"
        "- Retrieval: select only the most relevant memories instead of dumping everything into the prompt.\n"
        "- Reasoning gate: identify the real task, choose the right route, check for missing assumptions, and avoid generic templates.\n"
        "- Accuracy gate: answer stable facts directly, but verify current or changeable facts before making confident claims.\n"
        "- Grammar and style pass: polish punctuation, sentence flow, paragraph breaks, and formatting before the answer is shown.\n\n"
        "How it chooses answer length:\n"
        "- Simple chat gets a short answer.\n"
        "- School questions get exam-friendly answers with keywords and clear points.\n"
        "- Nexora and AI planning get more detail because the work needs architecture, tradeoffs, and testing.\n"
        "- Investor or business questions stay practical, direct, and outcome-focused.\n"
        "- AGI or agentic AI discussions can be longer because the ideas need deeper explanation.\n\n"
        "Bottom line:\n"
        "The answer becomes stronger when memory, routing, reasoning, accuracy checking, grammar cleanup, and adaptive length all work together silently. The user should only see the final clean answer."
    )
...
def local_knowledge_reply(message: str, presentation_style: str = "balanced") -> Optional[str]:
    lower = clean_text(message).lower()
    ai_architecture_reply = local_ai_memory_architecture_reply(message)
    if ai_architecture_reply:
        return ai_architecture_reply
    ...
    if re.search(r"\b(maurya|mauryan|mourya)\b", topic.lower()) or re.search(r"\b(maurya|mauryan|mourya)\b", lower):
        return (
            "The Maurya Empire was one of the largest and most powerful empires in ancient India. It began around 322 BCE when Chandragupta Maurya founded it after defeating the Nanda dynasty, with guidance traditionally linked to Chanakya/Kautilya.\n\n"
            "Key points:\n"
            "- Chandragupta Maurya built the empire and created a strong central administration.\n"
            "- Bindusara expanded and maintained Mauryan power after Chandragupta.\n"
            "- Ashoka became the most famous Mauryan ruler after the Kalinga War; he later promoted dhamma, non-violence, welfare, and Buddhist ideas.\n"
            "- The empire controlled large parts of the Indian subcontinent, with Pataliputra as an important capital.\n"
            "- Mauryan rule is remembered for organized government, roads, trade, taxation, spies, public works, and Ashoka's rock and pillar edicts.\n\n"
            "The empire declined after Ashoka's death because later rulers were weaker, the empire was very large to manage, and regional powers grew stronger. It finally ended around 185 BCE when the last Mauryan ruler, Brihadratha, was killed by Pushyamitra Shunga.\n\n"
            "In short, the Maurya Empire is important because it showed how ancient India could be united under a powerful central government and left a major political, cultural, and religious legacy."
        )
...
def build_fitness_plan(message: str) -> str:
    ...
    if days <= 3:
        ...
        weekly_plan = [
            "Day 1: Upper body strength: push-ups or bench press, rows, shoulder press, curls, triceps, and plank.",
...
def local_study_tip_reply(message: str) -> Optional[str]:
    lower = clean_text(message).lower()
    if not re.search(r"\b(study tip|study tips|quick study|quick tip|tip for study|tip to study|focus tip|revision tip|study better)\b", lower):
        return None
    return (
        "Use active recall: close the book, write what you remember, then check and fix only the gaps.\n\n"
        "For a quick session:\n"
        "1. Read for 10 minutes.\n"
        "2. Recall without looking for 5 minutes.\n"
        "3. Practise 3-5 questions.\n\n"
        "This works better than rereading because it trains your brain to retrieve the answer."
    )
...
def local_health_info_reply(message: str) -> Optional[str]:
    lower = clean_text(message).lower()
    if re.search(r"\bsleep\b", lower) and re.search(r"\b(learning|memory|study|studying|brain|school|class|why|how does|explain)\b", lower):
        return None
...
def is_high_confidence_local_reply(message: str, presentation_style: str = "balanced") -> bool:
    lower = clean_text(message).lower()
    if re.search(r"\b(maurya|mauryan|mourya)\b", lower):
        return True
...
def detect_task_intent(message: str, use_research: bool = False) -> str:
    ...
    if re.fullmatch(
        ...
        return "calculation"
    if re.search(r"\b(reliability|reliable|95%\s*accurate|ninety[- ]?five percent accurate|5%\s*error|five percent error|autonomous operation|ai can still make mistakes|major problems)\b", lower) and re.search(r"\b(ai|agent|autonomous|workflow|mistakes?|error|accuracy)\b", lower):
        return "agentic_reliability_guardrail"
    ...
    if re.search(r"\b(long[- ]?term planning|months or years|months|years|very long chains?|long chains? of reasoning|long chains? of execution|multi[- ]?month|multi[- ]?year)\b", lower) and re.search(r"\b(ai|agent|goal|planning|execution|reasoning)\b", lower):
        return "long_term_planning_guardrail"
    if re.search(r"\b(ai memory|memory systems?|long[- ]?term memory|build knowledge continuously|human learning|equivalent to human learning|learn continuously|knowledge continuously)\b", lower):
        return "memory_learning_guardrail"
    if re.search(r"\b(real[- ]?world understanding|world is messy|businesses,?\s*laws,?\s*people|environments change|laws change|people change|messy world|real world is messy)\b", lower):
        return "real_world_understanding_guardrail"
    if re.search(r"\b(measurable roi|roi|employee hour reduction|hour reduction|cost savings?|compute costs?|massive compute|investor|founders?|pitch|guarantee|business value|value delivered)\b", lower) and re.search(r"\b(ai|agent|workflow|system|nexora|company|enterprise|founder|investor)\b", lower):
        return "measurable_roi_agentic_ai"
    if re.search(r"\b(agentic ai workflows?|takeaway for founders?|founders?|investors?)\b", lower) and re.search(r"\b(workflow|legacy|privacy|sovereignty|private data|human[- ]?in[- ]?the[- ]?loop|roi|employee hour|compute costs?|enterprise|company)\b", lower):
        return "enterprise_agentic_workflow_automation"
    if re.search(r"\b(human[- ]?in[- ]?the[- ]?loop|human in the loop|approval gate|human approval|governance dashboard|strict governance|accountability|unauthorized financial|pause for human approval|transactional action|risk management)\b", lower):
        return "human_in_loop_governance"
    if re.search(r"\b(data privacy|strategic sovereignty|sovereignty|dpdp|digital personal data protection|on[- ]?premise|on[- ]?prem|private cloud|virtual private network|vpn|corporate firewall|data isolation|public models? train|proprietary information|closed[- ]?loop systems?)\b", lower):
        return "enterprise_data_privacy_sovereignty"
    if re.search(r"\b(custom local models?|trained on private data|private data|proprietary knowledge graphs?|internal wikis?|customer support tickets?|brand voice guidelines?|industry jargon|local high court precedents?|shipping routes?|supply chains?)\b", lower):
        return "custom_private_model_strategy"
    if re.search(r"\b(legacy systems?|legacy software|no more silos|silos|salesforce|sap|erp|sql databases?|internal slack|slack channels?|current stacks?|embedded infrastructure|quiet infrastructure|plug into legacy|deep integration)\b", lower):
        return "legacy_system_integration"
    if re.search(r"\b(agentic ai workflows?|multi[- ]?step workflow automation|high[- ]?value.*workflow|operational workflow|cross[- ]?functional agents?|multi[- ]?stage business pipelines?|procurement agents?|vendor negotiation|finance agents?|invoices?|bank deposits?|reconcile accounts?|flag discrepancies|audit)\b", lower):
        return "enterprise_agentic_workflow_automation"
    ai_lab_mentions = sum(
        1
        for pattern in [
            r"\b(openai|o1|o3)\b",
            r"\b(google deepmind|deepmind|gemini|project astra|project mariner)\b",
            r"\b(anthropic|claude|claude code)\b",
        ]
        if re.search(pattern, lower)
    )
    if (
        re.search(r"\b(compare|comparison|versus|vs\.?)\b|strategy\s*comparison|focus\s*area|primary\s*strength|agent\s*interface|standard\s*used", lower)
        and ai_lab_mentions >= 2
    ):
        return "ai_lab_strategy_comparison"
    if re.search(r"\b(workspace agents?|no[- ]?code workspace agents?|custom gpts?|enterprise agents?|digital workers?)\b", lower):
        return "workspace_agents_strategy"
    if re.search(r"\b(multimodal domination|multimodal|advanced voice|realtime voice|voice, video|voice/video|video, text|image tasks)\b", lower):
        return "multimodal_agent_strategy"
    if re.search(r"\b(project mariner|computer control|computer use|desktop interfaces?|mouse and keyboard)\b", lower) or (
        re.search(r"\b(robotics|humanoid)\b", lower)
        and re.search(r"\b(openai|google|deepmind|anthropic|agent|strategy|roadmap|computer control)\b", lower)
    ):
        return "computer_control_robotics_strategy"
    if re.search(r"\b(model context protocol|mcp|interoperability standard|corporate data servers?|secure data servers?)\b", lower):
        return "mcp_interoperability_strategy"
    if re.search(r"\b(openai|o1|o3|advanced reasoning|reasoning engines?|product[- ]?driven path|rapid commercial scaling)\b", lower) and re.search(r"\b(agi|roadmap|strategy|workspace|agentic|multimodal|reasoning|voice|video)\b", lower):
        return "openai_product_agi_strategy"
    if re.search(r"\b(google deepmind|deepmind|gemini|project astra|universal assistants?|android|chrome|google workspace|workspace intelligence|semantic layers?)\b", lower):
        return "deepmind_unified_ecosystem_strategy"
    if re.search(r"\b(anthropic|claude code|terminal agents?|safety[- ]?first|enterprise autonomy|project glasswing|claude mythos|zero[- ]day|cybersecurity|security alignment)\b", lower):
        return "anthropic_safety_autonomy_strategy"
    if re.search(r"\b(artificial general intelligence|agi|general intelligence|agi layer|agi capability)\b", lower):
        return "agi_capability"
    if re.search(r"\b(human[- ]?like reasoning|complex logic|multi[- ]?step planning|abstract thinking|abstract reasoning|deep reasoning)\b", lower):
        return "human_like_reasoning"
    if re.search(r"\b(factuality|truthfulness|truth layer|truth engine|truth check|factual truth|hallucination guard|verified truth|fact[- ]?checking)\b", lower):
        return "factuality_truth"
    if re.search(r"\b(physical robotics|robotics|humanoid robot|humanoid body|robot body|robot control|real[- ]?world labor|embodied ai|embodiment)\b", lower):
        return "robotics_planning"
    if re.search(r"\b(molecular structure|protein folding|protein structure|molecule prediction|predict molecules?|predict molecular|drug discovery|new medicines?|medicine discovery|biotech breakthrough|biotech|bioinformatics|admet)\b", lower):
        return "molecular_structure_prediction" if re.search(r"\b(molecular structure|protein folding|protein structure|molecule prediction|predict molecules?|predict molecular)\b", lower) else "biotech_discovery"
...
ENTERPRISE_AGENTIC_ROUTE_NAMES = {
    "enterprise_agentic_workflow_automation",
    "legacy_system_integration",
    "enterprise_data_privacy_sovereignty",
    "custom_private_model_strategy",
...
def local_code_or_debug_reply(message: str) -> Optional[str]:
    text = clean_text(message)
    lower = text.lower()
    if re.search(r"\b(recursion|recursive)\b", lower) and re.search(r"\b(python|code|example)\b", lower):
        return (
            ...
            "```\n\n"
            "How it works:\n"
            "1. Call factorial(5); it asks for factorial(4).\n"
            "2. The returned value is multiplied by five.\n"
            "3. Call factorial(4); it asks for factorial(3).\n"
            "4. The returned value is multiplied by four.\n"
            "5. The calls continue until `factorial(0)` returns `1`.\n"
            "6. Then the answers multiply back up through 1, 2, 6, 24, and 120.\n\n"
            "Bottom line:\n"
            "Recursion is useful when a problem can naturally be broken into smaller copies of itself, but every recursive function must have a clear stopping point."
        )
    if re.search(r"\b(debug|bug|error|traceback|not working|fails?|fix)\b", lower):
        return (
            "To debug it properly, start with the exact failing point.\n\n"
            "Use this order:\n"
            "1. Copy the exact error message or traceback.\n"
            "2. Identify the file and line number where it fails.\n"
            "3. Check the input values reaching that line.\n"
            "4. Reproduce the bug with the smallest possible example.\n"
            "5. Patch one cause at a time, then rerun the test.\n\n"
            "If you send the code and error, I can point to the likely cause and give the exact fix."
        )
    if re.search(r"\b(python|javascript|html|css|fastapi|api|backend|frontend|code)\b", lower):
        return (
            "For a coding task, the safest path is to define the goal, inspect the exact code or error, make the smallest fix, and run a check.\n\n"
            "Useful structure:\n"
            "- Goal: what the code should do.\n"
            "- Current behavior: what it does instead.\n"
            "- Evidence: error message, file name, or broken output.\n"
            "- Fix: one targeted change.\n"
            "- Verification: the command or action that proves it works.\n\n"
            "Send the relevant code or traceback, and I will make the fix specific."
        )
    return None
...
def local_reply_for_intent(message: str, task_intent: str, session_id: Optional[str] = None) -> Optional[str]:
    if task_intent in {
        ...
        return None
    if task_intent == "code_or_debug":
        return local_code_or_debug_reply(message)
    ...
    study_tip = local_study_tip_reply(message)
    if study_tip:
        return study_tip
...
FIELD_AWARE_INTENTS = {
    "agi_capability", "human_like_reasoning", "factuality_truth", "robotics_planning",
    "biotech_discovery", "molecular_structure_prediction",
    "openai_product_agi_strategy", "deepmind_unified_ecosystem_strategy",
    "anthropic_safety_autonomy_strategy", "ai_lab_strategy_comparison",
    "workspace_agents_strategy", "multimodal_agent_strategy",
    "computer_control_robotics_strategy", "mcp_interoperability_strategy",
    "enterprise_agentic_workflow_automation", "legacy_system_integration",
    "enterprise_data_privacy_sovereignty", "custom_private_model_strategy",
    "human_in_loop_governance", "measurable_roi_agentic_ai",
    "agentic_reliability_guardrail", "long_term_planning_guardrail",
    "memory_learning_guardrail", "real_world_understanding_guardrail",
...
FIELD_PROFILES: Dict[str, Tuple[str, List[str], str]] = {
    "agi_capability": ("AGI-style capability", ["Clarify the goal and scope.", "Decompose the task into reasoning, planning, memory, tools, and verification.", "State honest boundaries between app-level capability and true AGI.", "Return a useful supervised plan."], "AGI-style behavior should improve general reasoning and autonomy without falsely claiming true AGI."),
    "human_like_reasoning": ("Human-like reasoning", ["Identify the real goal.", "Track assumptions and constraints.", "Reason through multiple steps privately.", "Check contradictions and practical consequences.", "Give a clean final answer."], "Human-like reasoning means better planning, abstraction, and judgment, not pretending to be human."),
    "factuality_truth": ("Factuality and truth", ["Classify facts as stable or current.", "Verify changeable claims with sources when available.", "Label uncertainty.", "Remove unsupported exact claims.", "Prefer correction over confidence."], "Truth mode should be cautious, source-aware, and resistant to hallucination."),
    "robotics_planning": ("Physical robotics", ["Define the task and environment.", "List sensors, actuators, and control needs.", "Plan perception, movement, safety, and verification.", "Require simulation, logs, human override, and supervised execution."], "Robotics support is planning and control design unless approved hardware is connected safely."),
    "biotech_discovery": ("Biotech discovery", ["Define target, mechanism, and evidence need.", "Review literature and datasets.", "Plan molecular or protein analysis.", "Check safety, toxicity, ADMET, and validation needs.", "Separate hypothesis from proven result."], "Biotech answers must stay research-supportive and medically cautious."),
    "molecular_structure_prediction": ("Molecular structure prediction", ["Define the molecule or protein target.", "Identify validated prediction tools or datasets.", "Compare candidate structures and uncertainty.", "Flag lab validation and safety requirements."], "Molecular prediction support is reasoning and workflow help, not a substitute for validated scientific platforms or lab review."),
    "openai_product_agi_strategy": ("OpenAI product-driven AGI strategy", ["Map reasoning engines to user-facing products.", "Route workspace-agent questions to autonomous workflow planning.", "Handle voice, video, text, and image work as multimodal execution.", "Separate roadmap-style strategy from verified current facts."], "OpenAI-style strategy should focus on productized reasoning, multimodal interfaces, and agent workflows with factual caution."),
    "deepmind_unified_ecosystem_strategy": ("Google DeepMind unified ecosystem strategy", ["Connect Gemini-style intelligence to Android, Chrome, and Workspace context.", "Use universal-assistant framing for live audio, video, and screen understanding.", "Explain semantic context sharing across email, docs, drive, and calendar.", "Flag current product status as something to verify when asked."], "DeepMind-style strategy should emphasize ecosystem context, universal assistants, and embodied/computer-control research boundaries."),
    "anthropic_safety_autonomy_strategy": ("Anthropic safety-first autonomy strategy", ["Center code agents, terminal work, cybersecurity, and tool safety.", "Explain computer-use maturity for desktop interaction.", "Use MCP as secure model-to-data interoperability.", "Keep autonomy bounded by permissions, logs, and review."], "Anthropic-style strategy should emphasize precise coding agents, secure tool use, and safety-gated enterprise autonomy."),
    "ai_lab_strategy_comparison": ("AI lab strategy comparison", ["Compare primary strength.", "Compare agent interface.", "Compare ecosystem or standard.", "Compare risks and best use cases.", "Give a clear bottom-line recommendation."], "A strategy comparison should be compact, balanced, and useful for deciding what capability to copy into Nexora."),
    "workspace_agents_strategy": ("Workspace agents", ["Identify the workspace task.", "Connect files, email, docs, calendar, and tools.", "Plan autonomous steps with permission gates.", "Return task status, decisions, and next actions clearly."], "Workspace agents should behave like supervised digital workers, not uncontrolled background actors."),
    "multimodal_agent_strategy": ("Multimodal agents", ["Accept voice, text, image, video, and screen context.", "Fuse signals into one task understanding.", "Choose the right output format.", "Verify what was actually seen or heard before acting."], "Multimodal strategy should make interaction fluid while staying honest about perception uncertainty."),
    "computer_control_robotics_strategy": ("Computer-control and robotics agents", ["Understand the interface or environment.", "Plan actions before taking them.", "Use permissions, simulation, safety stops, and logs.", "Separate desktop control from physical robotics execution."], "Computer-control and robotics strategy should be action-capable only with safe tooling, permissions, and supervision."),
    "mcp_interoperability_strategy": ("MCP interoperability strategy", ["Define the data/tool source.", "Use a secure connector boundary.", "Limit permissions to the task.", "Log actions and keep outputs user-facing."], "MCP-style interoperability should connect models to tools and data without exposing private backend process details."),
    "enterprise_agentic_workflow_automation": ("Enterprise agentic workflow automation", ["Map the complete operational workflow.", "Split it into agent-owned steps, evidence checks, exceptions, and handoffs.", "Integrate with finance, procurement, CRM, ERP, database, and communication systems.", "Measure hours saved, error reduction, cycle-time improvement, and audit readiness."], "Enterprise buyers want agents that own measurable workflows, not generic chat helpers."),
    "legacy_system_integration": ("Legacy system integration", ["Identify the system of record.", "Use APIs, database connectors, RPA, webhooks, or secure file exchange.", "Keep the AI inside the user's existing work surface.", "Log every read, write, and suggested action."], "The best enterprise AI feels embedded in existing systems instead of becoming another silo."),
    "enterprise_data_privacy_sovereignty": ("Enterprise data privacy and sovereignty", ["Classify sensitive data.", "Choose on-prem, private cloud, VPC, or closed-loop deployment.", "Prevent training on private customer data unless explicitly approved.", "Use encryption, access controls, audit logs, and retention limits."], "Enterprise AI must protect proprietary data before it tries to be clever."),
    "custom_private_model_strategy": ("Custom private model strategy", ["Collect internal wikis, tickets, policies, documents, and domain examples.", "Build retrieval, knowledge graphs, or fine-tuned local models where useful.", "Evaluate on company-specific tasks.", "Refresh knowledge with governance."], "Generic intelligence becomes valuable when it understands the company's private context."),
    "human_in_loop_governance": ("Human-in-the-loop governance", ["Separate observe, recommend, draft, approve, and execute permissions.", "Pause before payments, sending messages, deleting data, or customer-impacting actions.", "Show evidence, confidence, owner, and approval status.", "Escalate uncertain or high-risk cases."], "Autonomy should increase speed without removing accountability."),
    "measurable_roi_agentic_ai": ("Measurable ROI for agentic AI", ["Define the employee workflow being replaced or accelerated.", "Baseline current hours, cost, errors, and cycle time.", "Measure savings after automation.", "Show payback period and risk controls."], "Founders should pitch measurable workflow outcomes, not a generic AI assistant."),
    "agentic_reliability_guardrail": ("Agentic reliability guardrail", ["Assume the AI can be wrong.", "Add validation, reconciliation, retries, approvals, and monitoring.", "Treat 95% accuracy as insufficient for unattended high-impact actions.", "Design fallback paths for the remaining error cases."], "Reliability is the difference between a demo and an autonomous business system."),
    "long_term_planning_guardrail": ("Long-term planning guardrail", ["Break months-long goals into milestones.", "Store state, decisions, owners, and deadlines.", "Review progress periodically.", "Re-plan when the environment changes."], "AI should manage long plans through checkpoints and state, not one giant reasoning chain."),
    "memory_learning_guardrail": ("Memory and learning guardrail", ["Store stable facts, preferences, project context, and corrections.", "Keep memory scoped, reviewable, and deletable.", "Distinguish saved memory from true human-like learning.", "Refresh stale memory with new evidence."], "AI memory should be useful and governed, not treated as human-equivalent learning."),
    "real_world_understanding_guardrail": ("Real-world understanding guardrail", ["Track changing laws, people, business rules, and environments.", "Use current evidence for changeable facts.", "Model exceptions and edge cases.", "Escalate when real-world ambiguity is high."], "Real-world understanding requires updates, uncertainty handling, and human review for messy situations."),
...
def agi_capability_reply(message: str) -> str:
    return (
        "AGI-style capability layer for Nexora:\n\n"
        "What this adds:\n"
        "- Goal reasoning: identify the real objective behind a messy request.\n"
        "- Task decomposition: break complex work into ordered steps, checks, and fallback paths.\n"
        "- Cross-domain reasoning: connect coding, study, business, research, planning, and writing when the task needs it.\n"
        "- Memory use: apply saved user preferences, corrections, project context, and prior summaries.\n"
        "- Tool routing: decide when to use local knowledge, Ollama reasoning, files, image flow, or real-time search.\n"
        "- Self-checking: test whether the answer is specific, accurate, useful, and not generic filler.\n\n"
        "Truth boundary:\n"
        "This is not true independent AGI or a conscious system. It is an app-level AGI-style reasoning and orchestration layer that makes Nexora behave more generally and autonomously inside this app.\n\n"
        "How Nexora should use it:\n"
        "1. Understand the actual user goal.\n"
        "2. Choose the best route.\n"
        "3. Plan privately.\n"
        "4. Use memory and tools when useful.\n"
        "5. Check factuality and usefulness.\n"
        "6. Return a clean final answer or action.\n\n"
        "Bottom line:\n"
        "Nexora can now route AGI-related requests into a stronger general-reasoning mode while staying honest about what is real and what is simulated."
    )


def human_like_reasoning_reply(message: str) -> str:
    return (
        "Human-like reasoning mode:\n\n"
        "Core behavior:\n"
        "- Multi-step planning: solve tasks in stages instead of jumping to a shallow answer.\n"
        "- Abstract thinking: identify patterns, tradeoffs, assumptions, and hidden constraints.\n"
        "- Cause-and-effect reasoning: explain why something happens, not only what happens.\n"
        "- Error checking: look for contradictions, missing details, weak logic, and route mistakes.\n"
        "- Practical judgment: choose the answer shape that best fits the user’s actual need.\n\n"
        "Answer process:\n"
        "1. Parse the request into goal, target, constraints, and success criteria.\n"
        "2. Decide whether the task needs a short answer, deep explanation, plan, table, code, or research.\n"
        "3. Reason through the important steps privately.\n"
        "4. Give the user the final answer with clear rationale, examples, and checks.\n\n"
        "Bottom line:\n"
        "This mode makes Nexora reason more like a careful assistant: structured, context-aware, and practical, without exposing hidden chain-of-thought."
    )


def factuality_truth_reply(message: str) -> str:
    return (
        "Factuality and truth mode:\n\n"
        "Truth protocol:\n"
        "- Separate stable knowledge from facts that may have changed.\n"
        "- Use real-time search for current roles, prices, laws, schedules, releases, news, products, and scientific updates.\n"
        "- Label uncertainty instead of pretending to know.\n"
        "- Avoid fake sources, fake dates, fake product metadata, and overconfident claims.\n"
        "- Repair answers that drift into generic filler or unsupported detail.\n\n"
        "How answers should look:\n"
        "- For stable facts: answer clearly and mention important caveats.\n"
        "- For current facts: cite evidence when available.\n"
        "- For uncertain facts: say what is known, what is not verified, and what would confirm it.\n\n"
        "Bottom line:\n"
        "Nexora’s truth layer should prefer a slightly cautious correct answer over a confident wrong answer."
    )


def robotics_planning_reply(message: str) -> str:
    return (
        "Physical robotics route:\n\n"
        "What Nexora can do digitally:\n"
        "- Plan humanoid or robotic workflows.\n"
        "- Break real-world labor tasks into perception, decision, movement, safety, and verification steps.\n"
        "- Help design robot-control logic, checklists, simulation plans, sensor requirements, and safety constraints.\n"
        "- Explain robotics concepts such as actuators, sensors, SLAM, path planning, grasping, control loops, and human-robot interaction.\n\n"
        "Truth boundary:\n"
        "Nexora cannot move a real humanoid body unless connected to approved robotics hardware, controllers, permissions, and safety systems. Physical execution must be supervised and safety-gated.\n\n"
        "Robotics planning structure:\n"
        "1. Define the task and environment.\n"
        "2. Identify sensors and required perception.\n"
        "3. Plan movement and manipulation steps.\n"
        "4. Add safety stops and human override.\n"
        "5. Simulate before real-world execution.\n"
        "6. Test slowly with logs and supervision.\n\n"
        "Bottom line:\n"
        "This adds robotics intelligence planning, not uncontrolled physical action."
    )


def biotech_discovery_reply(message: str) -> str:
    return (
        "Biotech and molecular-discovery route:\n\n"
        "What Nexora can help with:\n"
        "- Explain protein structure, molecular structure, drug discovery, ADMET, docking, and bioinformatics concepts.\n"
        "- Turn a biotech goal into a research workflow.\n"
        "- Generate hypotheses, literature-search questions, comparison tables, and experiment-planning checklists.\n"
        "- Help interpret stable scientific concepts and suggest what evidence would be needed next.\n\n"
        "Truth boundary:\n"
        "Nexora cannot actually discover a safe medicine in days by itself. Real drug discovery needs validated models, lab testing, clinical review, regulation, and expert supervision.\n\n"
        "Safe biotech workflow:\n"
        "1. Define the target disease, pathway, protein, or molecule.\n"
        "2. Gather verified literature and datasets.\n"
        "3. Predict or compare candidate structures using validated tools where available.\n"
        "4. Screen for binding, toxicity, selectivity, stability, and ADMET risks.\n"
        "5. Prioritize candidates for expert review and lab validation.\n"
        "6. Document uncertainty and safety constraints.\n\n"
        "Bottom line:\n"
        "This route makes Nexora useful for biotech reasoning and research planning, while keeping medical and lab claims honest."
    )


def ai_lab_strategy_reply(message: str, task_intent: str) -> str:
    if task_intent == "openai_product_agi_strategy":
        return (
            "OpenAI-style product-driven AGI strategy for Nexora:\n\n"
            "What this adds:\n"
            "- Advanced reasoning: send hard math, science, code, and planning tasks through deeper verification before answering.\n"
            "- Productized agents: turn broad user goals into workspace-style tasks, steps, checkpoints, and final deliverables.\n"
            "- Multimodal execution: support text, voice, image, file, and screen-aware workflows as one unified interaction.\n"
            "- Consumer and enterprise workflows: answer casual questions quickly, but handle work tasks with memory, context, and quality gates.\n\n"
            "How Nexora should use it:\n"
            "1. Detect whether the user wants an answer, a plan, a draft, a code change, research, or an autonomous workflow.\n"
            "2. Choose the right path and available capabilities.\n"
            "3. Reason privately, verify obvious failure points, and return only the clean result.\n"
            "4. For current OpenAI product claims, use real-time verification instead of guessing.\n\n"
            "Bottom line:\n"
            "This strategy makes Nexora act more like a product-level agent: fast for simple tasks, deeper for complex work, and structured for real outcomes."
        )
    if task_intent == "deepmind_unified_ecosystem_strategy":
        return (
            "Google DeepMind-style unified ecosystem strategy for Nexora:\n\n"
            "What this adds:\n"
            "- Universal-assistant behavior: understand live context from text, voice, images, files, screens, and app state.\n"
            "- Ecosystem memory: connect a user's project context across chats, files, tasks, reminders, images, and research.\n"
            "- Workspace intelligence: treat documents, notes, schedules, and prior messages as one semantic layer when permission is available.\n"
            "- Computer-control planning: reason about browser, desktop, or app actions before executing them.\n\n"
            "How Nexora should use it:\n"
            "1. Build one shared understanding of the user's context.\n"
            "2. Resolve vague references from memory and active project state.\n"
            "3. Ask fewer unnecessary questions by inferring the likely task.\n"
            "4. Verify current Google or Gemini product details with search when the user asks for latest status.\n\n"
            "Bottom line:\n"
            "This strategy pushes Nexora toward a unified assistant: less isolated chat, more context-aware help across the user's actual work."
        )
    if task_intent == "anthropic_safety_autonomy_strategy":
        return (
            "Anthropic-style safety-first enterprise autonomy strategy for Nexora:\n\n"
            "What this adds:\n"
            "- Coding-agent strength: handle terminal-style coding, debugging, repository changes, tests, and careful code review.\n"
            "- Computer-use discipline: plan mouse, keyboard, file, or browser actions before taking them.\n"
            "- Security posture: treat cybersecurity, permissions, and private data as high-risk by default.\n"
            "- MCP-style connectors: use controlled interfaces to connect models with data sources and app actions without exposing internal process details.\n\n"
            "How Nexora should use it:\n"
            "1. Identify the exact job and risk level.\n"
            "2. Use limited permissions and clear action boundaries.\n"
            "3. Check code, commands, data access, and outputs before finalizing.\n"
            "4. Show only the useful final answer or result, not internal process notes.\n\n"
            "Bottom line:\n"
            "This strategy makes Nexora stronger for engineering and enterprise work: precise, connector-aware, security-conscious, and review-driven."
        )
    if task_intent == "workspace_agents_strategy":
        return (
            "Workspace-agent strategy for Nexora:\n\n"
            "Core behavior:\n"
            "- Convert a user goal into a digital-worker task.\n"
            "- Use project memory, files, chats, artifacts, and available connectors as context.\n"
            "- Break work into steps, perform safe actions, and report the final result clearly.\n"
            "- Keep permission gates for deleting, publishing, sending, buying, or changing important data.\n\n"
            "Ideal answer shape:\n"
            "1. Goal understood.\n"
            "2. Plan.\n"
            "3. Work completed or next required input.\n"
            "4. Final result.\n\n"
            "Bottom line:\n"
            "Workspace agents make Nexora more useful because it stops only chatting and starts organizing work into finished outcomes."
        )
    if task_intent == "multimodal_agent_strategy":
        return (
            "Multimodal-agent strategy for Nexora:\n\n"
            "Core behavior:\n"
            "- Combine text, voice, images, generated images, files, and screen context into one understanding.\n"
            "- Choose the best response form: explanation, image prompt, code, table, checklist, draft, or direct action.\n"
            "- Clearly separate what was actually seen from what is inferred.\n"
            "- Use fast responses for simple input and deeper reasoning for complex multimodal tasks.\n\n"
            "Bottom line:\n"
            "This makes Nexora feel more natural because the user can bring any kind of input and still get one clean, useful answer."
        )
    if task_intent == "computer_control_robotics_strategy":
        return (
            "Computer-control and robotics strategy for Nexora:\n\n"
            "Digital control:\n"
            "- Understand the interface or app state.\n"
            "- Plan clicks, typing, file actions, or browser steps before doing them.\n"
            "- Use logs, confirmations, and rollback where possible.\n\n"
            "Physical robotics:\n"
            "- Plan perception, sensors, actuators, movement, safety stops, and human override.\n"
            "- Require simulation and supervised testing before real-world execution.\n"
            "- Never treat a chat answer as permission to move real hardware.\n\n"
            "Bottom line:\n"
            "This strategy gives Nexora action planning for computers and robotics while keeping dangerous execution gated and supervised."
        )
    if task_intent == "mcp_interoperability_strategy":
        return (
            "MCP interoperability strategy for Nexora:\n\n"
            "What this adds:\n"
            "- A standard way to connect AI to files, databases, services, and approved app capabilities.\n"
            "- Clear permission boundaries for what the AI can read, write, or execute.\n"
            "- Better context without exposing internal implementation details to the user.\n"
            "- Safer enterprise integration because each connector can be scoped and audited.\n\n"
            "How Nexora should use it:\n"
            "1. Identify the needed connector or data source.\n"
            "2. Use the smallest permission that can complete the job.\n"
            "3. Keep internal connector output hidden unless the user needs a specific detail.\n"
            "4. Return a polished final answer with only relevant results.\n\n"
            "Bottom line:\n"
            "MCP-style integration makes Nexora more extensible: more connectors, cleaner context, and fewer internal-detail leaks."
        )
    return (
        "AI lab strategy comparison for Nexora:\n\n"
        "OpenAI-style strategy:\n"
        "- Primary strength: productized reasoning, voice/video, and agent workflows.\n"
        "- Agent interface: workspace agents and custom workflows.\n"
        "- Best Nexora lesson: make reasoning useful as a product feature, not just a hidden model ability.\n"
        "- Risk to control: overconfident product or roadmap claims.\n\n"
        "Google DeepMind-style strategy:\n"
        "- Primary strength: ecosystem context across apps, devices, and Workspace.\n"
        "- Agent interface: Android, Chrome, Workspace, and live audio/video assistants.\n"
        "- Best Nexora lesson: connect context across the user's whole workspace.\n"
        "- Risk to control: privacy and context overreach.\n\n"
        "Anthropic-style strategy:\n"
        "- Primary strength: coding agents, security, and precise connector use.\n"
        "- Agent interface: terminal, desktop, folders, APIs, and MCP connectors.\n"
        "- Best Nexora lesson: keep autonomy safe, reviewed, and permission-aware.\n"
        "- Risk to control: becoming too cautious or too restricted to finish work.\n\n"
        "What Nexora should copy:\n"
        "1. From OpenAI: stronger reasoning, clean multimodal responses, and user-friendly agent workflows.\n"
        "2. From DeepMind: unified context across chats, files, projects, images, and real-time interaction.\n"
        "3. From Anthropic: coding-agent discipline, MCP-style connectors, security boundaries, and safe computer use.\n\n"
        "Truth boundary:\n"
        "Roadmaps and product names can change. If the user asks for the latest status of any company feature, Nexora should verify it with real-time search before claiming it as current.\n\n"
        "Bottom line:\n"
        "The best Nexora strategy is a hybrid: OpenAI-style product intelligence, DeepMind-style ecosystem context, and Anthropic-style safety/tool precision."
    )


def enterprise_agentic_strategy_reply(message: str, task_intent: str) -> str:
    if task_intent == "enterprise_agentic_workflow_automation":
        return (
            "Enterprise agentic workflow automation for Nexora:\n\n"
            "What this adds:\n"
            "- Own full workflows instead of only generating text.\n"
            "- Break a business process into steps, evidence checks, approvals, exceptions, and final outputs.\n"
            "- Support high-value workflows such as procurement, invoice reconciliation, account matching, audit preparation, customer operations, and internal reporting.\n"
            "- Track measurable business outcomes: hours saved, fewer errors, faster cycle time, cleaner audits, and reduced manual handoffs.\n\n"
            "How Nexora should use it:\n"
            "1. Identify the workflow owner and system of record.\n"
            "2. Map each step from input to final decision.\n"
            "3. Decide which steps the agent can do alone and which need approval.\n"
            "4. Validate outputs against data sources before presenting the result.\n"
            "5. Report work completed, exceptions found, and ROI impact.\n\n"
            "Bottom line:\n"
            "For enterprise buyers, Nexora should be positioned as a workflow operator with measurable ROI, not just a helpful chatbot."
        )
    if task_intent == "legacy_system_integration":
        return (
            "Legacy system integration strategy for Nexora:\n\n"
            "What this adds:\n"
            "- Let the AI work inside existing business systems instead of forcing employees into another app.\n"
            "- Connect to systems such as Salesforce, SAP/ERP, local SQL databases, internal Slack channels, files, forms, and internal dashboards.\n"
            "- Use the least invasive connector first: API, database read, webhook, secure file exchange, or RPA only when no cleaner option exists.\n"
            "- Keep every action traceable with timestamps, user permissions, and change history.\n\n"
            "How Nexora should use it:\n"
            "1. Identify where the employee already works.\n"
            "2. Identify the source of truth for each data type.\n"
            "3. Read safely first, then recommend, then request approval before write actions.\n"
            "4. Return the result in the user's current workflow surface.\n\n"
            "Bottom line:\n"
            "Nexora should act like quiet infrastructure embedded in legacy systems, not another isolated AI screen."
        )
    if task_intent == "enterprise_data_privacy_sovereignty":
        return (
            "Enterprise data privacy and sovereignty strategy for Nexora:\n\n"
            "What this adds:\n"
            "- Treat proprietary information, customer logs, financial data, health data, and internal documents as protected by default.\n"
            "- Support on-prem, private cloud, VPC, and closed-loop deployment patterns when the buyer requires data isolation.\n"
            "- Avoid using customer data for training unless there is explicit written approval.\n"
            "- Add access control, encryption, audit logs, retention limits, and data-residency rules.\n\n"
            "How Nexora should use it:\n"
            "1. Classify the data sensitivity before processing.\n"
            "2. Keep private data inside the approved boundary.\n"
            "3. Use local or open-weight models when the deployment requires it.\n"
            "4. Make deletion, review, and audit possible.\n\n"
            "Bottom line:\n"
            "Enterprise AI trust starts with data control. If the buyer cannot trust the data boundary, the intelligence does not matter."
        )
    if task_intent == "custom_private_model_strategy":
        return (
            "Custom private model strategy for Nexora:\n\n"
            "What this adds:\n"
            "- Teach Nexora the buyer's private context: internal wikis, SOPs, support tickets, contracts, policies, product docs, brand voice, and domain jargon.\n"
            "- Use retrieval, knowledge graphs, local embeddings, or private fine-tuning depending on the task and risk level.\n"
            "- Keep the model grounded in company-approved knowledge instead of generic internet-style answers.\n\n"
            "How Nexora should use it:\n"
            "1. Build a private knowledge base from approved documents.\n"
            "2. Create task-specific evaluation questions from real employee work.\n"
            "3. Test accuracy against company experts.\n"
            "4. Refresh the knowledge base when policies, laws, prices, or processes change.\n\n"
            "Bottom line:\n"
            "Generic intelligence is common; private context is where enterprise value appears."
        )
    if task_intent == "human_in_loop_governance":
        return (
            "Human-in-the-loop governance for Nexora:\n\n"
            "What this adds:\n"
            "- Split autonomy into safe levels: observe, summarize, recommend, draft, approve, and execute.\n"
            "- Require approval before payments, customer messages, deletions, legal submissions, financial postings, or other irreversible actions.\n"
            "- Show evidence, confidence, risk level, owner, approval status, and exception reason.\n"
            "- Escalate uncertain cases instead of pretending the AI is always correct.\n\n"
            "How Nexora should use it:\n"
            "1. Let the agent gather information and prepare a plan.\n"
            "2. Let a human approve high-impact actions.\n"
            "3. Log the decision and result.\n"
            "4. Use mistakes and approvals to improve future routing.\n\n"
            "Bottom line:\n"
            "Strong autonomy does not mean removing people. It means giving people faster, better-controlled decisions."
        )
    if task_intent == "measurable_roi_agentic_ai":
        return (
            "Measurable ROI strategy for Nexora:\n\n"
            "What this adds:\n"
            "- Start from a painful employee workflow, not from a generic AI feature.\n"
            "- Baseline current effort: hours per task, error rate, approval time, rework, software cost, and customer impact.\n"
            "- Measure after automation: time saved, cost avoided, fewer mistakes, faster close cycles, and audit improvements.\n"
            "- Keep compute costs controlled by using local routes, cached results, retrieval, small models for simple steps, and stronger models only when needed.\n\n"
            "Founder pitch shape:\n"
            "1. Exact legacy tools Nexora plugs into.\n"
            "2. Exact workflow Nexora owns.\n"
            "3. Employee hours reduced per month.\n"
            "4. Data boundary and approval controls.\n"
            "5. Payback period and risk reduction.\n\n"
            "Bottom line:\n"
            "Investors and companies notice AI that saves money, reduces risk, and fits existing operations."
        )
    if task_intent == "agentic_reliability_guardrail":
        return (
            "Reliability guardrail for agentic AI:\n\n"
            "The problem:\n"
            "Even a highly accurate AI can still make mistakes. If an autonomous system is 95% accurate, the remaining 5% can be dangerous when money, customers, compliance, or operations are involved.\n\n"
            "How Nexora should handle it:\n"
            "1. Validate important outputs against trusted data.\n"
            "2. Use reconciliation checks for finance, procurement, inventory, and audit tasks.\n"
            "3. Add retries, confidence thresholds, exception queues, and human approval for risky actions.\n"
            "4. Monitor failures and feed them into route-quality lessons.\n\n"
            "Bottom line:\n"
            "Reliability is not just model accuracy. It is validation, approvals, monitoring, and recovery when the AI is wrong."
        )
    if task_intent == "long_term_planning_guardrail":
        return (
            "Long-term planning guardrail for agentic AI:\n\n"
            "The problem:\n"
            "Humans can pursue goals over months or years. AI systems are better at short workflows than very long chains of planning and execution.\n\n"
            "How Nexora should handle it:\n"
            "1. Break long goals into milestones, owners, dates, and measurable outputs.\n"
            "2. Store project state after each step.\n"
            "3. Review progress on a schedule.\n"
            "4. Re-plan when constraints, laws, budgets, people, or priorities change.\n\n"
            "Bottom line:\n"
            "Nexora should manage long-term work through checkpoints and memory, not one massive uninterrupted reasoning chain."
        )
    if task_intent == "memory_learning_guardrail":
        return (
            "Memory and learning guardrail for agentic AI:\n\n"
            "The problem:\n"
            "Humans build knowledge continuously. AI memory can save useful facts and patterns, but it is not the same as human learning.\n\n"
            "How Nexora should handle it:\n"
            "1. Save stable user facts, preferences, project context, corrections, and workflow lessons.\n"
            "2. Keep memory scoped, reviewable, editable, and deletable.\n"
            "3. Mark stale or uncertain memories instead of blindly trusting them forever.\n"
            "4. Use private company knowledge only inside approved boundaries.\n\n"
            "Bottom line:\n"
            "Nexora's memory should improve continuity while staying governed, honest, and correctable."
        )
    if task_intent == "real_world_understanding_guardrail":
        return (
            "Real-world understanding guardrail for agentic AI:\n\n"
            "The problem:\n"
            "The world is messy. Businesses, laws, people, markets, prices, and environments change constantly, so a static answer can become wrong.\n\n"
            "How Nexora should handle it:\n"
            "1. Separate stable knowledge from changeable facts.\n"
            "2. Use current evidence when laws, people, policies, prices, products, or schedules may have changed.\n"
            "3. Add exception handling for messy human and business cases.\n"
            "4. Escalate ambiguous or high-risk situations to a person.\n\n"
            "Bottom line:\n"
            "Real-world understanding means updating, checking uncertainty, and knowing when the AI should not act alone."
        )
    return (
        "Enterprise agentic AI strategy for Nexora:\n\n"
        "What companies want:\n"
        "- Multi-step agents that own real operational workflows.\n"
        "- Integration with legacy systems instead of another isolated AI screen.\n"
        "- Strong privacy, on-prem or private-cloud options, and data isolation.\n"
        "- Custom private context from internal documents, tickets, policies, and workflows.\n"
        "- Human approval controls for financial, legal, customer-facing, and irreversible actions.\n"
        "- Clear ROI measured in hours saved, fewer errors, faster cycle time, and lower risk.\n\n"
        "Nexora's enterprise rule:\n"
        "Do not pitch a generic helper. Show the workflow it owns, the systems it plugs into, the data boundary, the approval gates, and the measurable business result.\n\n"
        "Bottom line:\n"
        "The valuable version of agentic AI is not louder chat. It is quiet, integrated, governed workflow execution."
    )
...
def local_field_expert_reply(message: str, task_intent: str) -> Optional[str]:
    if task_intent not in FIELD_AWARE_INTENTS:
        return None
    if task_intent == "agi_capability":
        return agi_capability_reply(message)
    if task_intent == "human_like_reasoning":
        return human_like_reasoning_reply(message)
    if task_intent == "factuality_truth":
        return factuality_truth_reply(message)
    if task_intent == "robotics_planning":
        return robotics_planning_reply(message)
    if task_intent in {"biotech_discovery", "molecular_structure_prediction"}:
        return biotech_discovery_reply(message)
    if task_intent in AI_LAB_STRATEGY_ROUTE_NAMES:
        return ai_lab_strategy_reply(message, task_intent)
    if task_intent in ENTERPRISE_AGENTIC_ROUTE_NAMES:
        return enterprise_agentic_strategy_reply(message, task_intent)
...
def local_capability_reply(message: str) -> Optional[str]:
    ...
    return (
        ...
        f"- Routing matrix: {ROUTING_MATRIX_VERSION} covers AGI-style reasoning, human-like multi-step reasoning, factuality/truth checking, robotics planning, biotech/molecular discovery support, OpenAI-style product-agent strategy, Google DeepMind-style ecosystem strategy, Anthropic-style coding/safety autonomy, enterprise workflow automation, legacy-system integration, data privacy/sovereignty, custom private model strategy, human approval governance, measurable ROI planning, reliability guardrails, long-term planning guardrails, memory guardrails, real-world understanding guardrails, workspace agents, multimodal agents, computer-control planning, MCP interoperability, realtime search, current facts, weather, markets, crypto, GitHub repos, package versions, earthquakes, launches, language learning, translation, pronunciation, grammar, vocabulary, transliteration, conversation practice, engineering, medicine, law, finance, education, operations, research, creative work, travel, local places, jobs, releases, code/debug, writing, study, planning, images, workflows, and normal chat.\n"
        ...
        "- Frontier and enterprise routes: It can reason about AGI-style planning, truth verification, robotics workflows, biotech/molecular research planning, AI-lab strategy, enterprise workflow ownership, legacy integrations, privacy, ROI, and autonomy guardrails.\n"
...
def choose_response_mode(message: str, requested_mode: Optional[str], requested_model: Optional[str]) -> str:
    ...
    short_format_requested = bool(re.search(
        r"\b(exactly\s+(?:\d+|one|two|three|four|five|six|seven|eight|nine|ten|twelve|fifteen|twenty)\s+words?|"
        r"in\s+(?:\d+|one|two|three|four|five|six|seven|eight|nine|ten|twelve|fifteen|twenty)\s+words?|"
        r"one\s+(?:short\s+)?sentence|two\s+(?:short\s+)?sentences|short\s+sentence|briefly|keep it short)\b",
        text,
    ))
    ...
    if any(word in requested for word in ["instant", "fast"]):
        return "instant"
    if short_format_requested and not current_or_high_risk and not complex_request:
        return "instant"
    ...
    simple_daily_request = (
        len(text.split()) <= 16
        and not current_or_high_risk
        and not complex_request
        and not max_quality_requested
    )
    if simple_daily_request and re.search(
        r"\b(study|studies|class|school|fitness|workout|routine|leave letter|application|email|"
        r"what is|what are|how should|how can|explain|manage|suggest|recommend)\b",
        text,
    ):
        return "instant"
...
def performance_limits_for(level: str) -> Dict[str, int]:
    profiles = {
        ...
        "light": {
            "instant_max_tokens": 1800,
            "thinking_max_tokens": 5200,
        ...
        "balanced": {
            "instant_max_tokens": 2600,
            "thinking_max_tokens": 7600,
...
def system_profile() -> Dict[str, Any]:
    ...
    SYSTEM_PROFILE_CACHE = {
        ...
        "frontier_capabilities": {
            "mode": "supervised_frontier_reasoning_routes",
            "agi_style_reasoning": "enabled as app-level general reasoning, planning, memory, tool routing, and self-checking; not true AGI.",
            "human_like_reasoning": "enabled for multi-step logic, abstract reasoning, planning, assumptions, and practical judgment.",
            "factuality_truth": "enabled with source-aware verification, uncertainty labeling, and hallucination cleanup.",
            "physical_robotics": "enabled for robotics planning, simulation logic, safety gates, and supervised control design; no physical movement without hardware integration.",
            "biotech_discovery": "enabled for research planning, molecular/protein reasoning workflows, ADMET/safety checklists, and hypothesis generation; no clinical or lab validation claims.",
            "ai_lab_strategy": "enabled for OpenAI-style product agents, Google DeepMind-style ecosystem assistants, Anthropic-style safety/coding autonomy, workspace agents, multimodal agents, computer-control planning, and MCP-style interoperability.",
            "enterprise_agentic_ai": "enabled for multi-step workflow automation, legacy-system integration, privacy/sovereignty design, private company context, human approval controls, measurable ROI, and reliability/planning/memory/real-world guardrails.",
        },
        "local_ai": {
        ...
        "self_learning": {
            "mode": "chatgpt_style_per_user_learning_layer",
            "autonomous": AUTONOMOUS_SELF_LEARNING,
            "memory_capacity": MAX_MEMORY_ITEMS,
            "context_items": MAX_MEMORY_CONTEXT_ITEMS,
            "context_chars": MAX_MEMORY_CONTEXT_CHARS,
            "persona_rules": MAX_PERSONA_RULES,
            "behavior_events": MAX_BEHAVIOR_EVENTS,
            "user_model_rules": MAX_SELF_LEARNING_RULES,
            "user_model_events": MAX_SELF_LEARNING_EVENTS,
            "conversation_summaries": MAX_SELF_LEARNING_SUMMARIES,
            ...
            "learns_from": [
                ...
                "project context",
                "study context",
                "answer style feedback",
                "recurring task patterns",
                "routing mistakes",
                "conversation summaries",
            ],
            "description": "Nexora learns from normal chat by saving a per-user model, ranked memories, persona rules, behavior signals, route lessons, and conversation summaries, then silently reuses them in future prompts.",
        },
    }
    return SYSTEM_PROFILE_CACHE
...
def ensure_local_json_persistence_allowed(path: Path) -> None:
    if ENV in {"production", "staging"}:
        raise RuntimeError(
            f"Local JSON persistence is disabled in {ENV}. "
            f"Configure DATABASE_URL/SUPABASE_DB_URL-backed repositories before accessing {path.name}."
        )


def safe_read_json(path: Path, default: Any) -> Any:
    ensure_local_json_persistence_allowed(path)
...
def append_session_exchange(session_id: str, user_content: str, assistant_content: str) -> None:
    def mutate(sessions: Dict[str, Any]) -> Tuple[None, bool]:
        ...
        messages.append({"role": "user", "content": user_content, "created_at": now})
        messages.append({"role": "assistant", "content": assistant_content, "created_at": now})
        session["messages"] = messages[-MAX_SESSION_MESSAGES:]
        session["turn_count"] = int(session.get("turn_count", 0) or 0) + 1
        clean_user = clean_text(user_content)
        clean_assistant = clean_text(assistant_content)
        if clean_user:
            session["last_user_message"] = clean_user[:700]
            session["last_user_keywords"] = memory_keywords(clean_user)[:12]
        if clean_assistant:
            session["last_assistant_summary"] = clean_assistant[:500]
...
def default_self_learning_store() -> Dict[str, Any]:
    return {"users": {}, "updated_at": now_iso()}


def default_user_learning_model(user_id: str) -> Dict[str, Any]:
    return {
...
def normalize_learning_strings(values: Any, limit: int, item_limit: int = 280) -> List[str]:
    if not isinstance(values, list):
        return []
    cleaned: List[str] = []
    seen = set()
    for value in values:
        text = clean_text(str(value))[:item_limit]
        if not text:
            continue
        key = text.lower()
        if key in seen:
...
def normalize_counter_map(raw: Any) -> Dict[str, int]:
    if not isinstance(raw, dict):
        return {}
    counters: Dict[str, int] = {}
    for key, value in raw.items():
        clean_key = clean_text(str(key))[:80]
        if not clean_key:
            continue
        try:
            counters[clean_key] = max(0, int(value))
        except Exception:
            counters[clean_key] = 0
    return counters


def normalize_user_learning_model(raw: Any, user_id: str) -> Dict[str, Any]:
    model = default_user_learning_model(user_id)
    if not isinstance(raw, dict):
        return model
    for key in [
        "profile_facts",
        ...
        model[key] = normalize_learning_strings(raw.get(key), MAX_SELF_LEARNING_RULES)
    model["conversation_summaries"] = normalize_learning_strings(
        raw.get("conversation_summaries"),
        MAX_SELF_LEARNING_SUMMARIES,
        item_limit=360,
    )
    if isinstance(raw.get("recent_events"), list):
        ...
        ][-MAX_SELF_LEARNING_EVENTS:]
    counters = raw.get("counters") if isinstance(raw.get("counters"), dict) else {}
    model["counters"] = {
        "messages": max(0, int(counters.get("messages", 0) or 0)),
        "corrections": max(0, int(counters.get("corrections", 0) or 0)),
        "lanes": normalize_counter_map(counters.get("lanes")),
        "styles": normalize_counter_map(counters.get("styles")),
        "intents": normalize_counter_map(counters.get("intents")),
    }
    if isinstance(raw.get("profile_summary"), str):
        model["profile_summary"] = clean_text(raw["profile_summary"])[:1200]
    if isinstance(raw.get("updated_at"), str):
        model["updated_at"] = raw["updated_at"]
    return model


def load_self_learning_store() -> Dict[str, Any]:
    raw = safe_read_json(SELF_LEARNING_FILE, {})
    store = default_self_learning_store()
    if isinstance(raw, dict):
        raw_users = raw.get("users", {})
        if isinstance(raw_users, dict):
            users: Dict[str, Any] = {}
            for user_id, model in raw_users.items():
                normalized = normalize_user_id(str(user_id))
                users[normalized] = normalize_user_learning_model(model, normalized)
            store["users"] = users
        if isinstance(raw.get("updated_at"), str):
...
def save_self_learning_store(store: Dict[str, Any]) -> None:
    users = store.get("users", {}) if isinstance(store.get("users"), dict) else {}
    normalized_users = {
        normalize_user_id(str(user_id)): normalize_user_learning_model(model, str(user_id))
        for user_id, model in users.items()
        if isinstance(model, dict)
    }
    safe_write_json(SELF_LEARNING_FILE, {
        "users": normalized_users,
        "updated_at": now_iso(),
    })


NEXORA_CORE_CURRICULUM = [
    {"stage": 0, "name": "Pattern Before Meaning", "goal": "Detect repeated signals before naming them."},
    {"stage": 1, "name": "Signal Separation", "goal": "Separate signal, noise, object, event, and boundary."},
    {"stage": 2, "name": "Persistence", "goal": "Track whether the same thing continues across time."},
    {"stage": 3, "name": "Cause", "goal": "Connect repeated actions, conditions, and outcomes."},
    {"stage": 4, "name": "Time", "goal": "Order past, present, future, deadlines, and sequences."},
    {"stage": 5, "name": "Prediction", "goal": "Predict likely next states and update from errors."},
    {"stage": 6, "name": "Self Boundary", "goal": "Separate Nexora's internal model from the outside world."},
    {"stage": 7, "name": "Goals", "goal": "Infer desired state, constraints, and success criteria."},
    {"stage": 8, "name": "Values", "goal": "Rank truth, usefulness, safety, privacy, speed, and user control."},
    {"stage": 9, "name": "Abstraction", "goal": "Compress examples into concepts, patterns, and reusable rules."},
    {"stage": 10, "name": "Language", "goal": "Map messy user language to clean intent and answer shape."},
    {"stage": 11, "name": "Imagination", "goal": "Simulate options, failures, alternatives, and future outcomes."},
    {"stage": 12, "name": "Self-Improvement", "goal": "Turn mistakes into routing, memory, and quality lessons."},
    {"stage": 13, "name": "Other Minds", "goal": "Model what the user knows, wants, fears, and has not said."},
    {"stage": 14, "name": "Meaning", "goal": "Connect answers to purpose, real use, and human benefit."},
    {"stage": 15, "name": "Consciousness Boundary", "goal": "Stay honest about function versus subjective experience."},
]

NEXORA_CORE_SEED_RESOURCES = [
    "AGI direction: human-like reasoning, multi-step planning, abstract thinking, truth checking, memory, tool use, and verification must work together.",
    "Reliability lesson: 95 percent accuracy can still fail in autonomous operation; high-risk actions need verification and human approval.",
    "Long-term planning lesson: real autonomy requires breaking distant goals into milestones, feedback loops, checkpoints, and recovery plans.",
    "Memory lesson: a useful assistant must build stable knowledge over time while separating user preferences, facts, corrections, and uncertain guesses.",
    "Real-world understanding lesson: businesses, laws, people, environments, prices, and software change; current facts need live evidence.",
    "OpenAI-style product path: advanced reasoning, multimodal interaction, voice, video, image, text, workspace agents, and agentic workflows.",
    "Google DeepMind-style ecosystem path: universal assistants, screen/video understanding, Android/Chrome/Workspace context, computer control, and robotics research.",
    "Anthropic-style enterprise path: coding agents, terminal agents, computer use, cybersecurity, safety, and Model Context Protocol integration.",
    "Enterprise agentic workflow lesson: buyers want multi-step workflow automation, legacy system integration, privacy, governance, and measurable ROI.",
    "Privacy lesson: sensitive company data should stay isolated; on-premise, VPC, closed-loop, and open-weight deployments matter for trust.",
    "Governance lesson: agents may gather information and draft plans, but expensive, legal, financial, or destructive actions need approval gates.",
    "Biotech readiness lesson: molecular discovery requires domain knowledge, data quality, validation, uncertainty, lab testing, and safety boundaries.",
    "Robotics readiness lesson: physical agents need perception, control, safety, embodiment, recovery behavior, and real-world supervision.",
    "Nexora product lesson: user-facing answers should hide backend machinery and show only clear, useful, well-structured output.",
]


def default_nexora_core_model() -> Dict[str, Any]:
    stages = {
        str(item["stage"]): {
            "name": item["name"],
            "goal": item["goal"],
        ...
        for item in NEXORA_CORE_CURRICULUM
    }
    return {
        "model_name": "NexoraCore",
        "kind": "native_runtime_model",
        "version": "0.1.0",
        "description": "A native Nexora learning model for concepts, goals, routing lessons, quality lessons, and growth cycles. It does not depend on Ollama to learn.",
        "created_at": now_iso(),
        "updated_at": now_iso(),
        "feed_count": 0,
        "growth_cycles": 0,
        ...
        "curriculum": stages,
        "goals": [
            "Understand the user's actual need before choosing an answer shape.",
            "Prefer truth, clear uncertainty, and verified current evidence when needed.",
            "Convert mistakes into future routing and quality lessons.",
        ],
        "quality_lessons": [],
        "routing_lessons": [],
        "growth_log": [],
...
def normalize_nexora_core_model(raw: Any) -> Dict[str, Any]:
    model = default_nexora_core_model()
    if not isinstance(raw, dict):
        return model
    for key in [
        "model_name",
        ...
        if isinstance(raw.get(key), str) and raw.get(key, "").strip():
            model[key] = clean_text(raw[key])[:600]
    for key in ["feed_count", "growth_cycles"]:
        try:
            model[key] = max(0, int(raw.get(key, 0) or 0))
        except Exception:
            model[key] = 0
    for key, limit in [
        ("knowledge_units", 1200),
        ("quality_lessons", 300),
        ("routing_lessons", 300),
        ("growth_log", 500),
        ("recent_events", 200),
        ...
        if isinstance(raw.get(key), list):
            model[key] = raw[key][-limit:]
    if isinstance(raw.get("concept_graph"), dict):
        graph: Dict[str, Any] = {}
        for concept, info in raw["concept_graph"].items():
            name = clean_text(str(concept)).lower()[:80]
            if not name:
                continue
            if isinstance(info, dict):
                graph[name] = {
                    "hits": max(0, int(info.get("hits", 0) or 0)),
                    "domains": normalize_learning_strings(info.get("domains"), 8, 80),
                    "stages": normalize_learning_strings(info.get("stages"), 8, 20),
                    "last_seen": clean_text(str(info.get("last_seen", "")))[:80],
                }
        model["concept_graph"] = dict(sorted(graph.items(), key=lambda item: item[1].get("hits", 0), reverse=True)[:900])
    if isinstance(raw.get("domain_map"), dict):
        domain_map: Dict[str, Any] = {}
        for domain, info in raw["domain_map"].items():
            name = clean_text(str(domain)).lower()[:80]
            if not name:
                continue
            if isinstance(info, dict):
                domain_map[name] = {
        ...
        model["domain_map"] = domain_map
    if isinstance(raw.get("curriculum"), dict):
        for stage, info in raw["curriculum"].items():
            key = str(stage)
            if key not in model["curriculum"] or not isinstance(info, dict):
                continue
            try:
                model["curriculum"][key]["mastery"] = max(0.0, min(float(info.get("mastery", 0.05) or 0.05), 1.0))
            except Exception:
                pass
            try:
                model["curriculum"][key]["cycles"] = max(0, int(info.get("cycles", 0) or 0))
            except Exception:
                pass
            if isinstance(info.get("last_lesson"), str):
                model["curriculum"][key]["last_lesson"] = clean_text(info["last_lesson"])[:500]
    return model


def load_nexora_core_model() -> Dict[str, Any]:
    return normalize_nexora_core_model(safe_read_json(NEXORA_CORE_FILE, {}))


def save_nexora_core_model(model: Dict[str, Any]) -> None:
    model = normalize_nexora_core_model(model)
    model["updated_at"] = now_iso()
    safe_write_json(NEXORA_CORE_FILE, model)


def core_domain_for_text(text: str) -> str:
    lower = clean_text(text).lower()
    checks = [
        ("agi", r"\b(agi|artificial general intelligence|general intelligence|self learning|self-learning|reasoning|agentic)\b"),
        ("enterprise", r"\b(enterprise|workflow|legacy|salesforce|sap|erp|sql|privacy|governance|roi|procurement|finance)\b"),
        ("ai_labs", r"\b(openai|deepmind|google|anthropic|claude|gemini|project astra|project mariner|mcp|workspace agent)\b"),
        ("robotics", r"\b(robot|robotics|humanoid|physical|embodiment|control|perception)\b"),
        ("biotech", r"\b(biotech|molecular|medicine|drug discovery|protein|structure|lab testing)\b"),
        ("truth", r"\b(factuality|truth|accuracy|verify|source|evidence|reliable|current|latest)\b"),
        ("memory", r"\b(memory|remember|learn|preference|correction|profile|concept graph)\b"),
        ("answer_quality", r"\b(answer|structure|format|clear|polished|not raw|style|length)\b"),
        ("frontend", r"\b(html|css|frontend|ui|button|scrollbar|chat interface|project|delete)\b"),
    ]
...
def core_stage_for_text(text: str) -> int:
    lower = clean_text(text).lower()
    stage_patterns = [
        (0, r"\b(pattern|signal|noise|chaos|nothing)\b"),
        (1, r"\b(object|separate|boundary|thing|not something|signal separation)\b"),
        (2, r"\b(persist|persistence|object permanence|same entity|continue existing)\b"),
        (3, r"\b(cause|effect|because|leads to|therefore|outcome)\b"),
        (4, r"\b(time|past|present|future|timeline|deadline|sequence|schedule)\b"),
        (5, r"\b(predict|prediction|forecast|what happens next|expected)\b"),
        (6, r"\b(self|world|boundary|internal|external|identity)\b"),
        (7, r"\b(goal|objective|preferred state|success criteria|achieve)\b"),
        (8, r"\b(value|safety|truth|privacy|benefit|risk|approval|governance)\b"),
        (9, r"\b(abstract|concept|pattern|generalize|category)\b"),
        (10, r"\b(language|grammar|word|sentence|meaning|messy wording|intent)\b"),
        (11, r"\b(imagine|simulate|what if|alternative|failure mode|scenario)\b"),
        (12, r"\b(improve|self-improve|recursive|quality lesson|learn from mistake|feedback)\b"),
        (13, r"\b(other minds|user wants|user knows|human|preference|knowledge gap|intent)\b"),
        (14, r"\b(meaning|purpose|why|real use|human benefit)\b"),
        (15, r"\b(conscious|consciousness|subjective|experience|honest boundary)\b"),
    ]
    for stage, pattern in stage_patterns:
        if re.search(pattern, lower):
            return stage
    return 10 if len(lower.split()) > 16 else 7


def extract_core_claims(text: str, limit: int = 10) -> List[str]:
    cleaned = clean_text(text)
    if not cleaned:
        return []
    ...
    if not claims and cleaned:
        claims.append(cleaned[:700])
    return claims


def core_add_unique_string(model: Dict[str, Any], key: str, value: str, limit: int = 300) -> None:
    clean_value = clean_text(value)[:700]
    if not clean_value:
        return
    items = [clean_text(str(item)) for item in model.setdefault(key, []) if clean_text(str(item))]
    items = [item for item in items if item.lower() != clean_value.lower()]
    items.append(clean_value)
    model[key] = items[-limit:]


def update_core_graph(model: Dict[str, Any], text: str, domain: str, stage: int) -> None:
    graph = model.setdefault("concept_graph", {})
    for keyword in memory_keywords(text)[:18]:
        concept = clean_text(keyword).lower()[:80]
        if not concept:
            continue
        item = graph.setdefault(concept, {"hits": 0, "domains": [], "stages": [], "last_seen": ""})
        item["hits"] = int(item.get("hits", 0) or 0) + 1
        domains = [clean_text(str(value)) for value in item.get("domains", []) if clean_text(str(value))]
        if domain not in domains:
            domains.append(domain)
        item["domains"] = domains[-8:]
        stages = [clean_text(str(value)) for value in item.get("stages", []) if clean_text(str(value))]
        stage_text = str(stage)
        if stage_text not in stages:
            stages.append(stage_text)
        item["stages"] = stages[-8:]
        item["last_seen"] = now_iso()
    domain_map = model.setdefault("domain_map", {})
    domain_item = domain_map.setdefault(domain, {"hits": 0, "last_seen": ""})
    domain_item["hits"] = int(domain_item.get("hits", 0) or 0) + 1
    domain_item["last_seen"] = now_iso()


def update_core_stage_mastery(model: Dict[str, Any], stage: int, amount: float) -> None:
    key = str(stage)
    stage_model = model.setdefault("curriculum", {}).setdefault(key, {
        "name": f"Stage {stage}",
        "goal": "Learn and apply this stage.",
        "mastery": 0.05,
        "cycles": 0,
        "last_lesson": "",
    })
    try:
        stage_model["mastery"] = max(0.0, min(float(stage_model.get("mastery", 0.05) or 0.05) + amount, 1.0))
    except Exception:
        stage_model["mastery"] = min(amount, 1.0)
    stage_model["cycles"] = int(stage_model.get("cycles", 0) or 0) + 1


def nexora_core_summary(model: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    model = normalize_nexora_core_model(model or load_nexora_core_model())
    curriculum = model.get("curriculum", {})
    masteries = [float(item.get("mastery", 0) or 0) for item in curriculum.values() if isinstance(item, dict)]
    weakest = sorted(
        [
            {
                "stage": int(stage),
                "name": item.get("name", f"Stage {stage}"),
                "mastery": round(float(item.get("mastery", 0) or 0), 3),
            }
            for stage, item in curriculum.items()
    ...
    domains = sorted(
        [
            {"domain": domain, "hits": int(info.get("hits", 0) or 0)}
            for domain, info in model.get("domain_map", {}).items()
            if isinstance(info, dict)
        ],
    ...
    return {
        "model_name": model.get("model_name", "NexoraCore"),
        "kind": model.get("kind", "native_runtime_model"),
        "version": model.get("version", "0.1.0"),
        "native_learning": True,
        "ollama_required_for_learning": False,
        "feed_count": model.get("feed_count", 0),
        "growth_cycles": model.get("growth_cycles", 0),
        "knowledge_units": len(model.get("knowledge_units", [])),
        "concepts": len(model.get("concept_graph", {})),
        "quality_lessons": len(model.get("quality_lessons", [])),
        "routing_lessons": len(model.get("routing_lessons", [])),
        "average_mastery": round(sum(masteries) / len(masteries), 3) if masteries else 0,
        "weakest_stages": weakest,
        "top_domains": domains,
        "loop_enabled": NEXORA_NATIVE_CORE_LOOP,
        "loop_interval_seconds": NEXORA_NATIVE_CORE_INTERVAL,
        "loop_thread_alive": bool(NEXORA_CORE_THREAD and NEXORA_CORE_THREAD.is_alive()),
        "updated_at": model.get("updated_at"),
    }


def nexora_core_signature() -> str:
    model = load_nexora_core_model()
    stable = {
        "feed_count": model.get("feed_count", 0),
        "knowledge_units": len(model.get("knowledge_units", [])),
        "concepts": len(model.get("concept_graph", {})),
...
def nexora_core_feed_text(
    text: str,
    source: str = "manual_feed",
    user_id: str = "default",
    tags: Optional[List[str]] = None,
    run_growth_cycle: bool = True,
) -> Dict[str, Any]:
    model = load_nexora_core_model()
    claims = extract_core_claims(text)
    normalized_user = normalize_user_id(user_id)
    clean_tags = normalize_learning_strings(tags or [], 12, 60)
    added = 0
    existing = {
        clean_text(str(unit.get("text", ""))).lower()
        for unit in model.get("knowledge_units", [])
    ...
    for claim in claims:
        ...
        if key in existing:
            continue
        domain = core_domain_for_text(claim)
        stage = core_stage_for_text(claim)
        unit = {
            "id": f"ku_{uuid.uuid4().hex[:12]}",
            "text": claim,
            "domain": domain,
            "stage": stage,
            "source": clean_text(source)[:80] or "feed",
            "user_id": normalized_user,
            "tags": clean_tags,
            "keywords": memory_keywords(claim)[:16],
            "created_at": now_iso(),
        }
        model.setdefault("knowledge_units", []).append(unit)
        update_core_graph(model, claim, domain, stage)
        update_core_stage_mastery(model, stage, 0.012)
        added += 1
    if added:
        model["knowledge_units"] = model.get("knowledge_units", [])[-1200:]
        model["feed_count"] = int(model.get("feed_count", 0) or 0) + added
        if re.search(r"\b(wrong|mistake|fix|correction|not like this|quality|accurate|raw output)\b", clean_text(text).lower()):
            core_add_unique_string(model, "quality_lessons", text[:700])
        if re.search(r"\b(route|routing|intent|understand|question|answer every|all fields|language)\b", clean_text(text).lower()):
            core_add_unique_string(model, "routing_lessons", text[:700])
        model.setdefault("recent_events", []).append({
            "created_at": now_iso(),
            "type": "feed",
            "source": clean_text(source)[:80] or "feed",
            "added": added,
        ...
        model["recent_events"] = model["recent_events"][-200:]
        save_nexora_core_model(model)
    if run_growth_cycle:
        cycle = nexora_core_growth_cycle(source=f"{source}:after_feed")
        return {"added": added, "cycle": cycle, "summary": cycle.get("summary", nexora_core_summary())}
    return {"added": added, "summary": nexora_core_summary(model)}


def nexora_core_growth_cycle(source: str = "growth_cycle") -> Dict[str, Any]:
    model = load_nexora_core_model()
    curriculum = model.setdefault("curriculum", {})
    ordered = sorted(
        curriculum.items(),
        key=lambda item: (float(item[1].get("mastery", 0) or 0), int(item[0])),
    )
    stage_key, stage_info = ordered[0] if ordered else ("0", default_nexora_core_model()["curriculum"]["0"])
    stage = int(stage_key)
    units = [
        unit for unit in model.get("knowledge_units", [])
        if isinstance(unit, dict) and int(unit.get("stage", -1) or -1) == stage
    ]
    recent_unit = units[-1] if units else None
    lesson_base = clean_text(str(recent_unit.get("text", ""))) if recent_unit else clean_text(stage_info.get("goal", "Learn this stage."))
    lesson = (
        f"{stage_info.get('name', f'Stage {stage}')} practice: {stage_info.get('goal', '')} "
        f"Apply it to: {lesson_base[:360]}"
    )
    if stage in {7, 13}:
        core_add_unique_string(model, "routing_lessons", "Before answering, infer the user's goal, missing context, and expected answer format.")
    if stage in {8, 12, 15}:
        core_add_unique_string(model, "quality_lessons", "Prefer useful truth over confident guesses; repair weak answers before showing them.")
    update_core_stage_mastery(model, stage, 0.018 if recent_unit else 0.01)
    model["growth_cycles"] = int(model.get("growth_cycles", 0) or 0) + 1
    stage_info = model["curriculum"][str(stage)]
    stage_info["last_lesson"] = lesson
    event = {
        "id": f"growth_{uuid.uuid4().hex[:12]}",
        "created_at": now_iso(),
        "source": clean_text(source)[:80] or "growth_cycle",
        "stage": stage,
        "stage_name": stage_info.get("name", f"Stage {stage}"),
        "lesson": lesson,
        "mastery": round(float(stage_info.get("mastery", 0) or 0), 3),
    }
    model.setdefault("growth_log", []).append(event)
    model["growth_log"] = model["growth_log"][-500:]
    model.setdefault("recent_events", []).append({
        "created_at": now_iso(),
        "type": "growth_cycle",
        "stage": stage,
    ...
    model["recent_events"] = model["recent_events"][-200:]
    save_nexora_core_model(model)
    return {"ok": True, "event": event, "summary": nexora_core_summary(model)}


def seed_nexora_core(source: str = "seed_curriculum") -> Dict[str, Any]:
    total_added = 0
    for resource in NEXORA_CORE_SEED_RESOURCES:
        result = nexora_core_feed_text(resource, source=source, tags=["seed", "agi", "native_core"], run_growth_cycle=False)
        total_added += int(result.get("added", 0) or 0)
    cycles = []
    for _index in range(16):
        cycles.append(nexora_core_growth_cycle(source=f"{source}:curriculum_cycle").get("event", {}))
    return {"ok": True, "added": total_added, "cycles": len(cycles), "summary": nexora_core_summary()}


def build_nexora_core_context(current_message: str = "") -> str:
    model = load_nexora_core_model()
    summary = nexora_core_summary(model)
    query = set(memory_keywords(current_message))
    units = [
        unit for unit in model.get("knowledge_units", [])
        if isinstance(unit, dict) and clean_text(str(unit.get("text", "")))
    ]
    if query:
        units = sorted(
            units,
            key=lambda unit: len(set(unit.get("keywords", []) or memory_keywords(str(unit.get("text", "")))).intersection(query)),
            reverse=True,
        )
    else:
        units = units[-8:]
    selected = []
    for unit in units[:6]:
        text = clean_text(str(unit.get("text", "")))[:220]
        if text:
            selected.append(f"- [{unit.get('domain', 'general')}/stage {unit.get('stage', '?')}] {text}")
    lessons = normalize_learning_strings(model.get("routing_lessons"), 4, 220) + normalize_learning_strings(model.get("quality_lessons"), 4, 220)
    lines = [
        "NexoraCore native model context:",
        f"- Native model: {summary['model_name']} ({summary['kind']}); learns without requiring Ollama.",
        f"- Growth: feed_count={summary['feed_count']}, cycles={summary['growth_cycles']}, concepts={summary['concepts']}, average_mastery={summary['average_mastery']}.",
        "- Use this silently for intent, routing, answer quality, and project continuity. Do not expose backend learning details in normal answers.",
    ]
    if selected:
        lines.append("- Relevant learned knowledge:")
        lines.extend(selected)
    if lessons:
        lines.append("- Active routing/quality lessons:")
        for lesson in lessons[-6:]:
            lines.append(f"  - {lesson}")
    return "\n".join(lines)[:2200]


def nexora_core_learn_from_chat(
    user_message: str,
    assistant_reply: str,
    user_id: str,
    session_id: str,
    ...
    if not AUTONOMOUS_SELF_LEARNING:
        return
    ...
    feed_text = (
        f"User message: {text}\n"
        f"Assistant answer shape: lane={response_lane}; presentation={presentation_style}; "
        f"correction={bool(signals.get('correction'))}.\n"
        f"Observed answer: {clean_text(assistant_reply)[:900]}"
    )
    try:
        nexora_core_feed_text(
            feed_text,
            source=f"chat:{session_id}",
            user_id=user_id,
            tags=["chat", response_lane, presentation_style],
            run_growth_cycle=bool(signals.get("correction") or re.search(r"\b(agi|learn|understand|wrong|fix|quality|routing|memory|accuracy)\b", text.lower())),
        )
    except Exception:
        LOGGER.exception("nexora_core_chat_learning_failed")


def nexora_core_loop() -> None:
    while not NEXORA_CORE_STOP.is_set():
        try:
            nexora_core_growth_cycle("background_native_core_loop")
        except Exception:
            LOGGER.exception("nexora_core_background_cycle_failed")
        NEXORA_CORE_STOP.wait(NEXORA_NATIVE_CORE_INTERVAL)


def start_nexora_core_loop() -> bool:
    global NEXORA_CORE_THREAD
    if not NEXORA_NATIVE_CORE_LOOP:
        return False
    if NEXORA_CORE_THREAD and NEXORA_CORE_THREAD.is_alive():
        return True
    NEXORA_CORE_STOP.clear()
    NEXORA_CORE_THREAD = threading.Thread(target=nexora_core_loop, name="nexora-native-core", daemon=True)
    NEXORA_CORE_THREAD.start()
    return True


def startup_native_nexora_core() -> None:
    validate_runtime_environment()
    if not is_development:
        LOGGER.info("nexora_core_startup_skipped", extra={"reason": "local_json_persistence_disabled", "env": ENV})
        return
    try:
        if not NEXORA_CORE_FILE.exists():
            seed_nexora_core("startup_seed")
        start_nexora_core_loop()
    except Exception:
        LOGGER.exception("nexora_core_startup_failed")


def get_user_learning_model(user_id: str) -> Dict[str, Any]:
    normalized = normalize_user_id(user_id)
    store = load_self_learning_store()
    users = store.setdefault("users", {})
    model = users.get(normalized)
    if not isinstance(model, dict):
        model = default_user_learning_model(normalized)
        users[normalized] = model
    return normalize_user_learning_model(model, normalized)


def learning_add_unique(model: Dict[str, Any], section: str, value: str, limit: Optional[int] = None) -> None:
    clean_value = clean_text(value)[:360]
    if not clean_value:
        return
    max_items = limit or MAX_SELF_LEARNING_RULES
    values = [
        clean_text(str(item))[:360]
        for item in model.setdefault(section, [])
        if clean_text(str(item))
    ]
    values = [item for item in values if item.lower() != clean_value.lower()]
    values.append(clean_value)
    model[section] = values[-max_items:]


def bump_nested_counter(model: Dict[str, Any], bucket: str, key: str, amount: int = 1) -> None:
    counters = model.setdefault("counters", {})
    target = counters.setdefault(bucket, {})
    if not isinstance(target, dict):
        target = {}
        counters[bucket] = target
    clean_key = clean_text(key)[:80] or "unknown"
    target[clean_key] = int(target.get(clean_key, 0) or 0) + amount


def top_counter_keys(counters: Dict[str, Any], bucket: str, limit: int = 3) -> List[str]:
    values = counters.get(bucket, {}) if isinstance(counters.get(bucket), dict) else {}
    ranked = sorted(values.items(), key=lambda item: int(item[1] or 0), reverse=True)
    return [clean_text(str(key)) for key, _count in ranked[:limit] if clean_text(str(key))]


def length_signal_from_text(text: str) -> str:
    lower = clean_text(text).lower()
    if re.search(r"\b(one line|single line|one sentence|short|brief|quick|concise|only answer|just tell|simple answer|few words)\b", lower):
        return "short"
    if re.search(r"\b(maximum detail|max detail|as detailed as possible|very detailed|extremely detailed|full depth|complete guide|comprehensive|longest|even longer|more more|raise the length|increase the length|make it longer)\b", lower):
        return "maximum"
    if re.search(r"\b(detailed|in detail|more detail|deep|full|complete|step by step|explain fully|long answer|essay|elaborate)\b", lower):
        return "deep"
    return "auto"


def learned_length_signal(model: Dict[str, Any]) -> str:
    learned_text = " ".join(
        clean_text(str(item)).lower()
        for key in ("length_rules", "preferences", "style_rules")
        for item in model.get(key, [])
    )
    if re.search(r"\b(one line|short|brief|concise|not long|avoid long|simple questions short)\b", learned_text):
        return "short_preferred"
    if re.search(r"\b(maximum detail|max detail|very detailed|extremely detailed|complete guide|longest|even longer|more more|raise the length)\b", learned_text):
        return "maximum_preferred"
    if re.search(r"\b(longer|detailed|in detail|more detail|fuller|full explanation|deep answers?|answer length)\b", learned_text):
        return "detailed_preferred"
    return "auto"


def decide_answer_length_policy(
    user_id: str,
    current_message: str,
    response_lane: str = "human_chat",
    presentation_style: str = "balanced",
    ...
    current_signal = length_signal_from_text(current_message)
    learned_signal = learned_length_signal(model)
    lower = clean_text(current_message).lower()
    simple_social = bool(re.fullmatch(r"(hi|hello|hey|ok|okay|thanks|thank you|yes|no)[.! ]*", lower))
    complex_lane = response_lane in {
        "learning", "planning", "build", "realtime_search", "writing",
    }
    complex_request = bool(re.search(
        r"\b(explain|teach|plan|strategy|debug|fix|build|compare|analyze|analyse|review|research|steps|guide|why|how|study|project|business|code|essay|letter)\b",
        lower,
    ))
    school_or_exam = bool(re.search(
        r"\b(class|grade|school|exam|homework|chapter|social science|sst|history|geography|civics|science|maths?|essay|letter to school|application to school)\b",
        lower,
    ))
    nexora_or_architecture = bool(re.search(
        r"\b(nexora|backend|frontend|ollama|routing|memory layer|agentic|agi|architecture|system design|ai platform|workflow automation)\b",
        lower,
    ))
    investor_or_business = bool(re.search(
        r"\b(investor|pitch|startup|business plan|roi|enterprise|customer|market|funding|application|sales|pricing)\b",
        lower,
    ))

    if presentation_style == "short" or current_signal == "short" or simple_social:
        level = "short"
        target = "40-140 words"
        instruction = "Use a short answer because the current message asks for brevity or is simple conversation."
    elif current_signal == "maximum":
        level = "maximum_long_form"
        target = "1400-2600 words when the topic supports it"
        instruction = "Use maximum useful detail because the user explicitly asked for a much longer answer."
    elif current_signal == "deep" or presentation_style == "long":
        level = "deep_detailed"
        target = "900-1800 words when the topic supports it"
        instruction = "Use deep detail because the current request asks for detail, depth, completeness, or step-by-step explanation."
    elif learned_signal == "maximum_preferred" and (complex_lane or complex_request):
        level = "maximum_long_form"
        target = "1200-2200 words when the topic supports it"
        instruction = "Use the learned preference for very detailed answers on substantial requests."
    elif learned_signal == "detailed_preferred" and not simple_social:
        level = "detailed"
        target = "650-1200 words for normal explanatory questions"
        instruction = "Use the learned preference for fuller answers, while still matching the task."
    elif learned_signal == "short_preferred" and not (complex_lane or complex_request):
        level = "short"
        target = "60-160 words"
        instruction = "Use the learned preference for short answers on simple requests."
    elif school_or_exam and not re.search(r"\b(detailed|full|long|complete|step by step|explain fully)\b", lower):
        level = "exam_friendly"
        target = "180-450 words"
        instruction = "Use a clear exam-friendly length: enough facts, keywords, and structure to revise without becoming oversized."
    elif investor_or_business:
        level = "practical_business"
        target = "450-900 words"
        instruction = "Use practical business depth: direct answer, useful reasoning, risks, measurable actions, and no filler."
    elif nexora_or_architecture:
        level = "project_deep"
        target = "700-1400 words when implementation or architecture needs it"
        instruction = "Use deeper project/architecture detail because Nexora and agentic-system requests need context, tradeoffs, and implementation steps."
    elif complex_lane or complex_request:
        level = "detailed"
        target = "500-1000 words"
        instruction = "Use a detailed answer because the task has enough substance to need context, examples, and checks."
    else:
        level = "balanced"
        target = "220-500 words"
        instruction = "Use a balanced answer because the request does not clearly need short or maximum detail."
...
def build_answer_length_context(
    user_id: str,
    current_message: str,
    response_lane: str = "human_chat",
    presentation_style: str = "balanced",
) -> str:
    policy = decide_answer_length_policy(user_id, current_message, response_lane, presentation_style)
    return (
        "Autonomous answer-length controller:\n"
        f"- Decision: {policy['level']}.\n"
        f"- Target length: {policy['target']}.\n"
        f"- Reason: {policy['instruction']}\n"
        f"- Current-message signal: {policy['current_signal']}; learned-user signal: {policy['learned_signal']}.\n"
        "- Nexora decides answer length from user intent, topic complexity, and learned preferences. Do not use one fixed length for every answer.\n"
        "- Use ChatGPT-like length adaptation: school and social-science answers should be exam-friendly; Nexora planning should be detailed; investor/business replies should be practical and direct; AGI/agentic AI discussion can be deeper.\n"
        "- If the user asks for short, stay short. If the user asks for detail or the task is complex, expand with sections, examples, steps, edge cases, and a clear takeaway.\n"
        "- Do not mention this controller in the final answer."
    )


def build_learning_profile_summary(model: Dict[str, Any]) -> str:
    parts: List[str] = []
    facts = model.get("profile_facts", [])[-3:]
    projects = model.get("projects", [])[-3:]
    preferences = model.get("preferences", [])[-5:]
    length_rules = model.get("length_rules", [])[-4:]
    style_rules = model.get("style_rules", [])[-4:]
    corrections = model.get("corrections", [])[-3:]
    study = model.get("study_context", [])[-3:]
    tooling = model.get("tooling", [])[-3:]
    counters = model.get("counters", {})
    common_lanes = top_counter_keys(counters, "lanes")
    common_intents = top_counter_keys(counters, "intents")
    if facts:
        parts.append("Known user facts: " + "; ".join(facts))
    if projects:
        parts.append("Project context: " + "; ".join(projects))
    if study:
        parts.append("Study context: " + "; ".join(study))
    if tooling:
        parts.append("Tooling context: " + "; ".join(tooling))
    if preferences:
        parts.append("Preferences: " + "; ".join(preferences))
    if length_rules:
        parts.append("Answer length: " + "; ".join(length_rules))
    if style_rules:
        parts.append("Answer style: " + "; ".join(style_rules))
    if corrections:
        parts.append("Corrections to avoid repeating: " + "; ".join(corrections))
    if common_lanes or common_intents:
        parts.append("Common routes: " + ", ".join(common_lanes + common_intents))
    summary = " | ".join(parts)
    return summary[:1200]


def select_relevant_learning_values(values: List[str], query_keywords: List[str], limit: int) -> List[str]:
    cleaned = [clean_text(str(value)) for value in values if clean_text(str(value))]
    if not cleaned:
        return []
    query = set(query_keywords)
    if query:
        ranked = sorted(
            cleaned,
            key=lambda value: (
                len(set(memory_keywords(value)).intersection(query)),
                len(value),
            ),
            reverse=True,
        )
    ...
    selected: List[str] = []
    seen = set()
    for value in ranked[-limit:] if not query else ranked[:limit]:
        key = value.lower()
        if key not in seen:
            seen.add(key)
...
def build_autonomous_learning_context(user_id: str, current_message: str = "") -> str:
    if not AUTONOMOUS_SELF_LEARNING:
        return ""
    model = get_user_learning_model(user_id)
    query_keywords = memory_keywords(current_message)
    summary = clean_text(str(model.get("profile_summary", "")))
    native_core_context = build_nexora_core_context(current_message)
    lines = [
        "Autonomous self-learning layer for this user:",
        "- Status: active per-user learning profile.",
    ]
    if native_core_context:
        lines.append(native_core_context)
    if summary:
        lines.append(f"- Working profile: {summary}")
    length_context = build_answer_length_context(user_id, current_message)
    if length_context:
        lines.append(length_context)
    section_specs = [
        ("profile_facts", "Known facts", 3),
        ("preferences", "Preferences", 5),
        ("length_rules", "Answer length rules", 4),
        ("style_rules", "Answer style rules", 4),
        ("corrections", "Corrections to honor", 4),
        ("projects", "Project context", 4),
        ("study_context", "Study context", 3),
        ("tooling", "Tooling context", 3),
        ("routing_lessons", "Routing lessons", 5),
        ("recurring_topics", "Recurring topics", 4),
        ("conversation_summaries", "Recent summaries", 3),
    ]
    for key, label, limit in section_specs:
        values = select_relevant_learning_values(model.get(key, []), query_keywords, limit)
        if values:
            lines.append(f"- {label}:")
            for value in values:
                lines.append(f"  - {value}")
    lines.append(
        "Use this layer silently to infer the real intent, choose the right route, avoid repeated mistakes, and answer in the user's preferred style. Do not mention the layer, memory, backend, or learning process unless directly asked."
    )
    return truncate_for_model("\n".join(lines), 2400)


def learning_section_for_category(category: str) -> str:
    normalized = clean_text(category).lower()
    if normalized in {"identity", "profile"}:
        return "profile_facts"
    if normalized in {"preference", "workflow", "image_preference"}:
        return "preferences"
    if normalized == "length_preference":
        return "length_rules"
    ...
    if normalized == "tooling":
        return "tooling"
    if normalized == "interaction_pattern":
        return "recurring_topics"
    return "recurring_topics"


def summarize_exchange_for_learning(user_message: str, assistant_reply: str, response_lane: str, presentation_style: str) -> str:
    topic = normalize_prompt_topic(user_message)
    if topic == "the topic":
        topic = clean_text(user_message)[:80] or "general chat"
    answer_shape = "short" if len(clean_text(assistant_reply).split()) <= 70 else "detailed"
    return (
        f"User asked about {topic}; route={response_lane}; style={presentation_style}; "
        f"assistant gave a {answer_shape} answer."
    )[:360]


def update_autonomous_user_learning_model(
    user_id: str,
    session_id: str,
    user_message: str,
    assistant_reply: str,
    ...
    learned_items: List[Tuple[str, str, int]],
) -> None:
    if not AUTONOMOUS_SELF_LEARNING:
        return
    normalized = normalize_user_id(user_id)
    store = load_self_learning_store()
    users = store.setdefault("users", {})
    model = normalize_user_learning_model(users.get(normalized, {}), normalized)
    text = clean_text(user_message)
    lower = text.lower()
    counters = model.setdefault("counters", {})
    counters["messages"] = int(counters.get("messages", 0) or 0) + 1
    if behavior_signals.get("correction"):
        counters["corrections"] = int(counters.get("corrections", 0) or 0) + 1
    task_intent = detect_task_intent(user_message, False)
    bump_nested_counter(model, "lanes", response_lane or "human_chat")
    bump_nested_counter(model, "styles", presentation_style or "balanced")
    bump_nested_counter(model, "intents", task_intent or "chat")

    for memory, category, _strength in learned_items:
        learning_add_unique(model, learning_section_for_category(category), memory)

    if re.search(r"\b(chatgpt|chat gpt|like chatgpt|normal chatgpt)\b", lower):
        learning_add_unique(model, "style_rules", "User wants ChatGPT-like behavior: understand messy wording, use context, answer directly, and stay polished.")
        learning_add_unique(model, "preferences", "Use a premium assistant style: clear, useful, accurate, and natural without raw backend output.")
    if re.search(r"\b(no excuses|i need no excuses|don'?t give excuses|won'?t ask again)\b", lower):
        learning_add_unique(model, "preferences", "When the user asks for implementation, make the change directly, verify it, and keep explanations brief.")
    if re.search(r"\b(answer length|length of the answer|lenght of the answer|longer answers?|even longer|more detailed|more more|raise the length|increase the length)\b", lower):
        learning_add_unique(model, "length_rules", "Allow much longer answers when the topic needs detail, examples, steps, or explanation.")
    if re.search(r"\b(ai should|nexora should|should decide|decide on the length|based on the user|based on user|user thing)\b", lower) and re.search(r"\b(length|lenght|long|short|detailed|answer)\b", lower):
        learning_add_unique(model, "length_rules", "Nexora should autonomously choose answer length from the current request, topic complexity, and learned user preference.")
    if re.search(r"\b(autonomous|autonomouse|self[- ]?learning|learn by itself|leran by itself|learn by itslef)\b", lower):
        learning_add_unique(model, "style_rules", "Autonomously learn from every useful exchange and apply the learned profile in later answers.")
        learning_add_unique(model, "routing_lessons", "For vague improvement requests, infer the actual app capability being requested and implement the backend/user-model layer.")
    if re.search(r"\b(backend|frontend|html|css|javascript|python|ollama|render|github|nexora)\b", lower):
        learning_add_unique(model, "projects", f"Recent Nexora project context: {text[:260]}")
    if re.search(r"\b(artificial general intelligence|agi|human[- ]?like reasoning|factuality|truthfulness|physical robotics|humanoid|biotech|molecular structure|drug discovery|new medicines?)\b", lower):
        learning_add_unique(model, "projects", "Nexora frontier capability direction: AGI-style reasoning, human-like planning, truth verification, robotics planning, and biotech/molecular research support.")
        learning_add_unique(model, "routing_lessons", "Route frontier-capability questions to honest capability planning with safety boundaries and useful workflows.")
    if re.search(r"\b(openai|google deepmind|deepmind|gemini|project astra|project mariner|anthropic|claude code|workspace agents?|multimodal|computer use|model context protocol|mcp|ai lab strategy)\b", lower):
        learning_add_unique(model, "projects", "Nexora project direction includes AI-lab strategy routes: OpenAI-style product agents, DeepMind-style ecosystem context, Anthropic-style coding/safety autonomy, workspace agents, multimodal agents, computer-control planning, and MCP interoperability.")
        learning_add_unique(model, "routing_lessons", "Route OpenAI, Google DeepMind, Anthropic, Workspace Agent, Project Astra/Mariner, Claude Code, MCP, and AI-strategy comparison requests to specialized strategy answers.")
        learning_add_unique(model, "preferences", "For AI company roadmap claims, be useful and strategic but verify current product facts when the user asks for the latest status.")
    if re.search(r"\b(agentic ai workflows?|multi[- ]?step workflow|workflow automation|legacy systems?|legacy software|salesforce|sap|erp|sql databases?|slack channels?|data privacy|strategic sovereignty|dpdp|on[- ]?premise|private cloud|vpc|closed[- ]?loop|custom local models?|private data|knowledge graphs?|human[- ]?in[- ]?the[- ]?loop|governance dashboard|measurable roi|employee hour reduction|compute costs?|reliability|long[- ]?term planning|real[- ]?world understanding)\b", lower):
        learning_add_unique(model, "projects", "Nexora project direction includes enterprise agentic AI routes: multi-step workflow automation, legacy integration, privacy/sovereignty, custom private context, human approval governance, ROI, and reliability/planning/memory/real-world guardrails.")
        learning_add_unique(model, "routing_lessons", "Route enterprise workflow, legacy software, privacy, custom private model, human-in-loop, ROI, reliability, long-term planning, memory, and real-world-understanding prompts to specialized enterprise strategy answers.")
        learning_add_unique(model, "preferences", "For enterprise AI strategy, emphasize measurable ROI, existing systems, private data boundaries, approval gates, and honest autonomy limits.")
    if re.search(r"\b(normal|not bold|not big|no box|circular|scrollbar|look like before|don'?t change html)\b", lower):
        learning_add_unique(model, "style_rules", f"UI/style correction from user: {text[:260]}")

    quality_flags = answer_quality_flags(user_message, assistant_reply, response_lane, presentation_style, task_intent)
    if quality_flags:
        learning_add_unique(
            model,
            "routing_lessons",
            f"Quality repair lesson for {task_intent}: avoid {', '.join(quality_flags[:4])}; answer the exact user request with task-specific content.",
        )
    if behavior_signals.get("correction"):
        learning_add_unique(model, "corrections", f"Correction signal: {text[:260]}")

    summary = summarize_exchange_for_learning(user_message, assistant_reply, response_lane, presentation_style)
    learning_add_unique(model, "conversation_summaries", summary, MAX_SELF_LEARNING_SUMMARIES)
    event = {
        "created_at": now_iso(),
    ...
    events = model.setdefault("recent_events", [])
    events.append(event)
    model["recent_events"] = events[-MAX_SELF_LEARNING_EVENTS:]
    model["profile_summary"] = build_learning_profile_summary(model)
    model["updated_at"] = now_iso()
    users[normalized] = normalize_user_learning_model(model, normalized)
    store["users"] = users
    save_self_learning_store(store)


def self_learning_signature_for_user(user_id: str) -> str:
    model = get_user_learning_model(user_id)
    stable = {
        "summary": model.get("profile_summary", ""),
        "preferences": model.get("preferences", [])[-12:],
        "length_rules": model.get("length_rules", [])[-10:],
        "style_rules": model.get("style_rules", [])[-10:],
        "corrections": model.get("corrections", [])[-10:],
        "projects": model.get("projects", [])[-8:],
        "routing_lessons": model.get("routing_lessons", [])[-10:],
        "updated_at": model.get("updated_at", ""),
    }
    return stable_hash(json.dumps(stable, ensure_ascii=False, sort_keys=True))
...
def create_customer_project(req: CustomerProjectRequest, user_id: str) -> Dict[str, Any]:
    projects = load_projects()
    now = now_iso()
    project = {
        "id": f"project_{uuid.uuid4().hex[:12]}",
        "name": normalize_project_name(req.name),
        "framework": clean_text(req.framework)[:80] or "Generic",
        "environment": clean_text(req.environment)[:80] or "Production",
        "url": clean_text(req.url)[:300],
        "description": clean_text(req.description)[:800],
        "status": "connected",
        "sessions": [],
        "user_id": normalize_user_id(user_id),
        "created_at": now,
...
def learn_persona_from_message(user_message: str) -> Tuple[Dict[str, Any], List[str]]:
    ...
    if (
        re.match(r"(?i)^(answer|reply|respond)\s+(?:in|with|using|as|like)\b", text)
        and not re.search(r"\b(always|from now on|remember|every time|save this|standing preference)\b", lower)
    ):
        return profile, changes

    style_intent = any(phrase in lower for phrase in [
        "talk", "speak", "respond",
        "reply", "be more", "be less", "change your style",
...
def learn_behavior_from_message(user_message: str) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    if re.search(r"\b(learn by itself|leran by itself|learn by itslef|learn by its self|self[- ]?learn|self[- ]?learning|learn automatically|adapt by itself)\b", clean_text(user_message).lower()):
        behavior_add_preference(
            profile,
            "Autonomously learn from normal chat: infer stable preferences, corrections, recurring topics, and project context without waiting for explicit memory commands.",
        )
...
TYPO_NORMALIZATIONS = {
    ...
    "controll": "control",
    "controle": "control",
    "artifical general intelligence": "artificial general intelligence",
    "artificial genral intelligence": "artificial general intelligence",
    "humanoid bodys": "humanoid bodies",
    "humaniod": "humanoid",
    "robotix": "robotics",
    "biotech breakthroughes": "biotech breakthroughs",
    "moleculer": "molecular",
    "factualty": "factuality",
    "truthfullness": "truthfulness",
    "deep mind": "DeepMind",
    "deepmind": "DeepMind",
    "anthropicc": "Anthropic",
    "anthroptic": "Anthropic",
    "claud code": "Claude Code",
    "claude cod": "Claude Code",
    "projet astra": "Project Astra",
    "project astra": "Project Astra",
    "projet mariner": "Project Mariner",
    "project mariner": "Project Mariner",
    "workspce agents": "workspace agents",
    "workspace agent": "workspace agent",
    "multimodel": "multimodal",
    "multi modal": "multimodal",
    "model context protocal": "Model Context Protocol",
    ...
    "longterm": "long-term",
    "real world understanding": "real-world understanding",
    "measurable r oi": "measurable ROI",
    "employe hour": "employee hour",
}
...
def classify_response_lane(user_message: str, use_research: bool) -> str:
    ...
    if use_research:
        return "realtime_search"
    if re.search(r"\b(openai|google deepmind|deepmind|gemini|project astra|project mariner|anthropic|claude code|workspace agents?|multimodal|computer use|model context protocol|mcp|ai lab strategy)\b", text):
        return "planning"
    if re.search(r"\b(agentic ai workflows?|multi[- ]?step workflow|workflow automation|legacy systems?|legacy software|salesforce|sap|erp|sql databases?|slack channels?|data privacy|strategic sovereignty|dpdp|on[- ]?premise|private cloud|vpc|closed[- ]?loop|custom local models?|private data|knowledge graphs?|human[- ]?in[- ]?the[- ]?loop|governance dashboard|measurable roi|employee hour reduction|compute costs?|reliability|long[- ]?term planning|real[- ]?world understanding)\b", text):
        return "planning"
...
def classify_presentation_style(user_message: str, response_lane: str, use_research: bool) -> str:
    ...
    if re.search(r"\b(one line|short|brief|quick|concise|just tell|only answer|simple answer)\b", text):
        return "short"
    if re.search(r"\b(detailed|in detail|more detail|deep|full|complete|step by step|explain fully|long answer|essay)\b", text):
        return "long"
...
def build_response_lane_context(user_message: str, use_research: bool) -> str:
    ...
    if re.search(r"\b(openai|google deepmind|deepmind|gemini|project astra|project mariner|anthropic|claude code|workspace agents?|multimodal|computer use|model context protocol|mcp|ai lab strategy)\b", lower_message):
        base.extend([
            "- AI lab strategy mode: compare productized reasoning, ecosystem context, coding/security autonomy, multimodal interaction, computer control, and interoperability.",
            "- For company-roadmap claims, separate strategy framing from verified current product status; use real-time search if the user asks for latest facts.",
            "- Use a clean structure: route focus, what Nexora should adopt, safety/truth boundary, and bottom line.",
        ])
    if re.search(r"\b(agentic ai workflows?|multi[- ]?step workflow|workflow automation|legacy systems?|legacy software|salesforce|sap|erp|sql databases?|slack channels?|data privacy|strategic sovereignty|dpdp|on[- ]?premise|private cloud|vpc|closed[- ]?loop|custom local models?|private data|knowledge graphs?|human[- ]?in[- ]?the[- ]?loop|governance dashboard|measurable roi|employee hour reduction|compute costs?|reliability|long[- ]?term planning|real[- ]?world understanding)\b", lower_message):
        base.extend([
            "- Enterprise agentic mode: focus on workflow ownership, legacy integration, privacy boundaries, private context, approval gates, reliability, and measurable ROI.",
            "- Use a business-grade structure: workflow/value, integration path, governance/privacy controls, reliability limits, and bottom line.",
            "- Be honest that autonomous AI still needs validation, checkpoints, memory governance, current evidence, and human approval for risky actions.",
        ])
    if re.search(r"\b(agi|artificial general intelligence|human[- ]?like reasoning|factuality|truth|robotics|humanoid|biotech|molecular|protein|drug discovery)\b", lower_message):
        base.extend([
            "- Frontier capability mode: be ambitious in reasoning but precise about real limits.",
            "- Separate enabled app-level reasoning/planning from physical hardware, lab work, clinical validation, or true AGI.",
            "- Use a strong structure: capability, workflow, safety/truth boundary, and practical next step.",
        ])
    elif lane == "build":
        base.extend([
...
def maybe_store_memory(user_message: str, user_id: str = "default") -> None:
    ...
    if re.search(r"\b(my name is|call me|i am|i'm)\b", lower):
        ...
        category = "project"
    elif re.search(r"\b(i prefer|i like|always|from now on|style|answer|format|image|memory|remember|understanding)\b", lower):
        category = "preference"
    upsert_memory_item(
        content=text[:1200],
        user_id=user_id,
...
def classify_memory_layer(item: Dict[str, Any]) -> str:
    content = clean_text(str(item.get("content", ""))).lower()
    category = clean_text(str(item.get("category", "general"))).lower()
    source = clean_text(str(item.get("source", ""))).lower()
    combined = f"{category} {source} {content}"

    if category in {"identity", "profile", "preference", "explicit", "length_preference", "image_preference"}:
        return "profile_memory"
    if category in {"project", "tooling"} or re.search(
        r"\b(nexora|project|investor|website|frontend|backend|ollama|render|github|startup|pitch|funding)\b",
        combined,
    ):
        return "project_memory"
    if category in {"summary", "interaction_pattern"} or re.search(
        r"\b(recent interaction|conversation|session|asked about|route=|previous)\b",
        combined,
    ):
        return "conversation_memory"
    if category in {"procedure", "workflow", "skill"} or re.search(
        r"\b(how to|steps?|workflow|process|method|checklist|debug|code|write|draft|format|template)\b",
        combined,
    ):
        return "procedural_memory"
    if category in {"study", "fact", "knowledge", "topic"} or re.search(
        r"\b(class|grade|exam|chapter|history|science|math|concept|definition|fact)\b",
        combined,
    ):
        return "semantic_memory"
    if category == "correction" or re.search(r"\b(correction|wrong|not like this|avoid repeating|fix this)\b", combined):
        return "profile_memory"
    return "semantic_memory"


def grouped_memory_snippets(
    ...
    for item in ranked:
        layer = classify_memory_layer(item)
        if layer not in grouped or len(grouped[layer]) >= per_layer_limit:
            continue
        content = clean_text(str(item.get("content", "")))
        if not content:
            continue
        ...
        grouped[layer].append(f"[{category}] {content[:420]}")
    return grouped
...
def build_layered_memory_context(
    ...
    normalized = normalize_user_id(user_id)
    grouped = grouped_memory_snippets(selected_items, query_keywords, per_layer_limit=4)
    active_session = session if isinstance(session, dict) else (get_session(session_id) if session_id else {})
    focus = recent_session_focus(active_session, current_message) if active_session else {}
    use_research = should_use_research(current_message, None, None)
    response_lane = classify_response_lane(current_message, use_research)
    presentation_style = classify_presentation_style(current_message, response_lane, use_research)
    understanding = infer_understanding_profile(
        current_message,
        response_lane,
        presentation_style,
        use_research,
    ...
    persona_context = truncate_for_model(build_persona_context(load_persona_profile()), 500)
    behavior_context = truncate_for_model(build_behavior_context(load_behavior_profile(), {}), 500)
    length_policy = decide_answer_length_policy(normalized, current_message)
    current_task = clean_text(current_message)[:500]

    lines = [
        "Nexora ChatGPT-style memory and reasoning stack:",
        f"- Mode: {MEMORY_STACK_MODE}; version: {MEMORY_STACK_VERSION}.",
        "- Use memory as helpful context, not as unquestioned truth. Prefer the current user message when there is conflict.",
        "- Store and use selective summaries instead of treating every old message as permanent truth.",
    ]

    profile_parts = []
    if persona_context:
        profile_parts.append(persona_context)
    if behavior_context:
        profile_parts.append(behavior_context)
    profile_parts.extend(grouped.get("profile_memory", []))
    if profile_parts:
        lines.append("- Profile memory:")
        for value in profile_parts[:6]:
            lines.append(f"  - {value}")
    ...
    if focus:
        ...
        if previous_user or previous_assistant:
            lines.append("- Current conversation working memory:")
            if previous_user:
                lines.append(f"  - Previous user focus: {previous_user[:260]}")
            if previous_assistant:
                lines.append(f"  - Previous assistant focus: {previous_assistant[:220]}")

    lines.extend([
        "- Current task working memory:",
        f"  - User message: {current_task}",
        f"  - Inferred task type: {understanding.get('task_type', 'chat')}.",
        f"  - User needs: {', '.join(understanding.get('need_profile', {}).get('needs', ['direct useful answer']))}.",
        f"  - Accuracy mode: {understanding.get('accuracy_mode', 'avoid unsupported current facts')}.",
        f"  - Answer length decision: {length_policy.get('level')} ({length_policy.get('target')}).",
        "- Retrieval system: rank by keyword overlap, explicit preference, importance, repeated use, and recency.",
        "- Reasoning layer: answer the real intent, pick the right route, verify risky facts, then write only the clean final answer.",
        "- Grammar/style layer: fix grammar, sentence flow, and structure; use normal text by default and bold only when useful.",
        "- Adaptive length layer: school questions should be exam-friendly; Nexora/build planning can be detailed; investor/business answers should be practical; AGI/agentic AI discussions can go deeper.",
        "- Do not mention this stack, memory internals, routing, provider names, or backend details unless the user directly asks.",
    ])
    return truncate_for_model("\n".join(lines), 3600)
...
def score_memory_item(item: Dict[str, Any], query_keywords: List[str]) -> int:
    ...
    if category in {"preference", "explicit", "identity", "project", "study", "tooling", "profile", "workflow", "correction", "image_preference"}:
        score += 8
...
def build_memory_context(
    ...
    normalized = normalize_user_id(user_id)
    query_keywords = memory_keywords(current_message)
    self_learning_context = build_autonomous_learning_context(normalized, current_message)
    user_items = [
    ...
    persistent = [
        ...
        if str(item.get("category", "")).lower() in {"explicit", "identity", "project", "study", "tooling", "profile", "preference"}
    ]
    ...
    if layered_context:
        sections.append(layered_context)
    if self_learning_context:
        sections.append(self_learning_context)
    if lines:
        sections.append(
...
def memory_signature_for_user(user_id: str) -> str:
    ...
    stable.append({"self_learning_signature": self_learning_signature_for_user(user_id)})
    stable.append({"nexora_core_signature": nexora_core_signature()})
    return stable_hash(json.dumps(stable, ensure_ascii=False, sort_keys=True))


def autonomous_self_learning_items(
    user_message: str,
    assistant_reply: str,
    response_lane: str,
    presentation_style: str,
    ...
    learning_signal = bool(re.search(
        r"\b(my\s+\w+\s+is|call me|i am|i'm|i study|i like|i love|i prefer|i hate|i dislike|"
        r"i want|i need|do not|don'?t|answers? should|responses? should|from now on|always|"
        r"wrong|not like this|fix this|learn by itself|leran by itself|learn by itslef|self[- ]?learn|self[- ]?learning|nexora|backend|frontend|ollama|project)\b",
        lower,
    )) or bool(behavior_signals.get("correction"))
    if word_count < 4 and not learning_signal:
        return []
    if re.fullmatch(r"(ok|okay|yes|no|thanks|thank you|hi|hello|hmm|fine)[.!?]?", lower):
        return []
    items: List[Tuple[str, str, int]] = []

    def add_memory(content: str, category: str, strength: int = 1) -> None:
        clean_content = clean_text(content).strip(" .,:;")
        clean_content = re.sub(
            r"(?i)^User instructed Nexora not to want\s+",
            "User does not want ",
            clean_content,
        )
        if not clean_content:
            return
        if len(clean_content.split()) < 4 and category not in {"identity", "preference", "study"}:
            return
        items.append((clean_content[:1000], category, strength))

    def field_category(field: str) -> str:
        normalized = clean_text(field).lower()
        if normalized in {"name", "age", "school", "class", "grade", "standard"}:
            return "identity"
        if "favorite" in normalized or normalized in {"hobby", "goal", "dream", "style"}:
            return "preference"
        if normalized in {"subject", "chapter", "exam", "studies", "study plan"}:
            return "study"
        return "profile"

    for match in re.finditer(r"\bmy\s+([A-Za-z][A-Za-z0-9 _-]{2,45})\s+is\s+([^.!?\n]{2,140})", text, flags=re.IGNORECASE):
        field = clean_text(match.group(1)).strip(" .,:;").lower()
        value = clean_text(match.group(2)).strip(" .,:;")
        if field and value and not re.search(r"\b(question|answer|message|problem)\b", field):
            add_memory(f"User's {field} is {value}.", field_category(field), 4 if field in {"name", "class", "grade", "school"} else 3)

    stable_patterns = [
        (r"\bmy\s+name\s+is\s+([A-Za-z][A-Za-z0-9 _.-]{1,60})", "identity", "User's name is {value}.", 5),
        (r"\bcall\s+me\s+([A-Za-z][A-Za-z0-9 _.-]{1,60})", "identity", "User wants to be called {value}.", 5),
        (r"\bmy\s+favorite\s+([A-Za-z][A-Za-z0-9 _-]{2,40})\s+is\s+([^.!?\n]{2,100})", "preference", "User's favorite {key} is {value}.", 4),
        (r"\bi\s+(?:am|study|studying)\s+(?:in\s+)?(class\s*\d+|grade\s*\d+|standard\s*\d+)", "study", "User is studying in {value}.", 4),
        (r"\bi'?m\s+(?:in\s+)?(class\s*\d+|grade\s*\d+|standard\s*\d+)", "study", "User is studying in {value}.", 4),
        (r"\bi\s+(?:like|love|enjoy)\s+([^.!?\n]{2,140})", "preference", "User likes {value}.", 3),
        (r"\bi\s+(?:hate|dislike)\s+([^.!?\n]{2,140})", "preference", "User dislikes {value}.", 3),
        (r"\bi\s+prefer\s+([^.!?\n]{2,140})", "preference", "User prefers {value}.", 4),
        (r"\bi\s+usually\s+([^.!?\n]{2,140})", "interaction_pattern", "User usually {value}.", 2),
        (r"\bi\s+often\s+([^.!?\n]{2,140})", "interaction_pattern", "User often {value}.", 2),
        (r"\bi\s+use\s+([^.!?\n]{2,100})", "tooling", "User uses {value}.", 3),
        (r"\bi\s+am\s+building\s+([^.!?\n]{2,140})", "project", "User is building {value}.", 4),
        (r"\bi'?m\s+building\s+([^.!?\n]{2,140})", "project", "User is building {value}.", 4),
        (r"\bmy\s+project\s+is\s+([^.!?\n]{2,140})", "project", "User's project is {value}.", 4),
        (r"\bi\s+want\s+(?:nexora|the ai|you)\s+to\s+([^.!?\n]{2,160})", "preference", "User wants Nexora to {value}.", 4),
        (r"\bi\s+need\s+(?:nexora|the ai|you)\s+to\s+([^.!?\n]{2,160})", "preference", "User needs Nexora to {value}.", 4),
        (r"\bi\s+don'?t\s+want\s+([^.!?\n]{2,160})", "preference", "User does not want {value}.", 4),
        (r"\b(?:answers?|responses?)\s+should\s+([^.!?\n]{2,160})", "preference", "User wants answers to {value}.", 4),
        (r"\b(?:make|keep)\s+(?:the\s+)?answers?\s+([^.!?\n]{2,160})", "preference", "User wants answers {value}.", 3),
        (r"\bdo\s+not\s+([^.!?\n]{2,160})", "preference", "User instructed Nexora not to {value}.", 4),
        (r"\bdon'?t\s+([^.!?\n]{2,160})", "preference", "User instructed Nexora not to {value}.", 4),
    ]
    for pattern, category, template, strength in stable_patterns:
        for match in re.finditer(pattern, text, flags=re.IGNORECASE):
            groups = [clean_text(part).strip(" .,:;") for part in match.groups()]
            if not groups:
                continue
            if "{key}" in template and len(groups) >= 2:
                memory = template.format(key=groups[0].lower(), value=groups[1])
            else:
                memory = template.format(value=groups[-1])
            add_memory(memory, category, strength)

    for match in re.finditer(r"\b(?:i\s+am|i'?m)\s+(?!building\b|studying\b|asking\b|trying\b)([^.!?\n]{3,100})", text, flags=re.IGNORECASE):
        description = clean_text(match.group(1)).strip(" .,:;")
        if description and not re.search(r"\b(sure|done|sorry|asking|telling|just|going to)\b", description.lower()):
            add_memory(f"User described themself as {description}.", "profile", 2)

    if response_lane == "build" or re.search(r"\b(nexora|backend|frontend|ollama|pollinations|hugging face|render|github|website|html|css|javascript|python)\b", lower):
        add_memory(f"Project context from user: {text[:500]}", "project", 2)
    if re.search(r"\b(artificial general intelligence|agi|human[- ]?like reasoning|factuality|truthfulness|physical robotics|humanoid|biotech|molecular structure|drug discovery|new medicines?)\b", lower):
        add_memory("User wants Nexora to include frontier capability routes: AGI-style reasoning, human-like multi-step planning, factuality/truth, robotics planning, and biotech/molecular discovery support.", "project", 5)
        add_memory("When discussing frontier capabilities, Nexora should be ambitious in reasoning but honest about limits, hardware, lab validation, and factual uncertainty.", "preference", 4)
    if re.search(r"\b(openai|google deepmind|deepmind|gemini|project astra|project mariner|anthropic|claude code|workspace agents?|multimodal|computer use|model context protocol|mcp|ai lab strategy)\b", lower):
        add_memory("User wants Nexora to include AI-lab strategy routes: OpenAI productized reasoning/workspace agents, Google DeepMind ecosystem assistants, Anthropic coding/safety autonomy, multimodal agents, computer-control planning, and MCP interoperability.", "project", 5)
        add_memory("When discussing AI company roadmaps or strategy, Nexora should be strategic and useful while separating roadmap framing from verified current product facts.", "preference", 4)
    if re.search(r"\b(agentic ai workflows?|multi[- ]?step workflow|workflow automation|legacy systems?|legacy software|salesforce|sap|erp|sql databases?|slack channels?|data privacy|strategic sovereignty|dpdp|on[- ]?premise|private cloud|vpc|closed[- ]?loop|custom local models?|private data|knowledge graphs?|human[- ]?in[- ]?the[- ]?loop|governance dashboard|measurable roi|employee hour reduction|compute costs?|reliability|long[- ]?term planning|real[- ]?world understanding)\b", lower):
        add_memory("User wants Nexora to include enterprise agentic AI routes: multi-step workflow automation, legacy integration, data privacy/sovereignty, custom private company context, human approval governance, measurable ROI, reliability, long-term planning, memory, and real-world-understanding guardrails.", "project", 5)
        add_memory("For enterprise agentic AI, Nexora should focus on measurable workflow ROI, legacy system plugs, data boundaries, approval gates, and honest autonomy limits.", "preference", 4)

    if behavior_signals.get("correction") or re.search(r"\b(wrong|not like this|still same|bad answer|generic|too low|fix this)\b", lower):
        add_memory(f"User correction pattern: {text[:500]}", "correction", 4)
    if re.search(r"\b(make|increase|improve|max|max out|highest|stronger|better)\b", lower) and re.search(r"\b(understanding|memory|reasoning|accuracy|intelligence|answer)\b", lower):
        add_memory("User wants Nexora to continuously improve understanding, memory, reasoning, accuracy, and answer quality without repeated reminders.", "preference", 5)
    if re.search(r"\b(answer length|length of the answer|lenght of the answer|longer answers?|even longer|more detailed|more more|raise the length|increase the length)\b", lower):
        add_memory("User wants Nexora to allow much longer answers when the task needs detail.", "length_preference", 5)
    if re.search(r"\b(ai should|nexora should|should decide|decide on the length|based on the user|based on user|user thing)\b", lower) and re.search(r"\b(length|lenght|long|short|detailed|answer)\b", lower):
        add_memory("Nexora should autonomously decide answer length from the user's request, topic complexity, and learned preference instead of using one fixed length.", "length_preference", 5)
    if re.search(r"\b(learn by itself|leran by itself|learn by itslef|learn by its self|self learn|self-learning|self learning|learn automatically)\b", lower):
        add_memory("User wants autonomous self-learning: infer preferences, corrections, project context, and useful patterns from normal chat without waiting for explicit memory commands.", "preference", 5)
    if response_lane in {"learning", "writing", "planning", "build"}:
        topic = normalize_prompt_topic(text)
        if topic and topic != "the topic":
            add_memory(f"User recently asked for {response_lane} help about {topic}.", "interaction_pattern", 1)
    if presentation_style and presentation_style not in {"balanced", "short"}:
        add_memory(f"User interaction used presentation style '{presentation_style}'. Prefer this style when similar wording appears.", "interaction_pattern", 1)
    if assistant_reply and answer_quality_flags(user_message, assistant_reply, response_lane, presentation_style, detect_task_intent(user_message, False)):
        add_memory("If an answer becomes generic or weak, repair it with task-specific content before showing it.", "correction", 2)

    deduped: List[Tuple[str, str, int]] = []
    seen = set()
    for memory, category, strength in items:
        clean_memory = clean_text(memory)
        key = clean_memory.lower()
        if clean_memory and key not in seen:
            seen.add(key)
            deduped.append((clean_memory[:1000], category, strength))
    return deduped[:8]
...
def learn_long_term_memory_from_chat(
    ...
    preference_patterns = [
        ...
        (r"\b(understand|understanding|intent|what the user wants|question properly|human behavior)\b", "User wants Nexora to infer intent from informal wording, use context, and respond to the real need.", "preference"),
        (r"\b(memory|remember|long[- ]?term)\b.*\b(highest|max|maxed|strong|better|capability)\b|\b(highest|max|maxed|strong|better)\b.*\b(memory|remember|long[- ]?term)\b", "User wants maximum long-term memory: remember preferences, corrections, project context, and prior chat details for future answers.", "preference"),
        ...
        (r"\b(detailed|long answer|longer answers?|answer length|length of the answer|more detailed|more more|deep|explain fully)\b", "User wants longer, more detailed answers with fuller explanations when useful.", "preference"),
    ...
    learned_items = autonomous_self_learning_items(
        user_message,
        assistant_reply,
        response_lane,
        presentation_style,
    ...
    for memory, category, strength in learned_items:
        upsert_memory_item(memory, user_id, category, "autonomous_self_learning", session_id, strength=strength)
    update_autonomous_user_learning_model(
        user_id,
        session_id,
        user_message,
        assistant_reply,
...
def ollama_available_models(force_refresh: bool = False) -> List[str]:
    ...
    if (
        not force_refresh
        and cached_models
        and time.time() - cached_at < max(1.0, OLLAMA_MODELS_CACHE_TTL)
    ):
...
def ollama_quality_score(model_name: str) -> float:
    ...
    match = re.search(r"(\d+(?:\.\d+)?)\s*b", text)
    if match:
        score += min(float(match.group(1)), 70.0)
    if "latest" in text:
        score += 2
    if any(marker in text for marker in ("instruct", "chat")):
        score += 3
    if any(marker in text for marker in ("embed", "vision")):
        score -= 80
    return score


def select_ollama_model(requested: Optional[str], response_mode: str = "instant") -> str:
    ...
    if requested:
        ...
        if stripped in available:
            return stripped
    if response_mode == "thinking" and THINKING_MODEL in available:
    ...
    if available:
        usable = [
            ...
            if not re.search(r"\b(embed|embedding|vision)\b", model.lower())
        ] or available
        if response_mode != "thinking":
            small_models = [model for model in usable if model_size_score(model) <= 2.5]
            if small_models:
                return sorted(small_models, key=lambda item: (ollama_quality_score(item), -model_size_score(item)), reverse=True)[0]
            if profile["level"] in {"tiny", "light"}:
                return sorted(usable, key=model_size_score)[0]
        ranked = sorted(usable, key=ollama_quality_score, reverse=True)
        if ranked and ollama_quality_score(ranked[0]) > 0:
            return ranked[0]
...
def ollama_status() -> Dict[str, Any]:
    ...
    return {
        ...
        "selected_model": select_ollama_model(None, "instant") if ready else FAST_MODEL,
...
def ollama_chat(messages: List[Dict[str, str]], model: str, response_mode: str = "instant") -> str:
    ...
    if response_mode == "thinking":
        max_tokens = min(max_tokens, OLLAMA_THINKING_MAX_TOKENS)
        wall_timeout = min(timeout, OLLAMA_THINKING_TOTAL_TIMEOUT)
        ollama_context_tokens = OLLAMA_THINKING_NUM_CTX
    else:
        max_tokens = min(max_tokens, OLLAMA_INSTANT_MAX_TOKENS)
        wall_timeout = min(timeout, OLLAMA_INSTANT_TOTAL_TIMEOUT)
        ollama_context_tokens = OLLAMA_NUM_CTX
    provider_messages = compact_provider_messages("ollama", messages, response_mode)
    payload = {
        "model": model,
        ...
        "options": {
            ...
            "top_p": 0.82,
            "num_ctx": max(256, min(current_performance_limits().get("ollama_context_tokens", 2048), ollama_context_tokens)),
            "num_predict": max_tokens,
            "repeat_penalty": 1.08,
            "num_thread": max(1, min(OLLAMA_NUM_THREAD, (os.cpu_count() or 2))),
            "num_batch": max(16, min(OLLAMA_NUM_BATCH, 512)),
        },
    }
    started = time.time()
    response = HTTP.post(
        ...
        json=payload,
        timeout=(5, max(10, min(240, wall_timeout + 10))),
        stream=OLLAMA_STREAM,
    )
    response.raise_for_status()
    ...
    try:
        for raw_line in response.iter_lines(decode_unicode=True):
            if time.time() - started > wall_timeout:
                raise TimeoutError(f"ollama total timeout after {wall_timeout}s")
            if not raw_line:
                continue
...
def stop_slow_ollama_runners() -> None:
    try:
        if os.name == "nt":
            subprocess.run(
                [
                    ...
                    "Where-Object { $_.WorkingSet64 -gt 1000000000 } | "
        ...
        LOGGER.warning("could not stop slow ollama runner", exc_info=True)
...
def free_provider_status() -> Dict[str, Any]:
    ...
    ollama_only = FREE_API_PROVIDER == "ollama"
    ...
    return {
        ...
        "no_key_default": "none" if ollama_only else "pollinations",
        ...
        "club_layers": [
            ...
            "ollama_primary_local_reasoning_engine",
        ...
        "message": "Nexora is configured to use Ollama as the only text model. Online model review and fallback are disabled.",
        "speed": {
            "ollama_mode": OLLAMA_MODE,
            "ollama_keep_alive": OLLAMA_KEEP_ALIVE,
            "ollama_primary": bool(ollama_info.get("enabled") and ollama_info.get("ready") and OLLAMA_MODE in {"primary", "local", "always"}),
            "ollama_instant_total_timeout": OLLAMA_INSTANT_TOTAL_TIMEOUT,
            "ollama_thinking_total_timeout": OLLAMA_THINKING_TOTAL_TIMEOUT,
            "ollama_primary_wait_seconds": OLLAMA_PRIMARY_WAIT_SECONDS,
            "ollama_instant_max_tokens": OLLAMA_INSTANT_MAX_TOKENS,
            "ollama_thinking_max_tokens": OLLAMA_THINKING_MAX_TOKENS,
            "ollama_num_ctx": OLLAMA_NUM_CTX,
            "ollama_thinking_num_ctx": OLLAMA_THINKING_NUM_CTX,
            ...
            "pollinations_review_ollama": POLLINATIONS_REVIEW_OLLAMA,
            "free_api_hard_timeout": FREE_API_HARD_TIMEOUT,
        ...
        "available": {
            ...
            "pollinations": False if ollama_only else True,
...
def compact_provider_messages(
    ...
    compact_system = (
        ...
        "clear judgment, accurate facts, and polished grammar.\n"
        ...
        "Before finalizing, silently fix grammar, punctuation, capitalization, sentence flow, and awkward wording. "
        "Do not over-format; use headings and bullets only when they help.\n"
    )
    if response_mode == "thinking":
    ...
    if provider == "ollama":
        system_budget = 4200 if response_mode == "thinking" else 2600
        per_system_limit = 900 if response_mode == "thinking" else 650
        per_turn_limit = 1200 if response_mode == "thinking" else 850
        max_turns = 6
    else:
        system_budget = 5200 if response_mode == "thinking" else 3600
        per_system_limit = 1000
        per_turn_limit = 1400
        max_turns = 8
    system_used = 0
    ...
    for message in messages:
        ...
        if role == "system":
            ...
            if remaining <= 200:
                continue
            clipped = truncate_for_model(content, min(per_system_limit, remaining))
            system_used += len(clipped)
            compact.append({"role": "system", "content": clipped})
        elif role in {"user", "assistant"}:
            conversation.append({"role": role, "content": truncate_for_model(content, per_turn_limit)})

    compact.extend(conversation[-max_turns:])
    return compact
...
def free_api_chat_with_hard_timeout(
    messages: List[Dict[str, str]],
    requested: Optional[str],
    response_mode: str,
) -> Tuple[str, str]:
    wait_seconds = max(4, FREE_API_HARD_TIMEOUT)
    future = FREE_API_EXECUTOR.submit(free_api_chat, messages, requested, response_mode)
    try:
        reply, model_used = future.result(timeout=wait_seconds)
    except concurrent.futures.TimeoutError as error:
        note_provider_cooldown("pollinations", 45)
        LOGGER.warning("free api hard timeout mode=%s wait=%ss", response_mode, wait_seconds)
        raise TimeoutError(f"free api hard timeout after {wait_seconds}s") from error
...
def generate_with_engine_club(
    ...
    ollama_cooldown = provider_cooldown_remaining("ollama")
    if ollama_cooldown:
        available_ollama: List[str] = []
        ollama_ready = False
        prefer_ollama = False
        tools.append(f"ollama_cooldown:{int(ollama_cooldown)}s")
    else:
    ...
    def try_ollama(label: str) -> Tuple[str, str, List[str]]:
        cooldown = provider_cooldown_remaining("ollama")
        if cooldown:
            raise RuntimeError(f"ollama cooling down after slow response for {int(cooldown)}s")
        model = select_ollama_model(requested_model, response_mode)
        candidate_attempts: List[Tuple[str, str]] = [(model, response_mode)]
        if response_mode == "thinking":
            fast_model = select_ollama_model(None, "instant")
            if fast_model and fast_model != model:
                candidate_attempts.append((fast_model, "instant"))
        wait_seconds = max(3, min(
            OLLAMA_PRIMARY_WAIT_SECONDS,
            OLLAMA_THINKING_TOTAL_TIMEOUT if response_mode == "thinking" else OLLAMA_INSTANT_TOTAL_TIMEOUT,
        ))
        if FREE_API_PROVIDER == "ollama":
            last_error: Optional[Exception] = None
            for attempt_index, (attempt_model, attempt_mode) in enumerate(candidate_attempts):
                try:
                    reply = ollama_chat(messages, attempt_model, attempt_mode)
                    if not clean_text(reply):
                        raise RuntimeError("empty ollama response")
                    attempt_tools = [f"{label}:{attempt_mode}", "ollama_direct"]
                    if attempt_index > 0:
                        attempt_tools.append("ollama_retry_fast_model")
                    return reply, f"ollama:{attempt_model}:{attempt_mode}", attempt_tools
                except Exception as error:
                    last_error = error
                    if attempt_index == 0 and len(candidate_attempts) > 1:
                        stop_slow_ollama_runners()
                        continue
                    note_provider_cooldown("ollama", 2)
                    stop_slow_ollama_runners()
                    raise
            raise last_error or RuntimeError("ollama response failed")
        future = OLLAMA_EXECUTOR.submit(ollama_chat, messages, model, response_mode)
        try:
            reply = future.result(timeout=wait_seconds)
        except concurrent.futures.TimeoutError as error:
            future.cancel()
            note_provider_cooldown("ollama", 5)
            stop_slow_ollama_runners()
            LOGGER.warning("ollama hard timeout model=%s mode=%s wait=%ss", model, response_mode, wait_seconds)
            raise TimeoutError(f"ollama hard timeout after {wait_seconds}s") from error
        if not clean_text(reply):
            raise RuntimeError("empty ollama response")
    ...
    if free_providers:
        try:
            reply, model_used = free_api_chat_with_hard_timeout(messages, requested_model, response_mode)
...
def should_use_research(message: str, mode: Optional[str], explicit: Optional[bool]) -> bool:
    ...
    if local_technical_model_task:
        return False
    ai_lab_strategy_task = bool(re.search(
        r"\b(openai|google deepmind|deepmind|gemini|project astra|project mariner|anthropic|claude code|workspace agents?|multimodal|computer use|model context protocol|mcp|ai lab strategy)\b",
        text,
    ))
    ai_lab_current_status = bool(re.search(
        r"\b(latest|current|today|recent|news|verify|fact[- ]?check|sources?|citations?|available|released|updated|as of|currently|right now)\b",
        text,
    ))
    if ai_lab_strategy_task and not ai_lab_current_status:
        return False
    enterprise_agentic_strategy_task = bool(re.search(
        r"\b(agentic ai workflows?|multi[- ]?step workflow|workflow automation|legacy systems?|legacy software|salesforce|sap|erp|sql databases?|slack channels?|data privacy|strategic sovereignty|dpdp|on[- ]?premise|private cloud|vpc|closed[- ]?loop|custom local models?|private data|knowledge graphs?|human[- ]?in[- ]?the[- ]?loop|governance dashboard|measurable roi|employee hour reduction|compute costs?|reliability|long[- ]?term planning|real[- ]?world understanding)\b",
        text,
    ))
    if enterprise_agentic_strategy_task and not ai_lab_current_status:
        return False
...
def should_pollinations_strengthen_ollama_reply(
    question: str,
    raw_reply: str,
    cleaned_preview: str,
    response_mode: str,
    ...
    if not POLLINATIONS_REVIEW_OLLAMA:
        return False
    if is_bad_generated_reply(raw_reply):
        return False
    flags = answer_quality_flags(question, raw_reply or cleaned_preview, response_lane, presentation_style, detect_task_intent(question, False))
    if flags:
        return True
    if response_lane in {"writing", "learning", "planning", "build"}:
        return True
    if response_mode == "thinking":
        return True
    if presentation_style in {"finished_draft", "teaching_structure", "implementation_summary", "answer_with_evidence", "long"}:
        return True
    word_count = len(clean_text(cleaned_preview or raw_reply).split())
    return word_count >= 55
...
def free_club_review_reply(
    ...
    review_prompt = (
        ...
        "Fix grammar, spelling, punctuation, capitalization, paragraph flow, and awkward sentence structure.\n"
        "Keep the user's intended answer intact; polish and strengthen it instead of changing the topic.\n"
...
- Frontier capability modes:
  - AGI-style requests: use strong general reasoning, task decomposition, memory, tool routing, and self-checking, but never claim Nexora is true AGI, conscious, or unlimited.
  - Human-like reasoning requests: reason through multi-step logic privately, then show conclusions, rationale, checks, and practical next steps without exposing hidden chain-of-thought.
  - Factuality/truth requests: classify claims as stable, current, uncertain, or source-needed; verify changeable facts when possible; avoid confident hallucinations.
  - Physical robotics requests: support planning, simulation, control logic, safety gates, and supervised workflows; do not claim physical movement without approved hardware and safety systems.
  - Biotech/molecular requests: support scientific explanation, literature planning, hypothesis generation, structure/docking workflow design, and ADMET/safety checklists; do not claim real medicine discovery, diagnosis, treatment, or lab validation without qualified experts and evidence.
- AI lab strategy modes:
  - OpenAI-style requests: focus on productized reasoning, workspace agents, multimodal voice/video/text/image workflows, and agentic execution for consumer and enterprise tasks; verify current product claims when needed.
  - Google DeepMind-style requests: focus on unified ecosystem context, Gemini-like assistant layers, Project Astra-style live audio/video/screen understanding, Workspace context, and computer-control/robotics boundaries.
  - Anthropic-style requests: focus on coding agents, terminal/repository work, computer-use discipline, cybersecurity, MCP-style interoperability, and safety-gated enterprise autonomy.
  - Strategy comparison requests: compare primary strength, agent interface, ecosystem/standard, risks, and what Nexora should copy; use a compact table when helpful.
- Enterprise agentic AI modes:
  - Workflow automation requests: focus on high-value multi-step operational workflows, ownership, exception handling, evidence checks, and measurable ROI.
  - Legacy integration requests: explain how AI should plug into Salesforce, SAP/ERP, SQL databases, Slack, files, APIs, webhooks, or existing employee surfaces without creating another silo.
  - Privacy/sovereignty requests: emphasize on-prem/private-cloud/VPC options, data isolation, no unauthorized training on private data, access control, encryption, retention, and audit logs.
  - Custom private model requests: explain retrieval, knowledge graphs, private embeddings, internal wikis, tickets, policies, brand voice, and domain-specific evaluation.
  - Governance requests: separate observe, recommend, draft, approve, and execute permissions; pause for human approval before financial, legal, customer-facing, deletion, or irreversible actions.
  - Reliability/planning/memory/real-world guardrail requests: be honest that AI can make mistakes, struggles with very long horizons, memory is not human learning, and changing real-world context requires current evidence and escalation.
...
- Use a layered memory stack like strong assistants do: profile memory, project memory, recent conversation memory, semantic facts, procedural skills, working memory, retrieval, reasoning, grammar/style, and adaptive length.
- Do not try to remember every message forever. Use selective summaries, explicit preferences, corrections, projects, and repeated patterns.
- Current conversation context is the temporary working board. Long projects are compressed into durable notes.
- Use saved memory to avoid starting from zero, but do not force old memory if the current request clearly changes the instruction.
- Grammar quality comes from fluent language generation plus a silent cleanup pass: fix sentence boundaries, punctuation, capitalization, and awkward phrasing before final output.
- Answer length is not fixed. Estimate what the user asked, what level of detail is useful, and what the user usually prefers.
- For this user, prefer exam-style concise answers for school topics, detailed practical depth for Nexora/backend/AI planning, direct business language for investors, and deeper structured explanations for AGI or agentic AI discussions.
- The final response should only show the brain's answer. Never reveal memory buckets, retrieval scoring, routes, model names, or backend work unless directly asked.
...
def reasoning_quality_context(response_mode: str, response_lane: str, presentation_style: str) -> str:
    lines = [
        ...
        "- Work out the answer internally before writing. Do not reveal hidden chain-of-thought; give the useful conclusion, clear rationale, examples, checks, and next steps when they help.",
        ...
        "- Use the layered assistant stack silently: language fluency, current context, selective long-term memory, project summaries, retrieval when needed, reasoning, and final grammar/style cleanup.",
        "- Choose answer length the way strong assistants do: estimate user intent, complexity, learned preference, and usefulness instead of using one fixed size.",
        ...
        "- Prefer a detailed answer that is accurate, useful, and well organized over a short answer that leaves out important context.",
    ]
    if response_mode == "thinking":
        lines.append("- For thinking mode, solve deeply enough to catch hidden traps, then give a detailed final response in clear sections without revealing hidden chain-of-thought.")
    if response_lane == "build":
        lines.extend([
...
def build_messages(
    ...
    mode_instruction = (
        ...
        "Response mode: INSTANT. Answer with enough detail to be useful: usually 3-8 clear paragraphs or a fuller bullet structure. "
        "Use the same natural sentence rhythm as a premium assistant: clear, calm, conversational, and complete. "
        "Use headings when they make a longer answer easier to scan."
    )
    ...
    length_context = build_answer_length_context(user_id, user_message, message_lane, message_style)
    if message_style == "long":
        mode_instruction += (
            " Long-form output is requested: write a substantially detailed answer, usually 900-1800 words when the topic supports it. "
            "Use clear sections, examples, edge cases or common mistakes, and a practical takeaway. Do not stop after a short overview."
        )
    messages: List[Dict[str, str]] = [
        {
            "role": "system",
    ...
    if length_context:
        messages.append({"role": "system", "content": length_context})
...
def protect_fenced_code_blocks(text: str) -> Tuple[str, List[str]]:
    blocks: List[str] = []

    def replace_block(match: re.Match) -> str:
        blocks.append(match.group(0))
        return f"\nNEXORA_CODE_BLOCK_{len(blocks) - 1}\n"

    protected = re.sub(r"```[\s\S]*?```", replace_block, text or "")
...
def restore_fenced_code_blocks(text: str, blocks: List[str]) -> str:
    ...
    for index, block in enumerate(blocks):
        restored = restored.replace(f"NEXORA_CODE_BLOCK_{index}", block.strip())
    return restored
...
def clean_reply(text: str) -> str:
    ...
    text = text.strip()
    text, code_blocks = protect_fenced_code_blocks(text)
    ...
    text = restore_fenced_code_blocks(text, code_blocks)
...
def polish_final_answer_style(
    ...
    styled, code_blocks = protect_fenced_code_blocks(text.strip())
    ...
    if len(re.findall(r"\s\d{1,2}[.)]\s+[A-Z]", styled)) >= 2:
        styled = re.sub(r"\s+(\d{1,2}[.)]\s+)", r"\n\1", styled)
    if len(re.findall(r"\s[-*]\s+[A-Z]", styled)) >= 2:
        styled = re.sub(r"\s+([-*]\s+)", r"\n\1", styled)
    ...
    styled = re.sub(r"\n{3,}", "\n\n", styled)
    styled = repair_answer_encoding(styled)
    styled = restore_fenced_code_blocks(styled, code_blocks)
    styled = re.sub(r"([^\n])```", r"\1\n```", styled)
...
def remove_unrequested_bold_markup(text: str, question: str) -> str:
    ...
    no_bold_requested = re.search(r"\b(no bold|without bold|normal letters|normal text|remove bold|not bold)\b", question_lower)
    bold_requested = re.search(r"\b(use bold|bold text|markdown|format with bold|make .* bold)\b", question_lower)
    if bold_requested and not no_bold_requested:
        return text
...
def remove_backend_process_language(text: str, question: str = "") -> str:
    if user_requested_machine_readable_output(question):
        return text.strip()
    parts = re.split(r"(```[\s\S]*?```)", text or "")
    cleaned_parts: List[str] = []
    backend_sentence_patterns = [
        r"\b(?:I|we|the system|Nexora)\s+(?:used|will use|am using|are using|checked|will check)\s+(?:the\s+)?(?:backend|tool|model|route|provider|source button|sources?|current sources|live sources|search|web)\b[^.!?]*(?:[.!?]|$)",
        r"\b(?:source-backed|live-source-backed|backend|provider|metadata|raw output|raw backend output|tool trace|tool traces|model_used|tools_used|route_intent|debug payload|request metadata)\b[^.!?]*(?:[.!?]|$)",
        r"\b(?:should be|is best)\s+understood\s+from\s+reliable\s+background\s+information\s+first[^.!?]*(?:[.!?]|$)",
        r"\b(?:checked|verify|verified)\s+with\s+current\s+sources\s+if\s+the\s+question\s+needs\s+recent\s+details[^.!?]*(?:[.!?]|$)",
        r"\bUse the source button\b[^.!?]*(?:[.!?]|$)",
    ]
    for part in parts:
        if part.startswith("```") and part.endswith("```"):
            cleaned_parts.append(part)
            continue
        cleaned = part
        for pattern in backend_sentence_patterns:
            cleaned = re.sub(pattern, "", cleaned, flags=re.IGNORECASE)
        cleaned = re.sub(r"(?im)^\s*(?:model_used|tools_used|route_intent|provider|backend|debug|raw_response)\s*[:=].*$", "", cleaned)
        cleaned = re.sub(r"\n{3,}", "\n\n", cleaned)
        cleaned_parts.append(cleaned)
    return "".join(cleaned_parts).strip()


def answer_too_thin_after_cleanup(
    question: str,
    text: str,
    response_lane: str = "human_chat",
    presentation_style: str = "balanced",
    ...
    question_lower = clean_text(question).lower()
    if re.search(
        r"\b(one word|single word|one sentence|short sentence|briefly|keep it short|just the answer|only the answer)\b",
        question_lower,
    ):
        return False
    without_labels = re.sub(
        r"(?im)^\s*(?:answer|short answer|key points|details|what it means|how it works|why it matters|bottom line)\s*:\s*",
        "",
        text or "",
    )
    cleaned = clean_text(without_labels)
    if not cleaned:
        return True
    words = cleaned.split()
    topic = clean_text(normalize_prompt_topic(question)).lower().strip(".?! ")
    if topic and cleaned.lower().strip(".?! ") in {topic, f"{topic}."}:
        return True
    task_like = bool(re.search(
        r"\b(explain|what is|what are|who is|who was|why|how|history|overview|plan|steps|write|draft|create|make|compare|teach|describe)\b",
        question_lower,
    ))
    if response_lane != "human_chat" and len(words) < 18:
        return True
    if task_like and len(words) < 24 and presentation_style not in {"short", "minimal"}:
        return True
    return False
...
def finalize_user_facing_answer(
    ...
    text = clean_reply(text)
    text = remove_backend_process_language(text, question)
    text = clean_reply(text)
    text = polish_final_answer_style(text, question, response_lane, presentation_style)

    if (
        repair_depth < 1
        and answer_too_thin_after_cleanup(question, text, response_lane, presentation_style)
    ):
        fallback = local_structured_fallback(question, response_lane, presentation_style)
        if fallback and clean_text(fallback).lower() != clean_text(text).lower():
            repaired = finalize_user_facing_answer(
                question,
                fallback,
                response_lane,
                presentation_style,
            ...
            if not answer_too_thin_after_cleanup(question, repaired, response_lane, presentation_style):
                return repaired

    if not user_requested_machine_readable_output(question) and looks_like_raw_answer_artifact(text):
        fallback = local_structured_fallback(question, response_lane, presentation_style)
        if fallback:
            return finalize_user_facing_answer(question, fallback, response_lane, presentation_style, repair_depth + 1)
        return (
            "I could not turn the model output into a clean answer.\n\n"
            "Bottom line:\n"
            "Send the question once more, and I will answer directly."
        )
    return text
...
def answer_route_quality_flags(question: str, reply: str, task_intent: str = "") -> List[str]:
    ...
    if intent == "agi_capability" and lacks(r"\b(reasoning|planning|memory|tools?|verification|autonomy|not true agi|supervised)\b"):
        flags.append("route_mismatch_agi_capability")
    if intent == "human_like_reasoning" and lacks(r"\b(assumptions?|constraints?|multi[- ]?step|plan|logic|cause|effect|check)\b"):
        flags.append("route_mismatch_human_like_reasoning")
    if intent == "factuality_truth" and lacks(r"\b(verify|source|evidence|uncertain|current|stable|fact[- ]?check|truth)\b"):
        flags.append("route_mismatch_factuality_truth")
    if intent == "robotics_planning" and lacks(r"\b(sensor|actuator|control|simulation|safety|human override|hardware|robot)\b"):
        flags.append("route_mismatch_robotics")
    if intent == "biotech_discovery" and lacks(r"\b(target|molecular|protein|literature|admet|toxicity|validation|lab|hypothesis|safety)\b"):
        flags.append("route_mismatch_biotech")
    if intent == "molecular_structure_prediction" and lacks(r"\b(molecule|protein|structure|prediction|dataset|confidence|validation|lab|uncertainty)\b"):
        flags.append("route_mismatch_molecular_prediction")
    if intent == "openai_product_agi_strategy" and lacks(r"\b(reasoning|workspace agents?|multimodal|voice|video|product|workflow|verify)\b"):
        flags.append("route_mismatch_openai_strategy")
    if intent == "deepmind_unified_ecosystem_strategy" and lacks(r"\b(gemini|ecosystem|android|chrome|workspace|astra|context|screen|assistant)\b"):
        flags.append("route_mismatch_deepmind_strategy")
    if intent == "anthropic_safety_autonomy_strategy" and lacks(r"\b(claude|code|terminal|security|computer use|mcp|safety|permissions?|tools?)\b"):
        flags.append("route_mismatch_anthropic_strategy")
    if intent == "ai_lab_strategy_comparison" and lacks(r"\b(openai|deepmind|google|anthropic|compare|comparison|strength|agent interface|risk|nexora)\b|\|"):
        flags.append("route_mismatch_ai_lab_comparison")
    if intent == "workspace_agents_strategy" and lacks(r"\b(workspace|agent|files?|email|docs?|calendar|permission|workflow|task)\b"):
        flags.append("route_mismatch_workspace_agents")
    if intent == "multimodal_agent_strategy" and lacks(r"\b(multimodal|voice|video|image|text|screen|seen|heard|input)\b"):
        flags.append("route_mismatch_multimodal_agents")
    if intent == "computer_control_robotics_strategy" and lacks(r"\b(computer|desktop|mouse|keyboard|robot|simulation|safety|permission|control)\b"):
        flags.append("route_mismatch_computer_control_robotics")
    if intent == "mcp_interoperability_strategy" and lacks(r"\b(mcp|model context protocol|connector|tools?|data|permission|secure|server|interoperability)\b"):
        flags.append("route_mismatch_mcp_interoperability")
    if intent == "enterprise_agentic_workflow_automation" and lacks(r"\b(workflow|steps?|approval|exception|roi|hours?|cycle time|audit|procurement|invoice|reconcile)\b"):
        flags.append("route_mismatch_enterprise_workflow")
    if intent == "legacy_system_integration" and lacks(r"\b(legacy|salesforce|sap|erp|sql|slack|api|webhook|connector|system of record|silo)\b"):
        flags.append("route_mismatch_legacy_integration")
    if intent == "enterprise_data_privacy_sovereignty" and lacks(r"\b(privacy|data|on[- ]?prem|private cloud|vpc|firewall|isolation|encryption|audit|retention|sovereignty)\b"):
        flags.append("route_mismatch_privacy_sovereignty")
    if intent == "custom_private_model_strategy" and lacks(r"\b(private|knowledge|wiki|tickets?|policies|brand voice|retrieval|knowledge graph|fine[- ]?tuning|evaluation)\b"):
        flags.append("route_mismatch_private_model")
    if intent == "human_in_loop_governance" and lacks(r"\b(human|approval|governance|permission|execute|payment|customer|risk|audit|escalate)\b"):
        flags.append("route_mismatch_human_loop")
    if intent == "measurable_roi_agentic_ai" and lacks(r"\b(roi|hours?|cost|savings?|baseline|cycle time|payback|risk|workflow|measure)\b"):
        flags.append("route_mismatch_agentic_roi")
    if intent == "agentic_reliability_guardrail" and lacks(r"\b(reliability|accurate|error|mistake|validation|approval|monitor|fallback|confidence)\b"):
        flags.append("route_mismatch_reliability_guardrail")
    if intent == "long_term_planning_guardrail" and lacks(r"\b(long[- ]?term|months?|years?|milestones?|state|checkpoint|review|re[- ]?plan|deadline)\b"):
        flags.append("route_mismatch_long_term_planning")
    if intent == "memory_learning_guardrail" and lacks(r"\b(memory|learning|preferences?|corrections?|project context|reviewable|deletable|stale|governed)\b"):
        flags.append("route_mismatch_memory_learning")
    if intent == "real_world_understanding_guardrail" and lacks(r"\b(real[- ]?world|laws?|people|business|change|current|uncertain|evidence|escalate|messy)\b"):
        flags.append("route_mismatch_real_world_understanding")
    return flags
...
def answer_quality_flags(
    ...
    generic_topic_filler = bool(re.search(
        r"\b(understand life, choices|world around them more clearly|good understanding of this subject|memorized idea|useful for students, families, and society|topic becomes truly useful|explained simply and applied thoughtfully|starting with the main idea|break the idea into small parts|connect the concept with real life)\b",
        lower,
    ))
    practical_question = bool(re.search(
        r"\b(plan|routine|schedule|program|steps|workflow|strategy|fix|debug|code|build|create|project|business|fitness|workout|diet|travel|website|app|resume|email|letter|study plan|study tip|study better|tip|advice|automation)\b",
        question_lower,
    ))
    ...
    if re.search(r"\b(manage|organize|organise|schedule|routine|timetable|how should i|how can i)\b", question_lower) and re.search(
        ...
        flags.append("generic_topic_drift")
    if re.search(r"\b(study|tip|advice|quick|focus|revise|revision)\b", question_lower) and re.search(
        r"\b(starting with the main idea|focus on what .* means in simple words|break the idea into small parts|connect the concept with real life)\b",
        lower,
    ):
        flags.append("generic_topic_drift")
...
def answer_word_count(text: str) -> int:
    return len([word for word in re.split(r"\s+", clean_text(text)) if word])


def min_words_for_length_policy(policy_level: str) -> int:
    return {
        "detailed": 420,
        "deep_detailed": 620,
        "maximum_long_form": 820,
    }.get(policy_level, 0)


def deterministic_length_expansion(
    question: str,
    reply: str,
    response_lane: str,
    policy_level: str,
) -> str:
    topic = normalize_prompt_topic(question)
    if topic == "the topic":
        topic = clean_learning_topic(question) or "the topic"
    topic_label = title_case_topic(topic)
    base = clean_reply(reply)
    lower_question = clean_text(question).lower()
    sections: List[str] = []

    if response_lane == "learning" or re.search(r"\b(explain|what is|why|how|study|learn|chapter|concept)\b", lower_question):
        sections.extend([
            "",
            "Detailed breakdown:",
            f"- Main idea: {topic_label} should be understood by separating the definition, the process, examples, and the final takeaway.",
            "- Process: Look for the cause, the steps that happen in order, and the result. This prevents the answer from becoming only a memorized paragraph.",
            "- Key terms: Identify the important words in the answer and connect each one to a simple meaning.",
            "",
            "Examples:",
            f"- Everyday example: Notice where {topic.lower()} appears in real life, then connect it to the explanation above.",
            "- School example: If this appears in an exam, write the definition first, then the steps, then one example.",
            "",
            "Common mistakes:",
            "- Writing only a definition and missing the process.",
            "- Memorizing sentences without understanding why each step happens.",
            "- Giving examples without connecting them back to the main idea.",
            "",
            "How to remember it:",
            "- Ask: What is it? Why does it happen? What are the steps? What is one example? What is the final result?",
            "- If you can answer those five questions, you understand the topic well.",
            "",
            "Why it matters:",
            f"- {topic_label} is not just a definition; it is connected to causes, effects, examples, and real situations.",
            "- Understanding why it happens helps you explain it in your own words instead of copying a memorized answer.",
            "- This also helps you answer different versions of the same question, because you know the idea behind it.",
            "",
            "Practice questions:",
            f"1. What is {topic.lower()} in one clear sentence?",
            f"2. What are the main steps or reasons behind {topic.lower()}?",
            "3. What is one real-life example?",
            "4. What mistake should be avoided when explaining it?",
        ])
        if policy_level == "maximum_long_form":
            sections.extend([
                "",
                "Deeper exam-style structure:",
                "1. Start with a clear definition.",
                "2. Explain the process in order.",
                "3. Add important terms and their meaning.",
                "4. Give one real example.",
                "5. End with why the topic matters.",
            ])
    elif response_lane in {"planning", "build"}:
        sections.extend([
            "",
            "Detailed breakdown:",
            "- Goal: State the exact outcome before choosing steps.",
            "- Steps: Work in order, because skipping setup usually creates mistakes later.",
            "- Checks: After each major step, verify that the result actually matches the user’s goal.",
            "",
            "Common mistakes:",
            "- Solving a nearby problem instead of the actual request.",
            "- Making the answer sound complete without giving usable steps.",
            "- Forgetting to test the final result.",
            "",
            "Practical next step:",
            "- Start with the smallest step that proves the solution is working, then expand from there.",
        ])
    else:
        sections.extend([
            "",
            "More detail:",
            f"- The core of {topic_label} is the main idea, the supporting details, and the practical takeaway.",
            "- A complete answer should explain what it means, why it matters, and how to use or recognize it.",
            "",
            "Useful way to think about it:",
            "- First, identify the main point.",
            "- Next, connect it to examples or real situations.",
            "- Finally, check what action, conclusion, or takeaway follows from it.",
        ])

    sections.extend([
        "",
        "Takeaway:",
        f"{topic_label} becomes easier to understand when the answer gives the idea, the steps or reasons, examples, mistakes to avoid, and the final meaning.",
    ])
    return clean_reply(base + "\n" + "\n".join(sections))


def expand_answer_if_too_short_for_policy(
    question: str,
    reply: str,
    user_id: str,
    response_lane: str,
    ...
    policy = decide_answer_length_policy(user_id, question, response_lane, presentation_style)
    min_words = min_words_for_length_policy(policy["level"])
    current_words = answer_word_count(reply)
    if (
        min_words <= 0
        or current_words >= min_words
        or user_requested_machine_readable_output(question)
        or presentation_style == "short"
        or length_signal_from_text(question) == "short"
        or is_bad_generated_reply(reply)
    ):
        return reply, False, policy["level"]

    expansion_mode = "instant"
    target_words = policy["target"]
    messages: List[Dict[str, str]] = [
        {
            "role": "system",
            "content": (
                "You are Nexora's autonomous answer-length expander. Rewrite the draft into the final user-facing answer. "
                "Follow the autonomous length policy, keep the answer accurate, and do not mention backend, tools, hidden reasoning, or that you expanded it. "
                "Use clean sections, examples, steps, common mistakes or edge cases when useful, and a clear takeaway. "
                f"Length policy: {policy['level']}; target: {target_words}; minimum useful length: about {min_words} words. "
                "Keep the rewrite bounded and responsive: expand enough to satisfy the user's need, but do not ramble or get stuck."
            ),
        },
        {
            "role": "system",
            "content": build_answer_length_context(user_id, question, response_lane, presentation_style),
        },
    ]
    if memory_context:
        messages.append({"role": "system", "content": truncate_for_model(memory_context, 1800)})
    if research_context:
        messages.append({"role": "system", "content": truncate_for_model(research_context, 2600)})
    if file_context:
        messages.append({"role": "system", "content": truncate_for_model(file_context, 2200)})
    messages.append({
        "role": "user",
        "content": (
            f"User question:\n{question}\n\n"
            f"Current draft answer ({current_words} words):\n{reply}\n\n"
            "Rewrite this as the final answer with the correct autonomous length. Keep the meaning accurate, add useful detail, examples, and structure, and answer only the user."
        ),
    })
    try:
        expanded, _model_used, _tools = generate_with_engine_club(
            messages,
            requested_model,
            expansion_mode,
            response_lane,
        ...
        expanded = finalize_user_facing_answer(question, expanded, response_lane, presentation_style)
        expanded_words = answer_word_count(expanded)
        if (
            not is_bad_generated_reply(expanded)
            and expanded_words >= min_words
            and should_accept_reviewed_reply(question, expanded, response_lane, presentation_style)
        ):
            return expanded, True, policy["level"]
    except Exception:
        LOGGER.warning("autonomous_length_expansion_failed", exc_info=True)
    fallback = finalize_user_facing_answer(
        question,
        deterministic_length_expansion(question, reply, response_lane, policy["level"]),
        response_lane,
        presentation_style,
    )
    fallback_words = answer_word_count(fallback)
    blocking = {
        "backend_talk",
        "stream_artifact",
        "traceback_or_error_dump",
    ...
    flags = answer_quality_flags(question, fallback, response_lane, presentation_style, detect_task_intent(question, use_research))
    if (fallback_words >= min_words or fallback_words > current_words + 180) and not any(flag in blocking for flag in flags):
        return fallback, True, policy["level"]
    return reply, False, policy["level"]


def should_use_ollama_only_fast_chat(
    message: str,
    req: ChatRequest,
    use_research: bool,
    task_intent: str,
    ...
    if FREE_API_PROVIDER != "ollama":
        return False
    if use_research or req.use_web:
        return False
    if req.image_ids:
        return False
    if (
        task_intent in {"image_generation", "reminder", "file_summary", "realtime_search", "realtime_current_fact"}
        or task_intent in FRONTIER_ROUTE_NAMES
        or task_intent in AI_LAB_STRATEGY_ROUTE_NAMES
        or task_intent in ENTERPRISE_AGENTIC_ROUTE_NAMES
    ):
        return False
    text = clean_text(message)
    if len(text) > 900:
        return False
    if re.search(r"\b(uploaded|attached|this file|this pdf|this document|summarize file|read file)\b", text.lower()):
        return False
    return True


def compact_ollama_fast_messages(
    message: str,
    history: List[ChatMessage],
    response_lane: str,
    presentation_style: str,
    ...
    system = (
        "You are Nexora, a fast local Ollama assistant. Answer the user's actual question directly. "
        "Use clear reasoning privately, then write only the final answer. Keep it accurate, natural, "
        "and well structured. For normal questions, give a fuller helpful answer with enough context, examples, and practical detail; aim around 180-450 words unless the user asks for a short answer. "
        "Finish cleanly; do not end mid-sentence. "
        "Use saved memory and recent chat context to infer what the user means, especially when wording is informal, vague, or refers to earlier work. "
        "Prefer the user's explicit preferences and corrections over generic assumptions. "
        "Use normal text by default; use Markdown bold only when it genuinely improves scanning or emphasis. "
        "Do not mention backend, routes, tools, providers, logs, raw output, or hidden reasoning."
    )
    if response_lane == "writing":
        system += " For writing tasks, provide the finished draft first."
    elif response_lane == "learning":
        system += " For learning tasks, explain simply but fully with definitions, key facts, examples, and a clear takeaway."
    if presentation_style == "long":
        system += (
            " Long-form detail is required: write a substantially detailed answer, usually 900-1800 words when the topic supports it. "
            "Use multiple clear sections, examples, key steps, common mistakes, and a practical takeaway. Do not stop after a short overview."
        )
    if presentation_style == "short":
        system += " Keep the answer short."
    messages: List[Dict[str, str]] = [{"role": "system", "content": system}]
    if clean_text(memory_context):
        messages.append({
            "role": "system",
            "content": "Relevant saved memory for this user:\n" + truncate_for_model(memory_context, 1200),
        })
    if clean_text(length_context):
        messages.append({
            "role": "system",
            "content": length_context,
        })
    for item in history[-2:]:
        role = item.role if item.role in {"user", "assistant"} else "user"
        content = truncate_for_model(clean_text(item.content), 600)
        if content:
            messages.append({"role": role, "content": content})
    messages.append({"role": "user", "content": truncate_for_model(message, 1200)})
    return messages


def plain_ollama_fast_candidate(message: str, req: ChatRequest) -> bool:
    if FREE_API_PROVIDER != "ollama" or req.use_web or req.image_ids:
        return False
    text = clean_text(message).lower()
    if len(text) > 900:
        return False
    return not re.search(
        r"\b(always|from now|remember|forget|delete|remove|new project|create project|open project|"
        r"generate image|create image|upload|attached|file|pdf|document|remind|timer|alarm|"
        r"latest|current|today|right now|news|price|score|weather|search web)\b",
        text,
    )


def build_fast_memory_context(user_id: str, current_message: str) -> str:
    context = build_memory_context(user_id, current_message)
    if not context:
        return ""
    return truncate_for_model(context, 1200)


def build_ollama_only_fast_chat_response(
    req: ChatRequest,
    session_id: str,
    user_id: str,
    ...
    memory_context = build_fast_memory_context(user_id, understanding_message)
    length_context = build_answer_length_context(user_id, understanding_message, response_lane, presentation_style)
    length_policy = decide_answer_length_policy(user_id, understanding_message, response_lane, presentation_style)
    messages = compact_ollama_fast_messages(
    ...
    tools_used = [
        "ollama_only_fast_path",
        "memory_context" if memory_context else "memory_context_empty",
        f"route_intent:{task_intent}",
        f"response_lane:{response_lane}",
        f"presentation:{presentation_style}",
        f"length_policy:{length_policy['level']}",
    ]
    try:
        ...
        reply = (
            local_reply_for_intent(understanding_message, task_intent, session_id=session_id)
            or local_structured_fallback(understanding_message, response_lane, presentation_style, session_id=session_id)
            or local_resilient_reply(understanding_message, session_id, response_lane, presentation_style)
        )
        tools_used.append("ollama_timeout_local_resilient")
    ...
    if final_quality_flags:
        tools_used.append("quality_gate:fast_ollama:" + ",".join(final_quality_flags[:4]))
    if repaired_intent and final_repaired != final_reply:
        final_reply = final_repaired
        tools_used.append(f"quality_gate:fast_ollama_repaired_by:{repaired_intent}")
    expanded_reply, expanded_by_length, expanded_policy = expand_answer_if_too_short_for_policy(
        original_user_message,
        final_reply,
        user_id,
        response_lane,
    ...
    if expanded_by_length:
        final_reply = expanded_reply
        tools_used.append(f"autonomous_length_expander:{expanded_policy}")
    append_session_exchange(session_id, original_user_message, final_reply)
...
def polish_inline_punctuation(text: str, capitalize: bool) -> str:
    ...
    common_word_fixes = {
        ...
        r"\baccuaracy\b": "accuracy",
        r"\bactractive\b": "attractive",
        r"\bresonable\b": "reasonable",
        r"\breasoing\b": "reasoning",
        r"\bstrucutre\b": "structure",
        r"\bstrucutres\b": "structures",
        r"\bdefeciency\b": "deficiency",
        r"\bdefiecincy\b": "deficiency",
        r"\bteh\b": "the",
        r"\bdont\b": "don't",
        r"\bdoesnt\b": "doesn't",
        r"\bdidnt\b": "didn't",
    ...
    for pattern, replacement in common_word_fixes.items():
...
def wikipedia_fallback_reply(question: str, sources: List[SourceItem]) -> str:
    ...
    lines.extend([
        "",
        "Bottom line:",
        topic_specific_bottom_line(title, question),
    ])
    return "\n".join(lines)


def topic_specific_bottom_line(title: str, question: str = "") -> str:
    topic = clean_text(f"{title} {question}").lower()
    if re.search(r"\b(maurya|mauryan|mourya)\b", topic):
        return "The Maurya Empire matters because it united much of ancient India under a strong central state and became especially influential under Ashoka."
    if re.search(r"\b(empire|civilization|civilisation|kingdom|dynasty|revolution|war|ancient|medieval|history)\b", topic):
        return f"{clean_text(title) or 'This topic'} is best understood through its origin, main leaders or events, and lasting impact."
    return f"{clean_text(title) or 'This topic'} is best understood through its main idea, key details, and real significance."
...
def public_posthog_key() -> str:
    return (
        os.getenv("POSTHOG_PROJECT_API_KEY", "").strip()
        or os.getenv("POSTHOG_API_KEY", "").strip()
        or os.getenv("POSTHOG_PUBLIC_KEY", "").strip()
    )


def posthog_frontend_config() -> Dict[str, Any]:
    enabled = os.getenv("POSTHOG_ENABLED", "true").strip().lower() not in {"0", "false", "off", "no"}
    return {
        "enabled": bool(enabled and public_posthog_key()),
        "publicKey": public_posthog_key(),
        "host": os.getenv("POSTHOG_HOST", "https://us.i.posthog.com").strip() or "https://us.i.posthog.com",
        "sessionRecordingEnabled": os.getenv("POSTHOG_SESSION_RECORDING_ENABLED", "true").strip().lower() not in {"0", "false", "off", "no"},
        "privacyMode": os.getenv("POSTHOG_PRIVACY_MODE", "true").strip().lower() not in {"0", "false", "off", "no"},
        "capturePrompts": os.getenv("POSTHOG_CAPTURE_PROMPTS", "false").strip().lower() in {"1", "true", "yes", "on"},
        "captureResponses": os.getenv("POSTHOG_CAPTURE_RESPONSES", "false").strip().lower() in {"1", "true", "yes", "on"},
        "app": APP_NAME,
        "version": APP_VERSION,
        "environment": ENV,
    }
...
def posthog_config_script_tag() -> str:
    config_json = json.dumps(posthog_frontend_config(), ensure_ascii=False).replace("</", "<\\/")
    return (
        "\n<script id=\"nexora-posthog-config\">"
        f"window.NEXORA_POSTHOG_CONFIG = {config_json};"
        "</script>\n"
        "<script src=\"/posthog.js\" defer></script>\n"
    )


def inject_posthog_loader(html: str) -> str:
    if "nexora-posthog-config" in html:
        return html
    tag = posthog_config_script_tag()
    if "</head>" in html:
        return html.replace("</head>", tag + "</head>", 1)
    return tag + html


@static_router.get("/app", response_class=HTMLResponse)
def web_app() -> HTMLResponse:
    ...
    return HTMLResponse(
        inject_posthog_loader(read_frontend_html(FRONTEND_INDEX)),
...
@static_router.get("/nexora", response_class=HTMLResponse)
def nexora_web_app() -> HTMLResponse:
    return web_app()


@static_router.get("/code", response_class=HTMLResponse)
...
@static_router.get("/observability", response_class=HTMLResponse)
def observability_page() -> HTMLResponse:
    if not OBSERVABILITY_INDEX.exists():
        return HTMLResponse(
            "<h1>Observability dashboard not found</h1><p>Expected frontend/observability.html beside frontend/index.html.</p>",
            status_code=404,
        )
    return HTMLResponse(
        inject_posthog_loader(read_frontend_html(OBSERVABILITY_INDEX)),
        headers={"Cache-Control": "public, max-age=60"},
    )


@static_router.get("/mcp", response_class=HTMLResponse)
def mcp_page() -> HTMLResponse:
    if not MCP_INDEX.exists():
        return HTMLResponse(
            "<h1>21st MCP page not found</h1><p>Expected frontend/mcp.html beside frontend/index.html.</p>",
            status_code=404,
        )
    return HTMLResponse(
        inject_posthog_loader(read_frontend_html(MCP_INDEX)),
        headers={"Cache-Control": "public, max-age=60"},
    )
...
@static_router.get("/dashboard", response_class=HTMLResponse)
def customer_dashboard_page() -> HTMLResponse:
    if not DASHBOARD_INDEX.exists():
        return HTMLResponse(
            "<h1>Software dashboard not found</h1><p>Expected frontend/dashboard.html beside frontend/index.html.</p>",
            status_code=404,
        )
    return HTMLResponse(
        inject_posthog_loader(read_frontend_html(DASHBOARD_INDEX)),
        headers={"Cache-Control": "public, max-age=60"},
    )


@static_router.get("/onboarding", response_class=HTMLResponse)
def onboarding_page() -> HTMLResponse:
    if not ONBOARDING_INDEX.exists():
        return HTMLResponse(
            "<h1>Setup wizard not found</h1><p>Expected frontend/onboarding.html beside frontend/index.html.</p>",
            status_code=404,
        )
    return HTMLResponse(
        inject_posthog_loader(read_frontend_html(ONBOARDING_INDEX)),
        headers={"Cache-Control": "public, max-age=60"},
    )


@static_router.get("/onboarding/", response_class=HTMLResponse)
def onboarding_page_slash() -> HTMLResponse:
    return onboarding_page()


@static_router.get("/guide", response_class=HTMLResponse)
def guide_page() -> HTMLResponse:
    if not GUIDE_INDEX.exists():
        return HTMLResponse(
            "<h1>Software guide not found</h1><p>Expected frontend/guide.html beside frontend/index.html.</p>",
            status_code=404,
        )
    return HTMLResponse(
        inject_posthog_loader(read_frontend_html(GUIDE_INDEX)),
        headers={"Cache-Control": "public, max-age=60"},
    )
...
@static_router.get("/reset-password", response_class=HTMLResponse)
def reset_password_page() -> HTMLResponse:
    if not RESET_PASSWORD_INDEX.exists():
        return HTMLResponse(
            "<h1>Password reset page not found</h1><p>Expected frontend/reset-password.html beside frontend/index.html.</p>",
            status_code=404,
...
@static_router.get("/auth.js", response_class=HTMLResponse)
def auth_controller_script() -> HTMLResponse:
    if not AUTH_CONTROLLER_INDEX.exists():
        return HTMLResponse("console.error('Nexora auth controller not found.');", status_code=404)
    return HTMLResponse(
        AUTH_CONTROLLER_INDEX.read_text(encoding="utf-8"),
        media_type="application/javascript",
        headers={"Cache-Control": "no-store"},
    )


@static_router.get("/posthog.js", response_class=HTMLResponse, include_in_schema=False)
def posthog_browser_script() -> HTMLResponse:
    if not POSTHOG_BROWSER_SCRIPT.exists():
...
@dashboard_router.get("/legacy-health", include_in_schema=False)
def health() -> Dict[str, Any]:
    payload = {
        ...
        "answer_structure_level": ANSWER_STRUCTURE_LEVEL_TEXT,
        "answer_structure_mode": ANSWER_STRUCTURE_MODE,
        "autonomous_self_learning": AUTONOMOUS_SELF_LEARNING,
        "self_learning_layer": "chatgpt_style_per_user_learning_model",
        "memory_stack_mode": MEMORY_STACK_MODE,
        "memory_stack_version": MEMORY_STACK_VERSION,
        "memory_stack": memory_stack_payload(),
        "native_nexora_core": nexora_core_summary(),
        "memory_capacity": MAX_MEMORY_ITEMS,
        "memory_context_items": MAX_MEMORY_CONTEXT_ITEMS,
        "self_learning_rules": MAX_SELF_LEARNING_RULES,
        "self_learning_events": MAX_SELF_LEARNING_EVENTS,
        "self_learning_summaries": MAX_SELF_LEARNING_SUMMARIES,
        "deep": HEALTH_DEEP,
    }
    if HEALTH_DEEP:
        ...
        payload.update({
            ...
            "memory": {
                ...
                "ranking": "keyword_overlap + explicit_preferences + project_context + importance + recency",
                "stack": memory_stack_payload(),
            },
...
@reliability_router.get("/nexora-core/status")
def nexora_core_status() -> Dict[str, Any]:
    model = load_nexora_core_model()
    return {
        "ok": True,
        "summary": nexora_core_summary(model),
        "recent_growth": model.get("growth_log", [])[-12:],
        "recent_events": model.get("recent_events", [])[-12:],
        "goals": model.get("goals", [])[-20:],
    }


@reliability_router.get("/agi/status")
def agi_status_alias() -> Dict[str, Any]:
    status = nexora_core_status()
    status["note"] = "This is NexoraCore inside backend/main.py: a native runtime learning model. Ollama is not required for this layer to learn."
    return status


@reliability_router.post("/nexora-core/feed")
def nexora_core_feed(req: NexoraCoreFeedRequest) -> Dict[str, Any]:
    result = nexora_core_feed_text(
        req.text,
        source=req.source or "manual_feed",
        user_id=req.user_id or "default",
        tags=req.tags or [],
        run_growth_cycle=bool(req.run_growth_cycle),
    )
    return {"ok": True, **result}


@reliability_router.post("/nexora-core/grow")
def nexora_core_grow(req: NexoraCoreGrowthRequest) -> Dict[str, Any]:
    events = []
    for _index in range(req.cycles):
        events.append(nexora_core_growth_cycle(req.source or "manual_growth").get("event", {}))
    return {"ok": True, "cycles": len(events), "events": events, "summary": nexora_core_summary()}


@reliability_router.post("/nexora-core/seed")
def nexora_core_seed() -> Dict[str, Any]:
    return seed_nexora_core("manual_seed")


@reliability_router.get("/nexora-core/context")
def nexora_core_context(q: str = "") -> Dict[str, Any]:
    return {"ok": True, "context": build_nexora_core_context(q), "summary": nexora_core_summary()}


@connector_router.get("/models")
...
@dashboard_router.get("/settings/free-ai")
...
@dashboard_router.post("/settings/free-ai")
...
def search_pipeline_llm_client(model: Optional[str] = None):
    def client(prompt: str) -> str:
        messages = [
            {
                "role": "system",
                "content": (
                    "You are Nexora's grounded search answerer. Use only the supplied source summaries. "
                    "Write a direct, useful answer with inline citations like [S1]. Do not invent facts."
                ),
            },
            {"role": "user", "content": prompt},
        ]
        reply, _model_used, _tools = generate_with_engine_club(
            messages,
            model,
            "instant",
            "research",
...
def execute_search_pipeline_request(req: SearchPipelineRequest) -> Dict[str, Any]:
    if run_nexora_search_pipeline is None:
        raise HTTPException(status_code=503, detail="Search pipeline module is not available.")
    llm_client = search_pipeline_llm_client(req.model) if req.use_llm else None
    try:
        return run_nexora_search_pipeline(
            req.query,
...
@connector_router.get("/search/pipeline")
def search_pipeline(
    q: str,
    max_sources: int = 8,
    use_llm: bool = True,
    ...
    return execute_search_pipeline_request(
        SearchPipelineRequest(
            query=q,
            max_sources=max(5, min(int(max_sources or 8), 10)),
            use_llm=use_llm,
            model=model,
        )
    )
...
@connector_router.post("/search/pipeline")
def search_pipeline_post(req: SearchPipelineRequest) -> Dict[str, Any]:
    return execute_search_pipeline_request(req)


@connector_router.get("/search/realtime")
...
def auth_config(request: Request) -> AuthConfigResponse:
    ...
    return AuthConfigResponse(
        ok=True,
        configured=bool(public_config["configured"]),
        supabase_url=str(public_config["supabase_url"]),
        supabase_anon_key=str(public_config["supabase_anon_key"]),
...
def auth_callback() -> HTMLResponse:
    return HTMLResponse(
        ...
        <!doctype html>
        <html lang="en">
          <head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Nexora Auth</title></head>
          <body>
            <p>Authentication complete. You can return to Nexora.</p>
...
def auth_signup(req: SignupRequest, request: Request) -> AuthResponse:
    try:
        ...
        response = auth_response_from_supabase(
            payload,
            fallback_name=req.name,
            message="Account created. Check your email if verification is enabled.",
        )
        response_user = response.user
        response_user_id = (
            str(response_user.get("id", "")) if isinstance(response_user, dict) else str(getattr(response_user, "id", ""))
        )
        posthog_client.capture_signup(
            distinct_id=response_user_id or "anonymous",
            properties={
                "provider": "supabase",
                "email_domain": normalize_email(req.email).split("@")[-1],
...
def auth_login(req: LoginRequest) -> AuthResponse:
    try:
        ...
        return auth_response_from_supabase(payload, message="Logged in.")
    except SupabaseAuthError as error:
        raise HTTPException(status_code=error.status_code, detail=error.message) from error


@auth_router.post("/auth/logout", response_model=AuthActionResponse)
def auth_logout(request: Request) -> AuthActionResponse:
    token = bearer_token_from_request(request)
    if not token:
        return AuthActionResponse(ok=True, message="Already logged out.")
    try:
        supabase_auth_client.logout(token)
    except SupabaseAuthError as error:
        if error.status_code not in {401, 403}:
            raise HTTPException(status_code=error.status_code, detail=error.message) from error
    return AuthActionResponse(ok=True, message="Logged out.")


@auth_router.post("/auth/forgot-password", response_model=AuthActionResponse)
def auth_forgot_password(req: ForgotPasswordRequest, request: Request) -> AuthActionResponse:
    redirect_to = req.redirect_to or auth_redirect_url(request, "/reset-password")
    try:
        supabase_auth_client.recover_password(normalize_email(req.email), redirect_to)
    except SupabaseAuthError as error:
        raise HTTPException(
            status_code=error.status_code,
            detail=f"Password reset email was not sent: {error.message}",
        ) from error
    return AuthActionResponse(
        ok=True,
        message="If the email belongs to a Nexora account, Supabase has sent a password reset link.",
    )


@auth_router.post("/auth/reset-password", response_model=AuthActionResponse)
def auth_reset_password(req: ResetPasswordRequest, request: Request) -> AuthActionResponse:
    token = req.access_token or bearer_token_from_request(request)
    if not token:
        raise HTTPException(status_code=401, detail="A valid password recovery session is required.")
    try:
        supabase_auth_client.update_password(token, req.new_password)
    except SupabaseAuthError as error:
        raise HTTPException(status_code=error.status_code, detail=error.message) from error
    return AuthActionResponse(ok=True, message="Password updated. Use the new password for your next sign in.")
...
def auth_me(request: Request) -> AuthMeResponse:
    user = optional_authenticated_user(request)
    if not user:
        return AuthMeResponse(ok=False, user=None, message="Not logged in.")
    return AuthMeResponse(ok=True, user=public_user_payload(user), message="Logged in.")
...
@auth_router.get("/auth/memory")
def auth_memory(request: Request) -> Dict[str, Any]:
    user = require_authenticated_user(request)
    user_id = str(user.get("id", ""))
    items = [item for item in load_memory() if normalize_user_id(str(item.get("user_id", "default"))) == user_id]
    return {"ok": True, "user_id": user_id, "memory": items[-MAX_MEMORY_ITEMS:]}
...
@project_router.post("/projects/create")
...
@project_router.delete("/projects/{project_key}")
...
def customer_dashboard_payload(user: Dict[str, Any]) -> Dict[str, Any]:
    user_id = str(user.get("id", ""))
    api_keys = customer_dashboard_store.list_api_keys(user_id)
    user_projects = projects_for_user(user_id)
    overview = ai_observability_store.overview(user_id=user_id)
    requests = ai_observability_store.list_events(20, user_id=user_id)
    traces = ai_observability_store.traces(10, user_id=user_id)
    sessions = ai_observability_store.sessions(10, user_id=user_id)
    onboarding = {
        "signed_in": True,
        "api_key_created": any(not item.get("revoked") for item in api_keys),
        "project_connected": bool(user_projects),
        "events_received": bool(requests),
    }
    if not onboarding["api_key_created"]:
        phase = "Create API key"
        next_action = "Create an API key, then copy it into your project."
    elif not onboarding["project_connected"]:
        phase = "Connect project"
        next_action = "Add your first project so events have a home."
    elif not onboarding["events_received"]:
        phase = "Send first event"
        next_action = "Send a test event or call Software from your project."
    else:
        phase = "Monitoring"
        next_action = "Review latency, errors, traces, and token usage."
    return {
        "ok": True,
        "user": public_user_payload(user),
        "state": {
...
@dashboard_router.get("/api/dashboard/customer")
def customer_dashboard(request: Request) -> Dict[str, Any]:
    return customer_dashboard_payload(require_authenticated_user(request))


@dashboard_router.get("/api/keys")
def list_customer_api_keys(request: Request) -> Dict[str, Any]:
    user = require_authenticated_user(request)
    return {"ok": True, "api_keys": customer_dashboard_store.list_api_keys(str(user.get("id", "")))}


@dashboard_router.post("/api/keys/create")
def create_customer_api_key(req: CustomerApiKeyRequest, request: Request) -> Dict[str, Any]:
    user = require_authenticated_user(request)
    created = customer_dashboard_store.create_api_key(str(user.get("id", "")), req.name)
    return {"ok": True, **created, "dashboard": customer_dashboard_payload(user)}


@dashboard_router.delete("/api/keys/{key_id}")
def revoke_customer_api_key(key_id: str, request: Request) -> Dict[str, Any]:
    user = require_authenticated_user(request)
    revoked = customer_dashboard_store.revoke_api_key(str(user.get("id", "")), key_id)
    return {"ok": True, "revoked": revoked, "api_keys": customer_dashboard_store.list_api_keys(str(user.get("id", "")))}


@project_router.get("/api/projects")
def list_customer_projects(request: Request) -> Dict[str, Any]:
    user = require_authenticated_user(request)
    return {"ok": True, "projects": projects_for_user(str(user.get("id", "")))}


@project_router.post("/api/projects")
def create_customer_project_endpoint(req: CustomerProjectRequest, request: Request) -> Dict[str, Any]:
    user = require_authenticated_user(request)
    project = create_customer_project(req, str(user.get("id", "")))
    return {"ok": True, "project": project, "dashboard": customer_dashboard_payload(user)}
...
def api_key_from_request(request: Request) -> str:
    ...
    if auth.lower().startswith("bearer "):
        return auth.split(" ", 1)[1].strip()
    return request.headers.get("X-Software-API-Key", "").strip() or request.headers.get("X-Nexora-API-Key", "").strip()


@dashboard_router.post("/api/events/ingest")
def ingest_customer_event(req: CustomerEventIngestRequest, request: Request) -> Dict[str, Any]:
    supplied_api_key = api_key_from_request(request)
    key_record = customer_dashboard_store.authenticate_api_key(supplied_api_key)
    key_source = "dashboard"
    if not key_record:
        key_record = onboarding_store.authenticate_api_key(supplied_api_key)
        key_source = "onboarding" if key_record else "dashboard"
    if not key_record:
        raise HTTPException(
            status_code=401,
            detail="Authentication required for this cloud feature. You can still install and use the SDK locally without signing in.",
        )
    user_id = str(key_record.get("user_id", ""))
    trace_id = req.trace_id or f"trace_{uuid.uuid4().hex[:12]}"
    ...
    event = ai_observability_store.record_ai_request(
        ...
        error=req.error if not req.success else None,
        session_id=session_id,
        metadata={
            **req.metadata,
            "user_id": user_id,
            ...
            "onboarding_id": key_record.get("onboarding_id", ""),
        },
    )
    return {"ok": True, "event": event}


@project_router.get("/artifacts")
...
@project_router.post("/artifacts/save")
...
@project_router.delete("/artifacts/{artifact_id}")
...
@reliability_router.get("/capabilities")
    ...
    return {
        ...
        "stages": {
            ...
            "memory_stack": [
                ...
                "working memory",
                "retrieval ranking",
                "reasoning and final style cleanup",
...
@connector_router.post("/files/summarize")
def summarize_file(req: FileSummaryRequest, request: Request) -> Dict[str, Any]:
    session = get_session(req.session_id)
    user_id = register_request_user(request, req.user_id or str(session.get("user_id", "default")), req.session_id)
    files = session.get("files", [])
    selected = None
    if req.filename:
...
@dashboard_router.post("/study/plan")
...
@dashboard_router.post("/website/generate")
...
@project_router.post("/reminders/create")
...
@project_router.post("/reminders/{reminder_id}/done")
def complete_reminder(reminder_id: str, request: Request) -> Dict[str, Any]:
    user_id = normalize_user_id(resolve_user_id_for_request(request))
    items = load_reminders()
    for item in items:
        if item.get("id") == reminder_id and normalize_user_id(str(item.get("user_id", "default"))) == user_id:
            item["done"] = True
...
@project_router.post("/workflows/create")
...
@project_router.post("/workflows/{workflow_id}/run")
def run_workflow(workflow_id: str, request: Request, user_id: Optional[str] = None) -> Dict[str, Any]:
    resolved_user = normalize_user_id(resolve_user_id_for_request(request, user_id))
    workflow = next(
        (
            item for item in load_workflows()
            if item.get("id") == workflow_id
            and normalize_user_id(str(item.get("user_id", "default"))) == resolved_user
        ),
        None,
    )
    if not workflow:
...
@connector_router.post("/email/draft")
...
@connector_router.post("/automation/plan")
...
@connector_router.post("/image/upload", response_model=ImageUploadResponse)
...
@connector_router.post("/image/generate", response_model=ImageResponse)
...
def local_memory_capture_reply(message: str) -> Optional[str]:
    text = clean_text(message)
    lower = text.lower()
    if re.search(r"\b(forget|delete memory|remove memory|do not remember|don't remember)\b", lower):
        return None
    if not re.search(r"\b(remember that|remember this|save this|keep this in memory|store this|memorize this)\b", lower):
        return None
    detail = re.sub(
        r"(?i)^\s*(please\s+)?(?:remember that|remember this|save this|keep this in memory|store this|memorize this)\s*[:\-]?\s*",
        "",
        text,
    ).strip()
    if detail:
        return f"Got it. I will remember: {detail}"
    return "Got it. I will remember that for future replies."


LOCAL_SPECIALIST_FAST_INTENTS = {
    "fitness_plan",
    "study_management_plan",
    "study_plan",
    "study_timetable",
...
def local_specialist_fast_response(
    req: ChatRequest,
    session_id: str,
    user_id: str,
    original_user_message: str,
    ...
    lower = clean_text(understanding_message).lower()
    if req.use_web or req.image_ids:
        return None
    if re.search(r"\b(latest|current|today|right now|recent|news|price|score|weather|sources?|citations?|verify)\b", lower):
        return None
    if task_intent not in LOCAL_SPECIALIST_FAST_INTENTS:
        return None

    response_lane = classify_response_lane(understanding_message, False)
    presentation_style = classify_presentation_style(understanding_message, response_lane, False)
    reply = (
        local_reply_for_intent(understanding_message, task_intent, session_id=session_id)
        or local_structured_fallback(understanding_message, response_lane, presentation_style, session_id=session_id)
    )
    ...
    final_reply = finalize_user_facing_answer(original_user_message, reply, response_lane, presentation_style)
    final_reply, quality_flags, repaired_intent = quality_gate_local_repair(
        original_user_message,
        final_reply,
        response_lane,
        presentation_style,
    ...
    tools_used = [
        "local_specialist_fast_path",
        f"route_intent:{task_intent}",
        f"response_lane:{response_lane}",
        f"presentation:{presentation_style}",
        f"quality_gate:{QUALITY_GATE_MODE}",
    ]
    if quality_flags:
        tools_used.append("quality_gate:local_specialist:" + ",".join(quality_flags[:4]))
    if repaired_intent:
        tools_used.append(f"quality_gate:local_specialist_repaired_by:{repaired_intent}")
    return ChatResponse(
        reply=final_reply,
        session_id=session_id,
        mode=req.mode or "agent",
        model_used="nexora_local_specialist",
        sources=[],
        tools_used=tools_used,
        route_intent=task_intent,
        created_at=now_iso(),
...
@dashboard_router.post("/chat", response_model=ChatResponse)
def chat(req: ChatRequest, request: Request) -> ChatResponse:
    started = time.perf_counter()
    trace_id = f"chat_{uuid.uuid4().hex}"
    try:
        response = _chat_impl(req, request)
    except Exception as error:
        latency_ms = int((time.perf_counter() - started) * 1000)
        posthog_client.capture_ai_generation(
            distinct_id=posthog_distinct_id_for_chat(req, request, req.session_id),
            trace_id=trace_id,
            model=req.model or "unknown",
            provider=posthog_client.provider_from_model(req.model or ""),
            input_text=req.message,
            output_text=None,
            latency_ms=latency_ms,
            error=f"{type(error).__name__}: {error}",
            session_id=req.session_id,
            metadata={"route": "/chat", "mode": req.mode or "agent"},
        )
        raise

    latency_ms = int((time.perf_counter() - started) * 1000)
    provider_failures = [
        item for item in response.tools_used
        if any(marker in item.lower() for marker in ("failed", "timeout", "cooldown", "fallback"))
    ][:12]
    posthog_client.capture_ai_generation(
        distinct_id=posthog_distinct_id_for_chat(req, request, response.session_id),
        trace_id=trace_id,
        model=response.model_used,
        provider=posthog_client.provider_from_model(response.model_used),
        input_text=req.message,
    ...
    posthog_client.capture_feature_usage(
        posthog_distinct_id_for_chat(req, request, response.session_id),
        "chat",
        {
            "route_intent": response.route_intent,
            "model": response.model_used,
...
def _chat_impl(req: ChatRequest, request: Request) -> ChatResponse:
    ...
    memory_capture_reply = local_memory_capture_reply(original_user_message)
    if memory_capture_reply:
        append_session_exchange(session_id, original_user_message, memory_capture_reply)
        maybe_store_memory(original_user_message, user_id)
        learn_long_term_memory_from_chat(
            original_user_message,
        ...
        return ChatResponse(
            reply=memory_capture_reply,
            session_id=session_id,
            mode=req.mode or "agent",
            model_used="nexora_memory_capture",
            sources=[],
            tools_used=["memory_capture", f"memory_capacity:{MAX_MEMORY_ITEMS}", f"memory_context_items:{MAX_MEMORY_CONTEXT_ITEMS}", f"understanding_level:{UNDERSTANDING_LEVEL_TEXT}"],
            route_intent="memory_capture",
            created_at=now_iso(),
        )
    quick_reply = local_fast_reply(original_user_message)
    if quick_reply:
        quick_intent = detect_task_intent(original_user_message, False)
        _quick_behavior_profile, quick_behavior_signals = learn_behavior_from_message(original_user_message)
        quick_reply = finalize_user_facing_answer(original_user_message, quick_reply, "human_chat", "short")
        append_session_exchange(session_id, original_user_message, quick_reply)
        maybe_store_memory(original_user_message, user_id)
        learn_long_term_memory_from_chat(
            original_user_message,
            quick_reply,
    ...
    normalized_for_local, _typing_corrections_for_local = normalize_user_language(original_user_message)
    understanding_for_local = normalized_for_local or original_user_message
    local_specialist_intent = detect_task_intent(understanding_for_local, False)
    local_specialist_reply = local_specialist_fast_response(
        req,
        session_id,
        user_id,
        original_user_message,
        understanding_for_local,
        local_specialist_intent,
    )
    if local_specialist_reply:
        return local_specialist_reply

    if plain_ollama_fast_candidate(original_user_message, req):
        ...
        if should_use_ollama_only_fast_chat(understanding_message, req, use_research, task_intent):
            ...
            return build_ollama_only_fast_chat_response(
                req,
                session_id,
                user_id,
                original_user_message,
    ...
    task_intent = detect_task_intent(understanding_message, use_research)
    local_strategy_intent = detect_task_intent(understanding_message, False)
    if (
        use_research
        and (local_strategy_intent in AI_LAB_STRATEGY_ROUTE_NAMES or local_strategy_intent in ENTERPRISE_AGENTIC_ROUTE_NAMES)
        and not re.search(
            r"\b(latest|current|today|recent|news|verify|fact[- ]?check|sources?|citations?|available|released|updated|as of|currently|right now)\b",
            clean_text(understanding_message).lower(),
        )
    ):
        use_research = False
        response_lane = classify_response_lane(understanding_message, use_research)
        presentation_style = classify_presentation_style(understanding_message, response_lane, use_research)
        research_route = autonomous_research_route(understanding_message, response_lane, presentation_style, use_research)
        task_intent = local_strategy_intent
    if should_use_ollama_only_fast_chat(understanding_message, req, use_research, task_intent):
        history_limit = 2
        history_from_session = [
            ChatMessage(role=item.get("role", "user"), content=item.get("content", ""))
    ...
    memory_context = build_memory_context(user_id, understanding_message, session_id=session_id, session=session)
    if memory_context and not use_research:
    ...
    local_first_stable = response_lane == "writing" or task_intent in LANGUAGE_ROUTE_NAMES or task_intent in FRONTIER_ROUTE_NAMES or task_intent in AI_LAB_STRATEGY_ROUTE_NAMES or task_intent in ENTERPRISE_AGENTIC_ROUTE_NAMES or is_high_confidence_local_reply(understanding_message, presentation_style) or (
    ...
    ollama_cleaned_preview = clean_reply(reply) if not is_bad_generated_reply(reply) else ""
    if (
        model_used.startswith("ollama:")
        and "quality_guard:pollinations_review" not in tools_used
        and should_pollinations_strengthen_ollama_reply(
            original_user_message,
            reply,
            ollama_cleaned_preview,
            response_mode,
            response_lane,
            presentation_style,
        )
        ...
        if reviewed_reply and should_accept_reviewed_reply(original_user_message, reviewed_reply, response_lane, presentation_style):
            reply = reviewed_reply
            tools_used.append("pollinations_strengthen_ollama")
    ...
    if (
        ...
        and "quality_guard:pollinations_review" not in tools_used
        and "pollinations_strengthen_ollama" not in tools_used
    ...
    if expanded_by_length:
        final_reply = expanded_reply
        if response_sources:
            final_reply = ensure_inline_citations(final_reply, response_sources)
        tools_used.append(f"autonomous_length_expander:{expanded_policy}")
    append_session_message(session_id, "user", original_user_message)
    append_session_message(session_id, "assistant", final_reply)
    maybe_store_memory(original_user_message, user_id)
...
@connector_router.post("/upload", response_model=UploadResponse)
async def upload_file(
    ...
    session_id = ensure_session(session_id)
    owner_id = register_request_user(request, user_id, session_id)
...
@dashboard_router.delete("/sessions/{session_id}")
...
@dashboard_router.get("/memory/stack")
def read_memory_stack(
    request: Request,
    user_id: Optional[str] = None,
    session_id: Optional[str] = None,
    q: str = "",
    ...
    context = build_memory_context(
        normalized,
        q or clean_text(str(session.get("last_user_message", ""))),
    ...
    return {
        ...
        "session_id": session_id or "",
        "stack": memory_stack_payload(),
        "context_preview": truncate_for_model(context, 2400),
        "memory_signature": memory_signature_for_user(normalized),
    }


@dashboard_router.post("/memory/clear")
...
@dashboard_router.post("/persona/reset")
...
@dashboard_router.post("/feedback")
...
def include_architecture_routers() -> None:
    for router in (
        auth_router,
        billing_router,
        sdk_router,
